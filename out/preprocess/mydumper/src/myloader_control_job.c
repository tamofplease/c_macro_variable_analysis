# 0 "/mydumper/src/myloader_control_job.c"
# 1 "/build//"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "/mydumper/src/myloader_control_job.c"
# 17 "/mydumper/src/myloader_control_job.c"
...
# 20 "/mydumper/src/myloader_control_job.c" 2
...
# 21 "/mydumper/src/myloader_control_job.c" 2
...
# 26 "/mydumper/src/myloader_common.h" 2

# 26 "/mydumper/src/myloader_common.h"
...
# 22 "/mydumper/src/myloader_control_job.c" 2
...
# 23 "/mydumper/src/myloader_control_job.c" 2
...
# 24 "/mydumper/src/myloader_control_job.c" 2
gboolean intermediate_queue_ended_local=(0);
extern gboolean innodb_optimize_keys_per_table;
extern guint num_threads;
extern gboolean resume;
extern GHashTable *tbl_hash;

gboolean dont_wait_for_schema_create=(0);

struct control_job * new_job (enum control_job_type type, void *job_data, char *use_database) {
  struct control_job *j = (struct control_job *) (__extension__ ({ gsize __n = (gsize) (1); gsize __s = sizeof (struct control_job); gpointer __p; if (__s == 1) __p = g_malloc0 (__n); else if (__builtin_constant_p (__n) && (__s == 0 || __n <= 
# 33 "/mydumper/src/myloader_control_job.c" 3 4
                         (0x7fffffffffffffffL * 2UL + 1UL) 
# 33 "/mydumper/src/myloader_control_job.c"
...
gboolean process_job(struct thread_data *td, struct control_job *job){
  switch (job->type) {
    ...
    case JOB_WAIT:
      ...
      g_async_queue_push(td->conf->ready, ((gpointer) (glong) (1)));
    ...
    default:
      g_free(job);
      g_log (((gchar*) 0), G_LOG_LEVEL_CRITICAL, "Something very bad happened!(1)");
      exit(
# 67 "/mydumper/src/myloader_control_job.c" 3 4
          1
# 67 "/mydumper/src/myloader_control_job.c"
                      );
  }
  g_free(job);
  return (!(0));
...
void schema_file_missed_lets_continue(struct thread_data * td){
  ...
  guint i=0;
  while (iter != 
# 76 "/mydumper/src/myloader_control_job.c" 3 4
                ((void *)0)
# 76 "/mydumper/src/myloader_control_job.c"
                    ){
...
gboolean are_we_waiting_for_schema_jobs_to_complete(struct thread_data * td){
  ...
  GList * iter=td->conf->table_list;
  while (iter != 
# 95 "/mydumper/src/myloader_control_job.c" 3 4
                ((void *)0)
# 95 "/mydumper/src/myloader_control_job.c"
                    ){
    struct db_table * dbt = iter->data;
    g_mutex_lock(dbt->mutex);
...
gboolean are_we_waiting_for_create_schema_jobs_to_complete(struct thread_data * td){
  ...
  GList * iter=td->conf->table_list;
  while (iter != 
# 116 "/mydumper/src/myloader_control_job.c" 3 4
                ((void *)0)
# 116 "/mydumper/src/myloader_control_job.c"
                    ){
    struct db_table * dbt = iter->data;
    g_mutex_lock(dbt->mutex);
    if (dbt->schema_state==CREATING){
...
gboolean are_available_jobs(struct thread_data * td){
  ...
  while (iter != 
# 135 "/mydumper/src/myloader_control_job.c" 3 4
                ((void *)0)
# 135 "/mydumper/src/myloader_control_job.c"
                    ){
    struct db_table * dbt = iter->data;
    g_mutex_lock(dbt->mutex);
...
void create_index_job(struct configuration *conf, struct db_table * dbt, guint tdid){
  if (dbt->indexes != 
# 151 "/mydumper/src/myloader_control_job.c" 3 4
                     ((void *)0) 
# 151 "/mydumper/src/myloader_control_job.c"
                          && ! dbt->index_enqueued){

      g_log (((gchar*) 0), G_LOG_LEVEL_MESSAGE, "Thread %d: Enqueuing index for table: `%s`.`%s`", tdid, dbt->database, dbt->table);
...
struct restore_job * give_me_next_data_job(struct thread_data * td, gboolean test_condition){
  ...
  GList * iter=td->conf->table_list;
  GList * next = 
# 163 "/mydumper/src/myloader_control_job.c" 3 4
                ((void *)0)
# 163 "/mydumper/src/myloader_control_job.c"
                    ;
  struct restore_job *job = 
# 164 "/mydumper/src/myloader_control_job.c" 3 4
                           ((void *)0)
# 164 "/mydumper/src/myloader_control_job.c"
                               ;


  while (iter != 
# 167 "/mydumper/src/myloader_control_job.c" 3 4
                ((void *)0)
# 167 "/mydumper/src/myloader_control_job.c"
                    ){
    struct db_table * dbt = iter->data;
    if (dbt->completed){
      iter=iter->next;
...
struct restore_job * give_any_data_job(struct thread_data * td){
 return give_me_next_data_job(td,(0));
}
...
void enqueue_indexes_if_possible(struct configuration *conf){
  ...
  struct db_table * dbt;
  while (iter != 
# 230 "/mydumper/src/myloader_control_job.c" 3 4
                ((void *)0)
# 230 "/mydumper/src/myloader_control_job.c"
                    ){
    dbt = iter->data;
    g_mutex_lock(dbt->mutex);
    if (dbt->schema_state==CREATED && !dbt->index_enqueued){
      if (intermediate_queue_ended_local && ((__extension__ ({ _Static_assert (sizeof *(&(dbt->remaining_jobs)) == sizeof (gint), "Expression evaluates to false"); gint gaig_temp; (void) (0 ? *(&(dbt->remaining_jobs)) ^ *(&(dbt->remaining_jobs)) : 1); __atomic_load ((gint *)(&(dbt->remaining_jobs)), &gaig_temp, 5); (gint) gaig_temp; })) == 0)){
        create_index_job(conf, dbt, 0);
# 246 "/mydumper/src/myloader_control_job.c"
      }
    }
    g_mutex_unlock(dbt->mutex);
    iter=iter->next;
...
void *process_stream_queue(struct thread_data * td) {
  struct control_job *job = 
# 255 "/mydumper/src/myloader_control_job.c" 3 4
                           ((void *)0)
# 255 "/mydumper/src/myloader_control_job.c"
                               ;
  gboolean cont=(!(0));
  enum file_type ft=-1;
...
# 260 "/mydumper/src/myloader_control_job.c" 3 4
                        ((void *)0)
# 260 "/mydumper/src/myloader_control_job.c"
                            ;
  guint pass=0;
  ...
  while (cont){
    ...
    ft=(enum file_type)((gint) (glong) (g_async_queue_pop(td->conf->stream_queue)));
    switch (ft){
    case SCHEMA_CREATE:
      job=g_async_queue_pop(td->conf->database_queue);
      if (job->type != JOB_SHUTDOWN){
        g_log (((gchar*) 0), G_LOG_LEVEL_DEBUG, "Restoring database");
      ...
      break;
    case SCHEMA_TABLE:
# 302 "/mydumper/src/myloader_control_job.c"
    ...
    if (real_db_name->schema_created){
      ...
      g_async_queue_push(td->conf->stream_queue,((gpointer) (glong) (ft)));
      if (max_jobs_to_wait > 0 && pass>max_jobs_to_wait){
        g_log (((gchar*) 0), G_LOG_LEVEL_MESSAGE, "Max jobs waited (%d) for schema: %s", max_jobs_to_wait, real_db_name->name);
        real_db_name->schema_created=(!(0));
    ...
    case DATA:
      rj = give_me_next_data_job(td, (!(0)));
      if (rj != 
# 325 "/mydumper/src/myloader_control_job.c" 3 4
               ((void *)0)
# 325 "/mydumper/src/myloader_control_job.c"
                   ){
        ...
        if (rj != 
# 333 "/mydumper/src/myloader_control_job.c" 3 4
                 ((void *)0)
# 333 "/mydumper/src/myloader_control_job.c"
                     ){
          g_log (((gchar*) 0), G_LOG_LEVEL_DEBUG, "Thread %d: Giving any data job", td->thread_id);
          job=new_job(JOB_RESTORE,rj,rj->dbt->database);
      ...
      case INTERMEDIATE_ENDED:
        g_async_queue_push(td->conf->database_queue, new_job(JOB_SHUTDOWN,
# 348 "/mydumper/src/myloader_control_job.c" 3 4
                                                                         ((void *)0)
# 348 "/mydumper/src/myloader_control_job.c"
                                                                             ,
# 348 "/mydumper/src/myloader_control_job.c" 3 4
                                                                              ((void *)0)
# 348 "/mydumper/src/myloader_control_job.c"
        ...
        if ( are_available_jobs(td) ){
          if (!are_we_waiting_for_schema_jobs_to_complete(td)){
            g_log (((gchar*) 0), G_LOG_LEVEL_WARNING, "Schema files missed, we continue...");
            schema_file_missed_lets_continue(td);
          }
          pass++;
        }
        max_jobs_to_wait=g_async_queue_length(td->conf->stream_queue)+1;
        intermediate_queue_ended_local=(!(0));
      default:
        
# 359 "/mydumper/src/myloader_control_job.c" 3 4
       ((void *)0)
# 359 "/mydumper/src/myloader_control_job.c"
           ;
    ...
    if (intermediate_queue_ended_local) {
      if ( are_available_jobs(td) ){
        ...
        g_async_queue_push(td->conf->stream_queue, ((gpointer) (glong) (SHUTDOWN)));
      }
    }
  }
  enqueue_indexes_if_possible(td->conf);
  g_log (((gchar*) 0), G_LOG_LEVEL_MESSAGE, "Thread %d: Data import ended", td->thread_id);
  return 
# 383 "/mydumper/src/myloader_control_job.c" 3 4
        ((void *)0)
# 383 "/mydumper/src/myloader_control_job.c"
            ;
}
