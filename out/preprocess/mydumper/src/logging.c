# 0 "/mydumper/src/logging.c"
# 1 "/build//"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "/mydumper/src/logging.c"
# 21 "/mydumper/src/logging.c"
...
void write_log_file(const gchar *log_domain, GLogLevelFlags log_level,
  ...
  if (write(fileno(logoutfile), message_out->str, message_out->len) <= 0) {
    fprintf(
# 73 "/mydumper/src/logging.c" 3 4
           stderr
# 73 "/mydumper/src/logging.c"
                 , "Cannot write to log file with error %d.  Exiting...",
            
# 74 "/mydumper/src/logging.c" 3 4
           (*__errno_location ())
# 74 "/mydumper/src/logging.c"
                );
  }
  g_string_free(message_out, (!(0)));
}
