# 0 "/mydumper/src/common.c"
# 1 "/build//"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "/mydumper/src/common.c"
# 15 "/mydumper/src/common.c"
...
# 25 "/mydumper/src/common.c" 2
# 1 "/mydumper/src/config.h" 1
# 26 "/mydumper/src/common.c" 2
...
# 32 "/mydumper/src/common.c" 3 4
                           ((void *)0)
# 32 "/mydumper/src/common.c"
                               ;
...
char *generic_checksum(MYSQL *conn, char *database, char *table, int *errn,const gchar *query_template, int column_number){
  MYSQL_RES *result = 
# 45 "/mydumper/src/common.c" 3 4
                     ((void *)0)
# 45 "/mydumper/src/common.c"
                         ;
  MYSQL_ROW row;
  *errn=0;
  char *query = g_strdup_printf(query_template, database, table);
  if (mysql_query(conn, query) || !(result = mysql_use_result(conn))) {
    g_log (((gchar*) 0), G_LOG_LEVEL_CRITICAL, "Error dumping checksum (%s.%s): %s", database, table, mysql_error(conn));
    *errn=mysql_errno(conn);
    g_free(query);
    return 
# 53 "/mydumper/src/common.c" 3 4
          ((void *)0)
# 53 "/mydumper/src/common.c"
              ;
  }
  ...
  char * r=
# 58 "/mydumper/src/common.c" 3 4
          ((void *)0)
# 58 "/mydumper/src/common.c"
              ;
  if (row != 
# 59 "/mydumper/src/common.c" 3 4
            ((void *)0)
# 59 "/mydumper/src/common.c"
                ) r=g_strdup_printf("%s",row[column_number]);
  g_free(query);
...
GKeyFile * load_config_file(gchar * config_file){
  GError *error = 
# 97 "/mydumper/src/common.c" 3 4
                 ((void *)0)
# 97 "/mydumper/src/common.c"
                     ;
  GKeyFile *kf = g_key_file_new ();

  if (!g_key_file_load_from_file (kf, config_file,
                                  G_KEY_FILE_KEEP_COMMENTS, &error)) {
    g_log (((gchar*) 0), G_LOG_LEVEL_WARNING, "Failed to load config file %s: %s", config_file, error->message);
    return 
# 103 "/mydumper/src/common.c" 3 4
          ((void *)0)
# 103 "/mydumper/src/common.c"
              ;
  }
  return kf;
}
...
void load_config_group(GKeyFile *kf, GOptionContext *context, const gchar * group){
  gsize len=0;
  GError *error = 
# 110 "/mydumper/src/common.c" 3 4
                 ((void *)0)
# 110 "/mydumper/src/common.c"
                     ;
  gchar ** keys=g_key_file_get_keys(kf,group, &len, &error);
  gsize i=0;
  GSList *list = 
# 113 "/mydumper/src/common.c" 3 4
                ((void *)0)
# 113 "/mydumper/src/common.c"
                    ;
  if (error != 
# 114 "/mydumper/src/common.c" 3 4
              ((void *)0)
# 114 "/mydumper/src/common.c"
                  ){
    g_log (((gchar*) 0), G_LOG_LEVEL_WARNING, "loading %s: %s",group,error->message);
  }else{
    ...
    for (i=0; i < len; i++){
      if (g_strcmp0("host",keys[i]) && g_strcmp0("user",keys[i]) && g_strcmp0("password",keys[i])){
        ...
        if ( value != 
# 122 "/mydumper/src/common.c" 3 4
                     ((void *)0) 
# 122 "/mydumper/src/common.c"
                          ) list=g_slist_append(list, value);
      }
    }
    gint slen = g_slist_length(list) + 1;
    gchar ** gclist = (gchar * *) (__extension__ ({ gsize __n = (gsize) (slen); gsize __s = sizeof (gchar *); gpointer __p; if (__s == 1) __p = g_malloc0 (__n); else if (__builtin_constant_p (__n) && (__s == 0 || __n <= 
# 126 "/mydumper/src/common.c" 3 4
                     (0x7fffffffffffffffL * 2UL + 1UL) 
# 126 "/mydumper/src/common.c"
    ...
    if (!g_option_context_parse(context, &slen, &gclist, &error)) {
      ...
      exit(
# 137 "/mydumper/src/common.c" 3 4
          1
# 137 "/mydumper/src/common.c"
                      );
    }else{
      g_log (((gchar*) 0), G_LOG_LEVEL_MESSAGE, "Config file loaded");
    }
    g_strfreev(gclist);
  }
  g_strfreev(keys);
...
void load_session_hash_from_key_file(GKeyFile *kf, GHashTable * set_session_hash, const gchar * group_variables){
  guint i=0;
  GError *error = 
# 148 "/mydumper/src/common.c" 3 4
                 ((void *)0)
# 148 "/mydumper/src/common.c"
                     ;
  gchar *value=
# 149 "/mydumper/src/common.c" 3 4
              ((void *)0)
# 149 "/mydumper/src/common.c"
                  ;
  gsize len=0;
...
void load_per_table_info_from_key_file(GKeyFile *kf, struct configuration_per_table * conf_per_table, fun_ptr get_function_pointer_for()){
  ...
  GHashTable *ht=
# 163 "/mydumper/src/common.c" 3 4
                ((void *)0)
# 163 "/mydumper/src/common.c"
                    ;
  GError *error = 
# 164 "/mydumper/src/common.c" 3 4
                 ((void *)0)
# 164 "/mydumper/src/common.c"
                     ;
  guint i=0,j=0;
  gchar *value=
# 166 "/mydumper/src/common.c" 3 4
              ((void *)0)
# 166 "/mydumper/src/common.c"
                  ;
  gchar **keys=
# 167 "/mydumper/src/common.c" 3 4
              ((void *)0)
# 167 "/mydumper/src/common.c"
                  ;
  for (i=0; i < len; i++){
    if (g_strstr_len(groups[i],strlen(groups[i]),"`.`") && g_str_has_prefix(groups[i],"`") && g_str_has_suffix(groups[i],"`")){
      ...
      for (j=0; j < len2; j++){
        if (g_str_has_prefix(keys[j],"`") && g_str_has_suffix(keys[j],"`")){
          ...
          struct function_pointer *fp = (struct function_pointer *) (__extension__ ({ gsize __n = (gsize) (1); gsize __s = sizeof (struct function_pointer); gpointer __p; if (__s == 1) __p = g_malloc0 (__n); else if (__builtin_constant_p (__n) && (__s == 0 || __n <= 
# 175 "/mydumper/src/common.c" 3 4
                                       (0x7fffffffffffffffL * 2UL + 1UL) 
# 175 "/mydumper/src/common.c"
                                       / __s)) __p = g_malloc0 (__n * __s); else __p = g_malloc0_n (__n, __s); __p; }));
...
void free_hash_table(GHashTable * hash){
  ...
  gchar *e=
# 205 "/mydumper/src/common.c" 3 4
          ((void *)0)
# 205 "/mydumper/src/common.c"
              ;
...
void refresh_set_session_from_hash(GString *ss, GHashTable * set_session_hash){
  ...
  gchar *e=
# 216 "/mydumper/src/common.c" 3 4
          ((void *)0)
# 216 "/mydumper/src/common.c"
              ;
  gchar *c=
# 217 "/mydumper/src/common.c" 3 4
          ((void *)0)
# 217 "/mydumper/src/common.c"
              ;
  while ( g_hash_table_iter_next ( &iter, (gpointer *) &lkey, (gpointer *) &e ) ) {
    c=g_strstr_len(e,strlen(e),"/*!");
    if (c!=
# 220 "/mydumper/src/common.c" 3 4
          ((void *)0)
# 220 "/mydumper/src/common.c"
              ){
      c[0]='\0';
      c++;
...
void free_hash(GHashTable * set_session_hash){
  ...
  g_hash_table_iter_init ( &iter, set_session_hash );
  gchar *e=
# 233 "/mydumper/src/common.c" 3 4
          ((void *)0)
# 233 "/mydumper/src/common.c"
              ;
  while ( g_hash_table_iter_next ( &iter, (gpointer *) &lkey, (gpointer *) &e ) ) {
    g_free(e);
    g_free(lkey);
...
{
  if (ss != 
# 242 "/mydumper/src/common.c" 3 4
           ((void *)0) 
# 242 "/mydumper/src/common.c"
                ){
    gchar** line=g_strsplit(ss->str, ";\n", -1);
    int i=0;
    for (i=0; i < (int)g_strv_length(line);i++){
       if (strlen(line[i]) > 3 && mysql_query(conn, line[i])){
         g_log (((gchar*) 0), G_LOG_LEVEL_WARNING, "Set session failed: %s",line[i]);
       }
    }
    g_strfreev(line);
  }
...
void create_backup_dir(char *new_directory) {
  if (mkdir(new_directory, 0750) == -1) {
    if (
# 314 "/mydumper/src/common.c" 3 4
       (*__errno_location ()) 
# 314 "/mydumper/src/common.c"
             != 
# 314 "/mydumper/src/common.c" 3 4
                17
# 314 "/mydumper/src/common.c"
                      ) {
      g_log (((gchar*) 0), G_LOG_LEVEL_CRITICAL, "Unable to create `%s': %s", new_directory, g_strerror(
# 315 "/mydumper/src/common.c" 3 4
     (*__errno_location ())
# 315 "/mydumper/src/common.c"
     ));
      exit(
# 316 "/mydumper/src/common.c" 3 4
          1
# 316 "/mydumper/src/common.c"
                      );
    }
  }
}
...
gboolean m_remove(gchar * directory, const gchar * filename){
  if (stream && no_delete == (0)){
    gchar *path = g_build_filename(directory == 
# 333 "/mydumper/src/common.c" 3 4
                                               ((void *)0)
# 333 "/mydumper/src/common.c"
                                                   ?"":directory, filename, 
# 333 "/mydumper/src/common.c" 3 4
                                                                            ((void *)0)
# 333 "/mydumper/src/common.c"
                                                                                );
    g_log (((gchar*) 0), G_LOG_LEVEL_MESSAGE, "Removing file: %s", path);
    remove(path);
    g_free(path);
  }
  return (!(0));
...
gboolean is_table_in_list(gchar *table_name, gchar **tl){
  guint i = 0;
  for (i = 0; tl[i] != 
# 343 "/mydumper/src/common.c" 3 4
                      ((void *)0)
# 343 "/mydumper/src/common.c"
                          ; i++)
...
void initialize_common_options(GOptionContext *context, const gchar *group){
  ...
  if (defaults_file != 
# 352 "/mydumper/src/common.c" 3 4
                      ((void *)0)
# 352 "/mydumper/src/common.c"
                          ){
    if (!g_file_test(defaults_file,G_FILE_TEST_EXISTS)){
      g_log (((gchar*) 0), G_LOG_LEVEL_CRITICAL, "Default file not found");
      exit(
# 355 "/mydumper/src/common.c" 3 4
          1
# 355 "/mydumper/src/common.c"
                      );
    }
    if (!g_path_is_absolute(defaults_file)){
      gchar *new_defaults_file=g_build_filename(g_get_current_dir(),defaults_file,
# 358 "/mydumper/src/common.c" 3 4
                                                                                 ((void *)0)
# 358 "/mydumper/src/common.c"
                                                                                     );
...
gchar **get_table_list(gchar *tables_list){
  ...
  for(i=0; i < g_strv_length(tl); i++){
    if (g_strstr_len(tl[i],strlen(tl[i]),".") == 
# 371 "/mydumper/src/common.c" 3 4
                                                ((void *)0) 
# 371 "/mydumper/src/common.c"
                                                     )
      do { g_log (((gchar*) 0), G_LOG_LEVEL_ERROR, "Table name %s is not in DATABASE.TABLE format", tl[i]); for (;;) ; } while (0);
  }
  return tl;
}
...
void print_version(const gchar *program){
    ...
    g_string_append_printf(str, "%s, built against MySQL %s", "0.13.1-1", "8.0.31");
