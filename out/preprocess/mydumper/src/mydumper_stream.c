# 0 "/mydumper/src/mydumper_stream.c"
# 1 "/build//"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "/mydumper/src/mydumper_stream.c"
# 17 "/mydumper/src/mydumper_stream.c"
...
# 18 "/mydumper/src/mydumper_stream.c" 2
...
MYSQL * mysql_real_connect_dns_srv(MYSQL *mysql,
                                          ...
                                          const char *db,
                                          unsigned long client_flag);
# 19 "/mydumper/src/mydumper_stream.c" 2
...
# 20 "/mydumper/src/mydumper_stream.c" 2
...
extern int gzvprintf (gzFile file, const char *format, va_list va)
                                                              ...
                                                              ;
# 24 "/mydumper/src/mydumper_stream.c" 2
...
# 29 "/mydumper/src/mydumper_stream.c" 2
...
# 35 "/mydumper/src/mydumper_stream.c" 3 4
                        ((void *)0)
# 35 "/mydumper/src/mydumper_stream.c"
                            ;

void *process_stream(void *data){
  (void)data;
  char * filename=
# 39 "/mydumper/src/mydumper_stream.c" 3 4
                 ((void *)0)
# 39 "/mydumper/src/mydumper_stream.c"
                     ;
  FILE * f=
# 40 "/mydumper/src/mydumper_stream.c" 3 4
          ((void *)0)
# 40 "/mydumper/src/mydumper_stream.c"
              ;
  char buf[1000000];
  int buflen;
  ...
  for(;;){
    ...
    len=write(fileno(
# 56 "/mydumper/src/mydumper_stream.c" 3 4
                    stdout
# 56 "/mydumper/src/mydumper_stream.c"
                          ), "\n-- ", 4);
    len=write(fileno(
# 57 "/mydumper/src/mydumper_stream.c" 3 4
                    stdout
# 57 "/mydumper/src/mydumper_stream.c"
                          ), used_filemame, strlen(used_filemame));
    len=write(fileno(
# 58 "/mydumper/src/mydumper_stream.c" 3 4
                    stdout
# 58 "/mydumper/src/mydumper_stream.c"
                          ), "\n", 1);
    total_size+=5;
    ...
    if (no_stream == (0)){
      ...
      f=fopen(filename,"r");
      if (!f){
        do { g_log (((gchar*) 0), G_LOG_LEVEL_ERROR, "File failed to open: %s",filename); for (;;) ; } while (0);
      }else{
        if (!f){
          g_log (((gchar*) 0), G_LOG_LEVEL_CRITICAL, "File failed to open: %s. Reetrying",filename);
          f=fopen(filename,"r");
          if (!f){
            do { g_log (((gchar*) 0), G_LOG_LEVEL_ERROR, "File failed to open: %s. Cancelling",filename); for (;;) ; } while (0);
            exit(
# 74 "/mydumper/src/mydumper_stream.c" 3 4
                1
# 74 "/mydumper/src/mydumper_stream.c"
                            );
          }
        }
        guint total_len=0;
        GDateTime *start_time=g_date_time_new_now_local();
        buflen = read(fileno(f), buf, 1000000);
        while(buflen > 0){
          len=write(fileno(
# 81 "/mydumper/src/mydumper_stream.c" 3 4
                          stdout
# 81 "/mydumper/src/mydumper_stream.c"
                                ), buf, buflen);
          total_len=total_len + buflen;
          if (len != buflen)
            do { g_log (((gchar*) 0), G_LOG_LEVEL_ERROR, "Stream failed during transmition of file: %s",filename); for (;;) ; } while (0);
          buflen = read(fileno(f), buf, 1000000);
        }
        datetime = g_date_time_new_now_local();
        diff=g_date_time_difference(datetime,start_time)/((1000000L));
        g_date_time_unref(start_time);
        total_diff=g_date_time_difference(datetime,total_start_time)/((1000000L));
        g_date_time_unref(datetime);
        if (diff > 0){
          g_log (((gchar*) 0), G_LOG_LEVEL_MESSAGE, "File %s transferred in %ld seconds at %ld MB/s | Global: %ld MB/s",filename,diff,total_len/1024/1024/diff,total_diff!=0?total_size/1024/1024/total_diff:total_size/1024/1024);
        }else{
          g_log (((gchar*) 0), G_LOG_LEVEL_MESSAGE, "File %s transferred | Global: %ld MB/s",filename,total_diff!=0?total_size/1024/1024/total_diff:total_size/1024/1024);
        }
  ...
  g_date_time_unref(datetime);
  g_log (((gchar*) 0), G_LOG_LEVEL_MESSAGE, "All data transferred was %ld at a rate of %ld MB/s",total_size,total_diff!=0?total_size/1024/1024/total_diff:total_size/1024/1024);
  return 
# 112 "/mydumper/src/mydumper_stream.c" 3 4
        ((void *)0)
# 112 "/mydumper/src/mydumper_stream.c"
            ;
}

void initialize_stream(){
  stream_queue = g_async_queue_new();
  stream_thread = g_thread_create((GThreadFunc)process_stream, stream_queue, (!(0)), 
# 117 "/mydumper/src/mydumper_stream.c" 3 4
                                                                                  ((void *)0)
# 117 "/mydumper/src/mydumper_stream.c"
                                                                                      );
}
