# 0 "/mydumper/src/myloader_common.c"
# 1 "/build//"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "/mydumper/src/myloader_common.c"
# 17 "/mydumper/src/myloader_common.c"
...
MYSQL * mysql_real_connect_dns_srv(MYSQL *mysql,
                                          ...
                                          const char *db,
                                          unsigned long client_flag);
# 18 "/mydumper/src/myloader_common.c" 2
...
# 19 "/mydumper/src/myloader_common.c" 2
...
# 20 "/mydumper/src/myloader_common.c" 2
...
extern int gzvprintf (gzFile file, const char *format, va_list va)
                                                              ...
                                                              ;
# 27 "/mydumper/src/myloader_common.c" 2
...
# 29 "/mydumper/src/myloader_common.c" 2
...
# 30 "/mydumper/src/myloader_common.c" 2
...
# 31 "/mydumper/src/myloader_common.c" 2
...
# 32 "/mydumper/src/myloader_common.c" 2
...
# 33 "/mydumper/src/myloader_common.c" 2
...
# 34 "/mydumper/src/myloader_common.c" 2
...
# 36 "/mydumper/src/myloader_common.c" 2
...
# 37 "/mydumper/src/myloader_common.c" 2
...
# 38 "/mydumper/src/myloader_common.c" 2
...
static GMutex *db_hash_mutex = 
# 50 "/mydumper/src/myloader_common.c" 3 4
                              ((void *)0)
# 50 "/mydumper/src/myloader_common.c"
                                  ;
GHashTable *db_hash=
# 51 "/mydumper/src/myloader_common.c" 3 4
                   ((void *)0)
# 51 "/mydumper/src/myloader_common.c"
                       ;
GHashTable *tbl_hash=
# 52 "/mydumper/src/myloader_common.c" 3 4
                    ((void *)0)
# 52 "/mydumper/src/myloader_common.c"
                        ;
...
gboolean m_filename_has_suffix(gchar const *str, gchar const *suffix){
  if (g_str_has_suffix(str, compress_extension)){
    return g_strstr_len(&(str[strlen(str)-strlen(compress_extension)-strlen(suffix)]), strlen(str)-strlen(compress_extension),suffix) != 
# 61 "/mydumper/src/myloader_common.c" 3 4
                                                                                                                                        ((void *)0)
# 61 "/mydumper/src/myloader_common.c"
                                                                                                                                            ;
  }
  return g_str_has_suffix(str,suffix);
}
struct database * new_database(gchar *database){
  struct database * d = (struct database *) (__extension__ ({ gsize __n = (gsize) (1); gsize __s = sizeof (struct database); gpointer __p; if (__s == 1) __p = g_malloc (__n); else if (__builtin_constant_p (__n) && (__s == 0 || __n <= 
# 66 "/mydumper/src/myloader_common.c" 3 4
                       (0x7fffffffffffffffL * 2UL + 1UL) 
# 66 "/mydumper/src/myloader_common.c"
...
struct database * get_db_hash(gchar *k, gchar *v){
  ...
  if (d==
# 75 "/mydumper/src/myloader_common.c" 3 4
        ((void *)0)
# 75 "/mydumper/src/myloader_common.c"
            ){
...
# 92 "/mydumper/src/myloader_common.c"
gboolean eval_table( char *db_name, char * table_name){
  if ( tables ){
...
# 118 "/mydumper/src/myloader_common.c"
guint execute_use(struct thread_data *td, const gchar * msg){
  gchar *query = g_strdup_printf("USE `%s`", td->current_database);
  if (mysql_query(td->thrconn, query)) {
    g_log (((gchar*) 0), G_LOG_LEVEL_CRITICAL, "Thread %d: Error switching to database `%s` %s", td->thread_id, td->current_database, msg);
    g_free(query);
    return 1;
  }
  g_free(query);
...
void execute_use_if_needs_to(struct thread_data *td, gchar *database, const gchar * msg){
  if ( database != 
# 130 "/mydumper/src/myloader_common.c" 3 4
                  ((void *)0) 
# 130 "/mydumper/src/myloader_common.c"
                       && db == 
# 130 "/mydumper/src/myloader_common.c" 3 4
                                ((void *)0) 
# 130 "/mydumper/src/myloader_common.c"
                                     ){
    if (td->current_database==
# 131 "/mydumper/src/myloader_common.c" 3 4
                             ((void *)0) 
# 131 "/mydumper/src/myloader_common.c"
                                  || g_strcmp0(database, td->current_database) != 0){
      td->current_database=database;
      if (execute_use(td, msg)){
        exit(
# 134 "/mydumper/src/myloader_common.c" 3 4
            1
# 134 "/mydumper/src/myloader_common.c"
                        );
      }
    }
  }
...
enum file_type get_file_type (const char * filename){
  if (m_filename_has_suffix(filename, "-schema.sql")) {
    ...
    if (!resume){
      g_log (((gchar*) 0), G_LOG_LEVEL_CRITICAL, "resume file found, but no --resume option passed. Use --resume or remove it and restart process if you consider that it will be safe.");
      exit(
# 152 "/mydumper/src/myloader_common.c" 3 4
          1
# 152 "/mydumper/src/myloader_common.c"
                      );
    }
    return RESUME;
  } else if ( strcmp(filename, "resume.partial") == 0 ){
    g_log (((gchar*) 0), G_LOG_LEVEL_CRITICAL, "resume.partial file found. Remove it and restart process if you consider that it will be safe.");
    exit(
# 157 "/mydumper/src/myloader_common.c" 3 4
        1
# 157 "/mydumper/src/myloader_common.c"
                    );
...
# 182 "/mydumper/src/myloader_common.c" 3 4
                                     ((void *)0)
# 182 "/mydumper/src/myloader_common.c"
                                         ) {
        if (feof(file)) {
          *eof = (!(0));
          buffer[0] = '\0';
    ...
    if (strlen(buffer) != 256)
      (*line)++;
  } while ((buffer[strlen(buffer)] != '\0') && *eof == (0));
...
void get_database_table_from_file(const gchar *filename,const char *sufix,gchar **database,gchar **table){
  ...
  if (count > 2){
    g_log (((gchar*) 0), G_LOG_LEVEL_WARNING, "We need to get the db and table name from the create table statement");
    return;
  }
...
void finish_alter_table(GString * alter_table_statement){
  ...
  if ((str - alter_table_statement->str) > (long int)(alter_table_statement->len - 5)){
    *str=';';
    g_string_append_c_inline (alter_table_statement, '\n');
  }else
    g_string_append(alter_table_statement,";\n");
}
...
int process_create_table_statement (gchar * statement, GString *create_table_statement, GString *alter_table_statement, GString *alter_table_constraint_statement, struct db_table *dbt){
  ...
  gchar *autoinc_column=
# 242 "/mydumper/src/myloader_common.c" 3 4
                       ((void *)0)
# 242 "/mydumper/src/myloader_common.c"
                           ;
  ...
  for (i=0; i < (int)g_strv_length(split_file);i++){
    if ( g_strstr_len(split_file[i],5,"  KEY")
      ...
      if ((autoinc_column != 
# 255 "/mydumper/src/myloader_common.c" 3 4
                            ((void *)0)
# 255 "/mydumper/src/myloader_common.c"
                                ) && (g_strrstr(split_file[i],autoinc_column))){
        g_string_append(create_table_statement, split_file[i]);
        g_string_append_c_inline (create_table_statement, '\n');
      }else{
        flag|=8;
    ...
    if (g_strrstr(split_file[i],"ENGINE=InnoDB")) flag|=2;
  }
  return flag;
}
...
void refresh_table_list_without_table_hash_lock(struct configuration *conf){
  GList * table_list=
# 307 "/mydumper/src/myloader_common.c" 3 4
                    ((void *)0)
# 307 "/mydumper/src/myloader_common.c"
                        ;
  GHashTableIter iter;
  gchar * lkey;
  ...
  struct db_table *dbt=
# 312 "/mydumper/src/myloader_common.c" 3 4
                      ((void *)0)
# 312 "/mydumper/src/myloader_common.c"
                          ;
...
void checksum_filename(const gchar *filename, MYSQL *conn, const gchar *suffix, const gchar *message, gchar* fun()) {
  gchar *database = 
# 329 "/mydumper/src/myloader_common.c" 3 4
                   ((void *)0)
# 329 "/mydumper/src/myloader_common.c"
                       , *table = 
# 329 "/mydumper/src/myloader_common.c" 3 4
                                  ((void *)0)
# 329 "/mydumper/src/myloader_common.c"
  ...
  gchar *real_table=
# 332 "/mydumper/src/myloader_common.c" 3 4
                   ((void *)0)
# 332 "/mydumper/src/myloader_common.c"
                       ;
  if (table != 
# 333 "/mydumper/src/myloader_common.c" 3 4
              ((void *)0) 
# 333 "/mydumper/src/myloader_common.c"
                   ){
  ...
  if (row == 
# 342 "/mydumper/src/myloader_common.c" 3 4
            ((void *)0)
# 342 "/mydumper/src/myloader_common.c"
                )
    row = g_strdup("0");

  gboolean is_compressed = (0);
  gchar *path = g_build_filename(directory, filename, 
# 346 "/mydumper/src/myloader_common.c" 3 4
                                                     ((void *)0)
# 346 "/mydumper/src/myloader_common.c"
                                                         );
  ...
  if (!infile) {
    g_log (((gchar*) 0), G_LOG_LEVEL_CRITICAL, "cannot open checksum file %s (%d)", filename, 
# 357 "/mydumper/src/myloader_common.c" 3 4
   (*__errno_location ())
# 357 "/mydumper/src/myloader_common.c"
   );
    errors++;
    return;
  }
  ...
  if (cs != 
# 363 "/mydumper/src/myloader_common.c" 3 4
           ((void *)0)
# 363 "/mydumper/src/myloader_common.c"
               ) {
    if (checksum[strlen(checksum)-1]=='\n')
      checksum[strlen(checksum)-1]='\0';
    if(g_strcasecmp(checksum, row) != 0) {
      if (real_table != 
# 367 "/mydumper/src/myloader_common.c" 3 4
                       ((void *)0)
# 367 "/mydumper/src/myloader_common.c"
                           )
        g_log (((gchar*) 0), G_LOG_LEVEL_WARNING, "%s mismatch found for `%s`.`%s`. Got '%s', expecting '%s' in file: %s", message, db ? db : real_database->name, real_table, row, checksum, filename);
      else
        g_log (((gchar*) 0), G_LOG_LEVEL_WARNING, "%s mismatch found for `%s`. Got '%s', expecting '%s' in file: %s", message, db ? db : real_database->name, row, checksum, filename);
      errors++;
    } else {
      if (real_table != 
# 373 "/mydumper/src/myloader_common.c" 3 4
                       ((void *)0)
# 373 "/mydumper/src/myloader_common.c"
                           )
        g_log (((gchar*) 0), G_LOG_LEVEL_MESSAGE, "%s confirmed for `%s`.`%s`", message, db ? db : real_database->name, real_table);
      else
        g_log (((gchar*) 0), G_LOG_LEVEL_MESSAGE, "%s confirmed for `%s`", message, db ? db : real_database->name);
    }
    g_free(row);
  } else {
    g_log (((gchar*) 0), G_LOG_LEVEL_CRITICAL, "error reading file %s (%d)", filename, 
# 380 "/mydumper/src/myloader_common.c" 3 4
   (*__errno_location ())
# 380 "/mydumper/src/myloader_common.c"
   );
    errors++;
    return;
  }
...
void checksum_databases(struct thread_data *td) {
  g_log (((gchar*) 0), G_LOG_LEVEL_MESSAGE, "Starting table checksum verification");

  gchar *filename = 
# 395 "/mydumper/src/myloader_common.c" 3 4
                   ((void *)0)
# 395 "/mydumper/src/myloader_common.c"
                       ;
