...
# 32 "/ejdb/src/ejdb2_internal.h" 2


# 1 "/ejdb/src/jbr/jbr.h" 1
       
# 33 "/ejdb/src/jbr/jbr.h"
...
# 35 "/ejdb/src/ejdb2_internal.h" 2
...
# 2 "/ejdb/src/ejdb2.c" 2
# 1 "/ejdb/build/include/ejdb2/iowow/murmur3.h" 1
       
# 13 "/ejdb/build/include/ejdb2/iowow/murmur3.h"


__attribute__((visibility("default"))) void murmur3_set_seed(const uint32_t seed);

__attribute__((visibility("default"))) uint32_t murmur3(const char *key, size_t len);


# 3 "/ejdb/src/ejdb2.c" 2

static iwrc _jb_put_new_lw(JBCOLL jbc, JBL jbl, int64_t *id);

static const IWKV_val EMPTY_VAL = { 0 };

static inline __attribute__((always_inline)) iwrc _jb_meta_nrecs_removedb(EJDB db, uint32_t dbid) {
  dbid = (dbid);
  IWKV_val key = {
    .size = sizeof(dbid),
    .data = &dbid
...
static inline __attribute__((always_inline)) iwrc _jb_meta_nrecs_update(EJDB db, uint32_t dbid, int64_t delta) {
  delta = (delta);
  dbid = (dbid);
  IWKV_val val = {
    .size = sizeof(delta),
  ...
  return iwkv_put(db->nrecdb, &key, &val, ((iwkv_opflags) 0x10U));
}
...
# 72 "/ejdb/src/ejdb2.c" 3 4
                                         (*__errno_location ())
# 72 "/ejdb/src/ejdb2.c"
                                              );
  }

  rc = jbl_from_buf_keep_onstack(&imeta, mval->data, mval->size); if (__builtin_expect((!!(rc)), 0)) goto finish;
  bn = &imeta.bn;
  ...
  rc = jbl_ptr_alloc(ptr, &idx->ptr); if (__builtin_expect((!!(rc)), 0)) goto finish;
  rc = iwkv_db(jbc->db->iwkv, idx->dbid, idx->idbf, &idx->idb); if (__builtin_expect((!!(rc)), 0)) goto finish;
...
static iwrc _jb_coll_load_indexes_lr(JBCOLL jbc) {
  ...
  IWKV_cursor cur;
  IWKV_val kval;
  char buf[sizeof("i.") + 32];

  int sz = snprintf(buf, sizeof(buf), "i." "%u.", jbc->dbid);
  if (sz >= sizeof(buf)) {
  ...
  do {
    IWKV_val key, val;
    rc = iwkv_cursor_key(cur, &key); if (__builtin_expect((!!(rc)), 0)) goto finish;
    if ((key.size > sz) && !strncmp(buf, key.data, sz)) {
      iwkv_val_dispose(&key);
      rc = iwkv_cursor_val(cur, &val); if (__builtin_expect((!!(rc)), 0)) goto finish;
      rc = _jb_coll_load_index_lr(jbc, &val);
      iwkv_val_dispose(&val);
      if (__builtin_expect((!!(rc)), 0)) break;
    } else {
      iwkv_val_dispose(&key);
    }
...
static iwrc _jb_coll_load_meta_lr(JBCOLL jbc) {
  ...
  rc = iwkv_db(jbc->db->iwkv, jbc->dbid, ((iwdb_flags_t) 0x20U), &jbc->cdb);
  if (__builtin_expect((!!(rc)), 0)) return (rc);
  ...
  if (rc) {
    ...
    size_t sz;
    rc = iwkv_cursor_copy_key(cur, &jbc->id_seq, sizeof(jbc->id_seq), &sz, 0); if (__builtin_expect((!!(rc)), 0)) goto finish;
  }
...
# 194 "/ejdb/src/ejdb2.c" 3 4
                                                              0
# 194 "/ejdb/src/ejdb2.c"
                                                                   );
    if (__builtin_expect((!!(rc)), 0)) return (rc);
  }
  if (!jbc->meta) {
    iwlog2(IWLOG_ERROR, 0, "/ejdb/src/ejdb2.c", 198, ("Collection %s seems to be initialized"), jbc->name);
...
static iwrc _jb_idx_add_meta_lr(JBIDX idx, binn *list) {
  ...
  if (!xstr) {
    return iwrc_set_errno(IW_ERROR_ALLOC, 
# 214 "/ejdb/src/ejdb2.c" 3 4
                                         (*__errno_location ())
# 214 "/ejdb/src/ejdb2.c"
                                              );
  }
  binn *meta = binn_object();
  if (!meta) {
    rc = iwrc_set_errno(IW_ERROR_ALLOC, 
# 218 "/ejdb/src/ejdb2.c" 3 4
                                       (*__errno_location ())
# 218 "/ejdb/src/ejdb2.c"
                                            );
    iwxstr_destroy(xstr);
    return rc;
  }
  rc = jbl_ptr_serialize(idx->ptr, xstr); if (__builtin_expect((!!(rc)), 0)) goto finish;
...
static iwrc _jb_coll_add_meta_lr(JBCOLL jbc, binn *list) {
  ...
  if (!meta) {
    rc = iwrc_set_errno(IW_ERROR_ALLOC, 
# 247 "/ejdb/src/ejdb2.c" 3 4
                                       (*__errno_location ())
# 247 "/ejdb/src/ejdb2.c"
                                            );
    return rc;
  }
  ...
  if (!ilist) {
    rc = iwrc_set_errno(IW_ERROR_ALLOC, 
# 258 "/ejdb/src/ejdb2.c" 3 4
                                       (*__errno_location ())
# 258 "/ejdb/src/ejdb2.c"
                                            );
    goto finish;
  }
  for (JBIDX idx = jbc->idx; idx; idx = idx->next) {
    rc = _jb_idx_add_meta_lr(idx, ilist); if (__builtin_expect((!!(rc)), 0)) goto finish;
  }
...
static iwrc _jb_db_meta_load(EJDB db) {
  iwrc rc = 0;
  if (!db->metadb) {
    rc = iwkv_db(db->iwkv, 1, 0, &db->metadb);
    if (__builtin_expect((!!(rc)), 0)) return (rc);
  }
  if (!db->nrecdb) {
    rc = iwkv_db(db->iwkv, 2, ((iwdb_flags_t) 0x20U), &db->nrecdb);
    if (__builtin_expect((!!(rc)), 0)) return (rc);
  }
  ...
  while (!(rc = iwkv_cursor_to(cur, IWKV_CURSOR_NEXT))) {
    IWKV_val key, val;
    rc = iwkv_cursor_get(cur, &key, &val); if (__builtin_expect((!!(rc)), 0)) goto finish;
    if (!strncmp(key.data, "c.", sizeof("c.") - 1)) {
      JBCOLL jbc = calloc(1, sizeof(*jbc));
      if (!jbc) {
        rc = iwrc_set_errno(IW_ERROR_ALLOC, 
# 301 "/ejdb/src/ejdb2.c" 3 4
                                           (*__errno_location ())
# 301 "/ejdb/src/ejdb2.c"
                                                );
        iwkv_val_dispose(&val);
        goto finish;
      }
...
static iwrc _jb_db_release(EJDB *dbp) {
  ...
  *dbp = 0;
  ...
  if (db->iwkv) {
    { iwrc __iwrc = (iwkv_close(&db->iwkv)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 338, ""); } };
  }
...
static iwrc _jb_coll_acquire_keeplock2(EJDB db, const char *coll, jb_coll_acquire_t acm, JBCOLL *jbcp) {
  if (!coll || *coll == '\0' || strlen(coll) > 255) {
...
# 361 "/ejdb/src/ejdb2.c" 3 4
 _Bool 
# 361 "/ejdb/src/ejdb2.c"
      wl = acm & ((jb_coll_acquire_t) 0x01U);
  if (!(db) || !((db)->open)) { iwlog3(IWLOG_ERROR, 0, "/ejdb/src/ejdb2.c", 362, ("Database is not open")); return IW_ERROR_INVALID_STATE; }; rci = pthread_rwlock_rdlock(&(db)->rwl); if (rci) return iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci);
  ...
  if (jbc) {
    ...
    *jbcp = jbc;
    ...
    if ((db->oflags & ((iwkv_openflags) 0x02U)) || (acm & ((jb_coll_acquire_t) 0x02U))) {
      return IW_ERROR_NOT_EXISTS;
    }
    if (!(db) || !((db)->open)) { iwlog3(IWLOG_ERROR, 0, "/ejdb/src/ejdb2.c", 373, ("Database is not open")); return IW_ERROR_INVALID_STATE; }; rci = pthread_rwlock_wrlock(&(db)->rwl); if (rci) return iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci);
    jbc = iwhmap_get(db->mcolls, coll);
    if (jbc) {
      ...
      *jbcp = jbc;
      ...
      IWDB cdb = 0;
      uint32_t dbid = 0;
      char keybuf[32 + sizeof("c.")];
      IWKV_val key, val;

      rc = iwkv_new_db(db->iwkv, ((iwdb_flags_t) 0x20U), &dbid, &cdb); if (__builtin_expect((!!(rc)), 0)) goto create_finish;
      jbc = calloc(1, sizeof(*jbc));
      if (!jbc) {
        rc = iwrc_set_errno(IW_ERROR_ALLOC, 
# 388 "/ejdb/src/ejdb2.c" 3 4
                                           (*__errno_location ())
# 388 "/ejdb/src/ejdb2.c"
                                                );
        goto create_finish;
      }
      rc = jbl_create_empty_object(&meta); if (__builtin_expect((!!(rc)), 0)) goto create_finish;
      ...
      rc = jbl_as_buf(meta, &val.data, &val.size); if (__builtin_expect((!!(rc)), 0)) goto create_finish;

      key.size = snprintf(keybuf, sizeof(keybuf), "c." "%u", dbid);
      if (key.size >= sizeof(keybuf)) {
        rc = IW_ERROR_OVERFLOW;
        goto create_finish;
      }
      key.data = keybuf;
      rc = iwkv_put(db->metadb, &key, &val, ((iwkv_opflags) 0x04U)); if (__builtin_expect((!!(rc)), 0)) goto create_finish;

      jbc->db = db;
      jbc->meta = meta;
      rc = _jb_coll_init(jbc, 0);
      if (rc) {
        iwkv_del(db->metadb, &key, ((iwkv_opflags) 0x04U));
        goto create_finish;
      }
...
static inline __attribute__((always_inline)) iwrc _jb_coll_acquire_keeplock(EJDB db, const char *coll, 
# 448 "/ejdb/src/ejdb2.c" 3 4
                                                                   _Bool 
# 448 "/ejdb/src/ejdb2.c"
                                                                        wl, JBCOLL *jbcp) {
  return _jb_coll_acquire_keeplock2(db, coll, wl ? ((jb_coll_acquire_t) 0x01U) : 0, jbcp);
}
...
# 458 "/ejdb/src/ejdb2.c" 3 4
 _Bool 
# 458 "/ejdb/src/ejdb2.c"
      jbv_found, jbvprev_found;
...
# 465 "/ejdb/src/ejdb2.c" 3 4
 _Bool 
# 465 "/ejdb/src/ejdb2.c"
      compound = idx->idbf & ((iwdb_flags_t) 0x40U);

  jbvprev_found = jblprev ? _jbl_at(jblprev, idx->ptr, &jbvprev) : 
# 467 "/ejdb/src/ejdb2.c" 3 4
                                                                  0
# 467 "/ejdb/src/ejdb2.c"
                                                                       ;
  jbv_found = jbl ? _jbl_at(jbl, idx->ptr, &jbv) : 
# 468 "/ejdb/src/ejdb2.c" 3 4
                                                  0
# 468 "/ejdb/src/ejdb2.c"
                                                       ;
  ...
  if ( ((jbvprev_type == JBV_OBJECT) || (jbvprev_type <= JBV_NULL))
    ...
    jbvprev_found = 
# 476 "/ejdb/src/ejdb2.c" 3 4
                   0
# 476 "/ejdb/src/ejdb2.c"
                        ;
  }
  if ( ((jbv_type == JBV_OBJECT) || (jbv_type <= JBV_NULL))
     || ((jbv_type == JBV_ARRAY) && !compound)) {
    jbv_found = 
# 480 "/ejdb/src/ejdb2.c" 3 4
               0
# 480 "/ejdb/src/ejdb2.c"
                    ;
  }

  if ( compound
     && (jbv_type == jbvprev_type)
     && (jbvprev_type == JBV_ARRAY)) {
    pool = iwpool_create(1024);
    if (!pool) {
      rc = iwrc_set_errno(IW_ERROR_ALLOC, 
# 488 "/ejdb/src/ejdb2.c" 3 4
                                         (*__errno_location ())
# 488 "/ejdb/src/ejdb2.c"
                                              );
      goto finish;
    }
    JBL_NODE jbvprev_node, jbv_node;
    rc = jbl_to_node(&jbv, &jbv_node, 
# 492 "/ejdb/src/ejdb2.c" 3 4
   0
# 492 "/ejdb/src/ejdb2.c"
   , pool); if (__builtin_expect((!!(rc)), 0)) goto finish;
    jbv.node = jbv_node;

    rc = jbl_to_node(&jbvprev, &jbvprev_node, 
# 495 "/ejdb/src/ejdb2.c" 3 4
   0
# 495 "/ejdb/src/ejdb2.c"
   , pool); if (__builtin_expect((!!(rc)), 0)) goto finish;
  ...
  if (jbvprev_found) {
    if (jbvprev_type == JBV_ARRAY) {
      ...
      if (!pool) {
        ...
        if (!pool) {
          rc = iwrc_set_errno(IW_ERROR_ALLOC, 
# 511 "/ejdb/src/ejdb2.c" 3 4
         (*__errno_location ())
# 511 "/ejdb/src/ejdb2.c"
         ); if (__builtin_expect((!!(rc)), 0)) goto finish;
        }
      }
      rc = jbl_to_node(&jbvprev, &n, 
# 514 "/ejdb/src/ejdb2.c" 3 4
     0
# 514 "/ejdb/src/ejdb2.c"
     , pool); if (__builtin_expect((!!(rc)), 0)) goto finish;
  ...
  if (jbv_found) {
    if (jbv_type == JBV_ARRAY) {
      ...
      if (!pool) {
        ...
        if (!pool) {
          rc = iwrc_set_errno(IW_ERROR_ALLOC, 
# 549 "/ejdb/src/ejdb2.c" 3 4
         (*__errno_location ())
# 549 "/ejdb/src/ejdb2.c"
         ); if (__builtin_expect((!!(rc)), 0)) goto finish;
        }
      }
      rc = jbl_to_node(&jbv, &n, 
# 552 "/ejdb/src/ejdb2.c" 3 4
     0
# 552 "/ejdb/src/ejdb2.c"
     , pool); if (__builtin_expect((!!(rc)), 0)) goto finish;
      for (n = n->child; n; n = n->next) {
        jbi_node_fill_ikey(idx, n, &key, numbuf);
        if (key.size) {
          key.compound = id;
          rc = iwkv_put(idx->idb, &key, &EMPTY_VAL, ((iwkv_opflags) 0x01U));
          if (!rc) {
            ++delta;
      ...
      if (key.size) {
        if (compound) {
          ...
          do { int64_t _num_ = (id); if (_num_ == 0) { ((signed char*) (vnbuf))[0] = 0; (step) = 1; } else { (step) = 0; while (_num_ > 0) { int _rem_ = _num_ & 0x7f; _num_ >>= 7; if (_num_ > 0) { ((signed char*) (vnbuf))[(step)] = ~(_rem_); } else { ((signed char*) (vnbuf))[(step)] = _rem_; } (step)++; } } } while (0);
          IWKV_val idval = {
            .data = vnbuf,
            .size = step
          };
          rc = iwkv_put(idx->idb, &key, &idval, ((iwkv_opflags) 0x01U));
          if (!rc) {
            ++delta;
          } else if (rc == IWKV_ERROR_KEY_EXISTS) {
...
static inline __attribute__((always_inline)) iwrc _jb_idx_record_remove(JBIDX idx, int64_t id, JBL jbl) {
...
static iwrc _jb_idx_fill(JBIDX idx) {
  ...
  { iwrc __iwrc = (iwkv_cursor_close(&cur)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 634, ""); } };
  return rc;
}
...
finish:
  ...
  if (rc && !oldval->size) {
    ...
    for (JBIDX idx = jbc->idx; idx && idx != fail_idx; idx = idx->next) {
      { iwrc __iwrc = (_jb_idx_record_remove(idx, ctx->id, ctx->jbl)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 678, ""); } };
    }
    { iwrc __iwrc = (iwkv_del(jbc->cdb, &key, 0)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 680, ""); } };
  }
  return rc;
}
...
static iwrc _jb_exec_scan_init(JBEXEC *ctx) {
  ...
  if (!ctx->jblbuf) {
    ctx->jblbufsz = 0;
    return iwrc_set_errno(IW_ERROR_ALLOC, 
# 699 "/ejdb/src/ejdb2.c" 3 4
                                         (*__errno_location ())
# 699 "/ejdb/src/ejdb2.c"
                                              );
  }
  struct JQP_AUX *aux = ctx->ux->q->aux;
  if (aux->expr->flags & 0x01U) {
  ...
  if (__builtin_expect((!!(rc)), 0)) return (rc);
  if (ctx->midx.idx) {
    if (ctx->midx.idx->idbf & ((iwdb_flags_t) 0x40U)) {
...
static inline __attribute__((always_inline)) iwrc _jb_put_impl(JBCOLL jbc, JBL jbl, int64_t id) {
  IWKV_val val, key = {
    .data = &id,
    .size = sizeof(id)
  };
...
static iwrc _jb_exec_upsert_lw(JBEXEC *ctx) {
  ...
  rc = jbl_from_node(&jbl, n); if (__builtin_expect((!!(rc)), 0)) goto finish;
  rc = _jb_put_new_lw(ctx->jbc, jbl, &id); if (__builtin_expect((!!(rc)), 0)) goto finish;

  if (!(q->aux->qmode & (((jqp_query_mode_t) 0x01U)))) {
    ...
    do {
      ctx->istep = 1;
      rc = ux->visitor(ux, &doc, &ctx->istep); if (__builtin_expect((!!(rc)), 0)) goto finish;
    } while (ctx->istep == -1);
  }
  ++ux->cnt;
...
iwrc ejdb_exec(EJDB_EXEC *ux) {
  ...
  if (ux->limit < 1) {
    ...
    if (ux->limit < 1) {
      ux->limit = 
# 838 "/ejdb/src/ejdb2.c" 3 4
                 (9223372036854775807L)
# 838 "/ejdb/src/ejdb2.c"
                          ;
    }
  }
  if (ux->skip < 1) {
  ...
  rc = _jb_coll_acquire_keeplock2(ux->db, ux->q->coll,
                                  jql_has_apply(ux->q) ? ((jb_coll_acquire_t) 0x01U) : ((jb_coll_acquire_t) 0x02U),
                                  &ctx.jbc);
  ...
  rc = _jb_exec_scan_init(&ctx); if (__builtin_expect((!!(rc)), 0)) goto finish;
  if (ctx.sorting) {
    if (ux->log) {
...
finish:
  ...
  do { rci = pthread_rwlock_unlock(&(ctx.jbc)->rwl); if (rci) { iwrc __iwrc = (iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 874, ""); } }; rci = pthread_rwlock_unlock(&((ctx.jbc)->db)->rwl); if (rci) { iwrc __iwrc = (iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 874, ""); } }; } while (0);
  jql_reset(ux->q, 
# 875 "/ejdb/src/ejdb2.c" 3 4
                  1
# 875 "/ejdb/src/ejdb2.c"
                      , 
# 875 "/ejdb/src/ejdb2.c" 3 4
                        0
# 875 "/ejdb/src/ejdb2.c"
...
static iwrc _jb_exec_list_visitor(struct _EJDB_EXEC *ctx, EJDB_DOC doc, int64_t *step) {
  ...
  if (!ndoc) {
    return iwrc_set_errno(IW_ERROR_ALLOC, 
# 889 "/ejdb/src/ejdb2.c" 3 4
                                         (*__errno_location ())
# 889 "/ejdb/src/ejdb2.c"
                                              );
  }
  ndoc->id = doc->id;
...
iwrc ejdb_list3(EJDB db, const char *coll, const char *query, int64_t limit, IWXSTR *log, EJDB_LIST *listp) {
  ...
  *listp = 0;
  ...
  if (!pool) {
    return iwrc_set_errno(IW_ERROR_ALLOC, 
# 986 "/ejdb/src/ejdb2.c" 3 4
                                         (*__errno_location ())
# 986 "/ejdb/src/ejdb2.c"
                                              );
  }
  EJDB_LIST list = iwpool_alloc(sizeof(*list), pool);
  if (!list) {
    rc = iwrc_set_errno(IW_ERROR_ALLOC, 
# 990 "/ejdb/src/ejdb2.c" 3 4
                                       (*__errno_location ())
# 990 "/ejdb/src/ejdb2.c"
                                            );
    goto finish;
  }
  list->first = 0;
  list->db = db;
  list->pool = pool;
  rc = jql_create(&list->q, coll, query); if (__builtin_expect((!!(rc)), 0)) goto finish;
...
iwrc ejdb_list4(EJDB db, JQL q, int64_t limit, IWXSTR *log, EJDB_LIST *listp) {
  ...
  *listp = 0;
  ...
  if (!pool) {
    return iwrc_set_errno(IW_ERROR_ALLOC, 
# 1016 "/ejdb/src/ejdb2.c" 3 4
                                         (*__errno_location ())
# 1016 "/ejdb/src/ejdb2.c"
                                              );
  }
  EJDB_LIST list = iwpool_alloc(sizeof(*list), pool);
  if (!list) {
    rc = iwrc_set_errno(IW_ERROR_ALLOC, 
# 1020 "/ejdb/src/ejdb2.c" 3 4
                                       (*__errno_location ())
# 1020 "/ejdb/src/ejdb2.c"
                                            );
    goto finish;
  }
  list->q = 0;
...
iwrc ejdb_remove_index(EJDB db, const char *coll, const char *path, ejdb_idx_mode_t mode) {
  ...
  IWKV_val key;
  JBL_PTR ptr = 0;
  char keybuf[sizeof("i.") + 1 + 2 * 32];

  iwrc rc = _jb_coll_acquire_keeplock2(db, coll, ((jb_coll_acquire_t) 0x01U) | ((jb_coll_acquire_t) 0x02U), &jbc);
  if (__builtin_expect((!!(rc)), 0)) return (rc);

  rc = jbl_ptr_alloc(path, &ptr); if (__builtin_expect((!!(rc)), 0)) goto finish;

  for (JBIDX idx = jbc->idx, prev = 0; idx; idx = idx->next) {
    if (((idx->mode & ~((ejdb_idx_mode_t) 0x01U)) == (mode & ~((ejdb_idx_mode_t) 0x01U))) && !jbl_ptr_cmp(idx->ptr, ptr)) {
      key.data = keybuf;
      key.size = snprintf(keybuf, sizeof(keybuf), "i." "%u" "." "%u", jbc->dbid, idx->dbid);
      if (key.size >= sizeof(keybuf)) {
        rc = IW_ERROR_OVERFLOW;
        goto finish;
      }
      rc = iwkv_del(db->metadb, &key, 0); if (__builtin_expect((!!(rc)), 0)) goto finish;
...
finish:
  free(ptr);
  do { rci = pthread_rwlock_unlock(&(jbc)->rwl); if (rci) { iwrc __iwrc = (iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1098, ""); } }; rci = pthread_rwlock_unlock(&((jbc)->db)->rwl); if (rci) { iwrc __iwrc = (iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1098, ""); } }; } while (0);
  return rc;
}
...
iwrc ejdb_ensure_index(EJDB db, const char *coll, const char *path, ejdb_idx_mode_t mode) {
  ...
  switch (mode & (((ejdb_idx_mode_t) 0x04U) | ((ejdb_idx_mode_t) 0x08U) | ((ejdb_idx_mode_t) 0x10U))) {
    case ((ejdb_idx_mode_t) 0x04U):
    case ((ejdb_idx_mode_t) 0x08U):
    case ((ejdb_idx_mode_t) 0x10U):
      break;
    default:
      return EJDB_ERROR_INVALID_INDEX_MODE;
  }

  iwrc rc = _jb_coll_acquire_keeplock(db, coll, 
# 1124 "/ejdb/src/ejdb2.c" 3 4
                                               1
# 1124 "/ejdb/src/ejdb2.c"
                                                   , &jbc);
  if (__builtin_expect((!!(rc)), 0)) return (rc);
  ...
  if (!idx) {
    rc = iwrc_set_errno(IW_ERROR_ALLOC, 
# 1141 "/ejdb/src/ejdb2.c" 3 4
                                       (*__errno_location ())
# 1141 "/ejdb/src/ejdb2.c"
                                            );
    goto finish;
  }
  idx->mode = mode;
  ...
  ptr = 0;
  idx->idbf = 0;
  if (mode & ((ejdb_idx_mode_t) 0x08U)) {
    idx->idbf |= ((iwdb_flags_t) 0x20U);
  } else if (mode & ((ejdb_idx_mode_t) 0x10U)) {
    idx->idbf |= ((iwdb_flags_t) 0x10U);
  }
  if (!(mode & ((ejdb_idx_mode_t) 0x01U))) {
    idx->idbf |= ((iwdb_flags_t) 0x40U);
  }

  rc = iwkv_new_db(db->iwkv, idx->idbf, &idx->dbid, &idx->idb); if (__builtin_expect((!!(rc)), 0)) goto finish;
  rc = _jb_idx_fill(idx); if (__builtin_expect((!!(rc)), 0)) goto finish;


  imeta = binn_object();
  if (!imeta) {
    rc = iwrc_set_errno(IW_ERROR_ALLOC, 
# 1164 "/ejdb/src/ejdb2.c" 3 4
                                       (*__errno_location ())
# 1164 "/ejdb/src/ejdb2.c"
                                            );
    goto finish;
  }
  ...
  rc = iwkv_put(db->metadb, &key, &val, 0); if (__builtin_expect((!!(rc)), 0)) goto finish;
...
finish:
  ...
  free(ptr);
  binn_free(imeta);
  do { rci = pthread_rwlock_unlock(&(jbc)->rwl); if (rci) { iwrc __iwrc = (iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1202, ""); } }; rci = pthread_rwlock_unlock(&((jbc)->db)->rwl); if (rci) { iwrc __iwrc = (iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1202, ""); } }; } while (0);
  return rc;
}

static iwrc _jb_patch(
  EJDB db, const char *coll, int64_t id, 
# 1207 "/ejdb/src/ejdb2.c" 3 4
                                        _Bool 
# 1207 "/ejdb/src/ejdb2.c"
                                             upsert,
  ...
  iwrc rc = _jb_coll_acquire_keeplock(db, coll, 
# 1222 "/ejdb/src/ejdb2.c" 3 4
                                               1
# 1222 "/ejdb/src/ejdb2.c"
                                                   , &jbc);
  if (__builtin_expect((!!(rc)), 0)) return (rc);
  ...
  rc = jbl_from_buf_keep_onstack(&sjbl, val.data, val.size); if (__builtin_expect((!!(rc)), 0)) goto finish;

  pool = iwpool_create_empty();
  if (!pool) {
    rc = iwrc_set_errno(IW_ERROR_ALLOC, 
# 1254 "/ejdb/src/ejdb2.c" 3 4
                                       (*__errno_location ())
# 1254 "/ejdb/src/ejdb2.c"
                                            );
    goto finish;
  }

  rc = jbl_to_node(&sjbl, &root, 
# 1258 "/ejdb/src/ejdb2.c" 3 4
 0
# 1258 "/ejdb/src/ejdb2.c"
 , pool); if (__builtin_expect((!!(rc)), 0)) goto finish;

  if (patchjson) {
    rc = jbn_from_json(patchjson, &patch, pool);
  } else if (patchjbl) {
    rc = jbl_to_node(patchjbl, &patch, 
# 1263 "/ejdb/src/ejdb2.c" 3 4
                                      0
# 1263 "/ejdb/src/ejdb2.c"
                                           , pool);
  } else if (patchjbn) {
    patch = patchjbn;
  } else {
  ...
  rc = jbn_patch_auto(root, patch, pool); if (__builtin_expect((!!(rc)), 0)) goto finish;

  if (root->type == JBV_OBJECT) {
    rc = jbl_create_empty_object(&ujbl); if (__builtin_expect((!!(rc)), 0)) goto finish;
  } else if (root->type == JBV_ARRAY) {
    rc = jbl_create_empty_array(&ujbl); if (__builtin_expect((!!(rc)), 0)) goto finish;
  } else {
    rc = JBL_ERROR_CREATION;
    goto finish;
  }

  rc = jbl_fill_from_node(ujbl, root); if (__builtin_expect((!!(rc)), 0)) goto finish;
  rc = _jb_put_impl(jbc, ujbl, id);

finish:
  do { rci = pthread_rwlock_unlock(&(jbc)->rwl); if (rci) { iwrc __iwrc = (iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1286, ""); } }; rci = pthread_rwlock_unlock(&((jbc)->db)->rwl); if (rci) { iwrc __iwrc = (iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1286, ""); } }; } while (0);
  if (ujbl != patchjbl) {
    jbl_destroy(&ujbl);
  }
  if (val.data) {
...
static iwrc _jb_wal_lock_interceptor(
# 1297 "/ejdb/src/ejdb2.c" 3 4
                                    _Bool 
# 1297 "/ejdb/src/ejdb2.c"
                                         before, void *op) {
  int rci;
  iwrc rc = 0;
  EJDB db = op;
  
# 1301 "/ejdb/src/ejdb2.c" 3 4
 ((void) (0))
# 1301 "/ejdb/src/ejdb2.c"
           ;
  if (before) {
    rci = pthread_rwlock_wrlock(&(db)->rwl); if (rci) return iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci);
  } else {
    rci = pthread_rwlock_unlock(&(db)->rwl); if (rci) { iwrc __iwrc = (iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1305, ""); } };
  }
  return rc;
}

iwrc ejdb_patch(EJDB db, const char *coll, const char *patchjson, int64_t id) {
  return _jb_patch(db, coll, id, 
# 1311 "/ejdb/src/ejdb2.c" 3 4
                                0
# 1311 "/ejdb/src/ejdb2.c"
                                     , patchjson, 0, 0);
}

iwrc ejdb_patch_jbn(EJDB db, const char *coll, JBL_NODE patch, int64_t id) {
  return _jb_patch(db, coll, id, 
# 1315 "/ejdb/src/ejdb2.c" 3 4
                                0
# 1315 "/ejdb/src/ejdb2.c"
                                     , 0, patch, 0);
}

iwrc ejdb_patch_jbl(EJDB db, const char *coll, JBL patch, int64_t id) {
  return _jb_patch(db, coll, id, 
# 1319 "/ejdb/src/ejdb2.c" 3 4
                                0
# 1319 "/ejdb/src/ejdb2.c"
                                     , 0, 0, patch);
}

iwrc ejdb_merge_or_put(EJDB db, const char *coll, const char *patchjson, int64_t id) {
  return _jb_patch(db, coll, id, 
# 1323 "/ejdb/src/ejdb2.c" 3 4
                                1
# 1323 "/ejdb/src/ejdb2.c"
                                    , patchjson, 0, 0);
}

iwrc ejdb_merge_or_put_jbn(EJDB db, const char *coll, JBL_NODE patch, int64_t id) {
  return _jb_patch(db, coll, id, 
# 1327 "/ejdb/src/ejdb2.c" 3 4
                                1
# 1327 "/ejdb/src/ejdb2.c"
                                    , 0, patch, 0);
}

iwrc ejdb_merge_or_put_jbl(EJDB db, const char *coll, JBL patch, int64_t id) {
  return _jb_patch(db, coll, id, 
# 1331 "/ejdb/src/ejdb2.c" 3 4
                                1
# 1331 "/ejdb/src/ejdb2.c"
                                    , 0, 0, patch);
}
...
iwrc ejdb_put(EJDB db, const char *coll, JBL jbl, int64_t id) {
  ...
  JBCOLL jbc;
  iwrc rc = _jb_coll_acquire_keeplock(db, coll, 
# 1340 "/ejdb/src/ejdb2.c" 3 4
                                               1
# 1340 "/ejdb/src/ejdb2.c"
                                                   , &jbc);
  if (__builtin_expect((!!(rc)), 0)) return (rc);
  ...
  do { rci = pthread_rwlock_unlock(&(jbc)->rwl); if (rci) { iwrc __iwrc = (iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1346, ""); } }; rci = pthread_rwlock_unlock(&((jbc)->db)->rwl); if (rci) { iwrc __iwrc = (iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1346, ""); } }; } while (0);
  return rc;
}
...
static iwrc _jb_put_new_lw(JBCOLL jbc, JBL jbl, int64_t *id) {
  ...
  rc = jbl_as_buf(jbl, &val.data, &val.size); if (__builtin_expect((!!(rc)), 0)) goto finish;
  rc = _jb_put_handler_after(iwkv_puth(jbc->cdb, &key, &val, 0, _jb_put_handler, &pctx), &pctx); if (__builtin_expect((!!(rc)), 0)) goto finish;
...
iwrc ejdb_put_new(EJDB db, const char *coll, JBL jbl, int64_t *id) {
  ...
  iwrc rc = _jb_coll_acquire_keeplock(db, coll, 
# 1393 "/ejdb/src/ejdb2.c" 3 4
                                               1
# 1393 "/ejdb/src/ejdb2.c"
                                                   , &jbc);
  if (__builtin_expect((!!(rc)), 0)) return (rc);

  rc = _jb_put_new_lw(jbc, jbl, id);

  do { rci = pthread_rwlock_unlock(&(jbc)->rwl); if (rci) { iwrc __iwrc = (iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1398, ""); } }; rci = pthread_rwlock_unlock(&((jbc)->db)->rwl); if (rci) { iwrc __iwrc = (iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1398, ""); } }; } while (0);
  return rc;
}
...
iwrc jb_get(EJDB db, const char *coll, int64_t id, jb_coll_acquire_t acm, JBL *jblp) {
  ...
  *jblp = 0;
  ...
  rc = iwkv_get(jbc->cdb, &key, &val); if (__builtin_expect((!!(rc)), 0)) goto finish;
  rc = jbl_from_buf_keep(&jbl, val.data, val.size, 
# 1427 "/ejdb/src/ejdb2.c" 3 4
 0
# 1427 "/ejdb/src/ejdb2.c"
 ); if (__builtin_expect((!!(rc)), 0)) goto finish;
...
finish:
  ...
  do { rci = pthread_rwlock_unlock(&(jbc)->rwl); if (rci) { iwrc __iwrc = (iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1439, ""); } }; rci = pthread_rwlock_unlock(&((jbc)->db)->rwl); if (rci) { iwrc __iwrc = (iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1439, ""); } }; } while (0);
  return rc;
}

iwrc ejdb_get(EJDB db, const char *coll, int64_t id, JBL *jblp) {
  return jb_get(db, coll, id, ((jb_coll_acquire_t) 0x02U), jblp);
}
...
iwrc ejdb_del(EJDB db, const char *coll, int64_t id) {
  ...
  rc = iwkv_get(jbc->cdb, &key, &val); if (__builtin_expect((!!(rc)), 0)) goto finish;
  rc = jbl_from_buf_keep_onstack(&jbl, val.data, val.size); if (__builtin_expect((!!(rc)), 0)) goto finish;

  for (JBIDX idx = jbc->idx; idx; idx = idx->next) {
    { iwrc __iwrc = (_jb_idx_record_remove(idx, id, &jbl)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1461, ""); } };
  }

  rc = iwkv_del(jbc->cdb, &key, 0); if (__builtin_expect((!!(rc)), 0)) goto finish;
  _jb_meta_nrecs_update(jbc->db, jbc->dbid, -1);
  jbc->rnum -= 1;
...
finish:
  ...
  do { rci = pthread_rwlock_unlock(&(jbc)->rwl); if (rci) { iwrc __iwrc = (iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1472, ""); } }; rci = pthread_rwlock_unlock(&((jbc)->db)->rwl); if (rci) { iwrc __iwrc = (iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1472, ""); } }; } while (0);
  return rc;
}
...
iwrc jb_del(JBCOLL jbc, JBL jbl, int64_t id) {
  ...
  IWKV_val key = { .data = &id, .size = sizeof(id) };
  for (JBIDX idx = jbc->idx; idx; idx = idx->next) {
    { iwrc __iwrc = (_jb_idx_record_remove(idx, id, jbl)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1480, ""); } };
  }
...
iwrc jb_cursor_del(JBCOLL jbc, IWKV_cursor cur, int64_t id, JBL jbl) {
  iwrc rc = 0;
  for (JBIDX idx = jbc->idx; idx; idx = idx->next) {
    { iwrc __iwrc = (_jb_idx_record_remove(idx, id, jbl)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1492, ""); } };
  }
...
iwrc ejdb_ensure_collection(EJDB db, const char *coll) {
  ...
  JBCOLL jbc;
  iwrc rc = _jb_coll_acquire_keeplock(db, coll, 
# 1504 "/ejdb/src/ejdb2.c" 3 4
                                               0
# 1504 "/ejdb/src/ejdb2.c"
                                                    , &jbc);
  if (__builtin_expect((!!(rc)), 0)) return (rc);
  do { rci = pthread_rwlock_unlock(&(jbc)->rwl); if (rci) { iwrc __iwrc = (iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1506, ""); } }; rci = pthread_rwlock_unlock(&((jbc)->db)->rwl); if (rci) { iwrc __iwrc = (iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1506, ""); } }; } while (0);
  return rc;
}
...
iwrc ejdb_remove_collection(EJDB db, const char *coll) {
  int rci;
  iwrc rc = 0;
  if (db->oflags & ((iwkv_openflags) 0x02U)) {
    return IW_ERROR_READONLY;
  }
  if (!(db) || !((db)->open)) { iwlog3(IWLOG_ERROR, 0, "/ejdb/src/ejdb2.c", 1516, ("Database is not open")); return IW_ERROR_INVALID_STATE; }; rci = pthread_rwlock_wrlock(&(db)->rwl); if (rci) return iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci);
  JBCOLL jbc;
  IWKV_val key;
  char keybuf[sizeof("i.") + 1 + 2 * 32];
  ...
  if (jbc) {
    key.data = keybuf;
    key.size = snprintf(keybuf, sizeof(keybuf), "c." "%u", jbc->dbid);

    rc = iwkv_del(jbc->db->metadb, &key, ((iwkv_opflags) 0x04U)); if (__builtin_expect((!!(rc)), 0)) goto finish;
    ...
    for (JBIDX idx = jbc->idx; idx; idx = idx->next) {
      key.data = keybuf;
      key.size = snprintf(keybuf, sizeof(keybuf), "i." "%u" "." "%u", jbc->dbid, idx->dbid);
      rc = iwkv_del(jbc->db->metadb, &key, 0); if (__builtin_expect((!!(rc)), 0)) goto finish;
      _jb_meta_nrecs_removedb(db, idx->dbid);
    }
    for (JBIDX idx = jbc->idx, nidx; idx; idx = nidx) {
      { iwrc __iwrc = (iwkv_db_destroy(&idx->idb)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1537, ""); } };
      idx->idb = 0;
      nidx = idx->next;
      _jb_idx_release(idx);
    }
    jbc->idx = 0;
    { iwrc __iwrc = (iwkv_db_destroy(&jbc->cdb)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1543, ""); } };
    iwhmap_remove(db->mcolls, coll);
  }

finish:
  rci = pthread_rwlock_unlock(&(db)->rwl); if (rci) { iwrc __iwrc = (iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1548, ""); } };
  return rc;
}

iwrc jb_collection_join_resolver(int64_t id, const char *coll, JBL *out, JBEXEC *ctx) {
  
# 1553 "/ejdb/src/ejdb2.c" 3 4
 ((void) (0))
# 1553 "/ejdb/src/ejdb2.c"
                           ;
  EJDB db = ctx->jbc->db;
  return jb_get(db, coll, id, ((jb_coll_acquire_t) 0x02U), out);
}
...
iwrc ejdb_rename_collection(EJDB db, const char *coll, const char *new_coll) {
  ...
  if (!(db) || !((db)->open)) { iwlog3(IWLOG_ERROR, 0, "/ejdb/src/ejdb2.c", 1590, ("Database is not open")); return IW_ERROR_INVALID_STATE; }; rci = pthread_rwlock_wrlock(&(db)->rwl); if (rci) return iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci);
  ...
  rc = jbl_create_empty_object(&nmeta); if (__builtin_expect((!!(rc)), 0)) goto finish;
  ...
  rc = jbl_as_buf(nmeta, &val.data, &val.size); if (__builtin_expect((!!(rc)), 0)) goto finish;
  key.size = snprintf(keybuf, sizeof(keybuf), "c." "%u", jbc->dbid);
  if (key.size >= sizeof(keybuf)) {
    rc = IW_ERROR_OVERFLOW;
    goto finish;
  ...
  rc = jbl_at(nmeta, "/name", &jbv); if (__builtin_expect((!!(rc)), 0)) goto finish;
  const char *new_name = jbl_get_str(jbv);
  rc = iwkv_put(db->metadb, &key, &val, ((iwkv_opflags) 0x04U)); if (__builtin_expect((!!(rc)), 0)) goto finish;
  rc = iwhmap_rename(db->mcolls, coll, (void*) new_name); if (__builtin_expect((!!(rc)), 0)) goto finish;
...
finish:
  ...
  rci = pthread_rwlock_unlock(&(db)->rwl); if (rci) { iwrc __iwrc = (iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1640, ""); } };
  return rc;
}
...
iwrc ejdb_get_meta(EJDB db, JBL *jblp) {
  ...
  *jblp = 0;
  ...
  if (__builtin_expect((!!(rc)), 0)) return (rc);
  binn *clist = 0;
  if (!(db) || !((db)->open)) { iwlog3(IWLOG_ERROR, 0, "/ejdb/src/ejdb2.c", 1651, ("Database is not open")); return IW_ERROR_INVALID_STATE; }; rci = pthread_rwlock_rdlock(&(db)->rwl); if (rci) return iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci);
  ...
  if (!clist) {
    rc = iwrc_set_errno(IW_ERROR_ALLOC, 
# 1666 "/ejdb/src/ejdb2.c" 3 4
                                       (*__errno_location ())
# 1666 "/ejdb/src/ejdb2.c"
                                            );
    goto finish;
  }
  ...
  while (iwhmap_iter_next(&iter)) {
    ...
    rc = _jb_coll_add_meta_lr(jbc, clist); if (__builtin_expect((!!(rc)), 0)) goto finish;
  }
...
finish:
  rci = pthread_rwlock_unlock(&(db)->rwl); if (rci) { iwrc __iwrc = (iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci)); if (__iwrc) { if (!(rc)) (rc) = __iwrc; else iwlog3(IWLOG_ERROR, __iwrc, "/ejdb/src/ejdb2.c", 1685, ""); } };
  if (rc) {
    if (clist) {
      binn_free(clist);
    }
...
iwrc ejdb_online_backup(EJDB db, uint64_t *ts, const char *target_file) {
  if (!(db) || !((db)->open)) { iwlog3(IWLOG_ERROR, 0, "/ejdb/src/ejdb2.c", 1698, ("Database is not open")); return IW_ERROR_INVALID_STATE; };
...
# 1727 "/ejdb/src/ejdb2.c"
  ...
  if (http->access_token) {
    ...
    if (!http->access_token) {
      return iwrc_set_errno(IW_ERROR_ALLOC, 
# 1750 "/ejdb/src/ejdb2.c" 3 4
                                           (*__errno_location ())
# 1750 "/ejdb/src/ejdb2.c"
                                                );
    }
  ...
  if (!(db->mcolls = iwhmap_create_str(_mcolls_map_entry_free))) { rc = iwrc_set_errno(IW_ERROR_ALLOC, 
# 1766 "/ejdb/src/ejdb2.c" 3 4
 (*__errno_location ())
# 1766 "/ejdb/src/ejdb2.c"
 ); goto finish; };
  ...
  rc = iwkv_open(&kvopts, &db->iwkv); if (__builtin_expect((!!(rc)), 0)) goto finish;

  db->oflags = kvopts.oflags;
  rc = _jb_db_meta_load(db); if (__builtin_expect((!!(rc)), 0)) goto finish;
  ...
  if (db->opts.http.enabled && !db->opts.http.blocking) {
    rc = jbr_start(db, &db->opts, &db->jbr); if (__builtin_expect((!!(rc)), 0)) goto finish;
  }
...
finish:
  if (rc) {
    ...
    db->open = 
# 1798 "/ejdb/src/ejdb2.c" 3 4
              1
# 1798 "/ejdb/src/ejdb2.c"
                  ;
    *ejdbp = db;
...
iwrc ejdb_close(EJDB *ejdbp) {
  ...
  if (!__sync_bool_compare_and_swap(&db->open, 1, 0)) {
    iwlog3(IWLOG_ERROR, 0, "/ejdb/src/ejdb2.c", 1815, ("Database is closed already"));
    return IW_ERROR_INVALID_STATE;
  }
