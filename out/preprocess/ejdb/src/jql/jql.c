# 0 "/ejdb/src/jql/jql.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "/ejdb/src/jql/jql.c"
...
# 2 "/ejdb/src/jql/jql.c" 2
# 11 "/ejdb/src/jql/jql.c"
typedef struct MCTX {
  int lvl;
  binn *bv;
  const char *key;
...
# 22 "/ejdb/src/jql/jql.c" 3 4
 _Bool 
# 22 "/ejdb/src/jql/jql.c"
      matched;
} MENCTX;
...
# 27 "/ejdb/src/jql/jql.c" 3 4
 _Bool 
# 27 "/ejdb/src/jql/jql.c"
           matched;
  int last_lvl;
  JQP_NODE *nodes;
  JQP_NODE *last_node;
...
static JQP_NODE* _jql_match_node(MCTX *mctx, JQP_NODE *n, 
# 34 "/ejdb/src/jql/jql.c" 3 4
                                                         _Bool 
# 34 "/ejdb/src/jql/jql.c"
                                                              *res, iwrc *rcp);

static inline __attribute__((always_inline)) void _jql_jqval_destroy(JQP_STRING *pv) {
  JQVAL *qv = pv->opaque;
  if (qv) {
    void *ptr;
    switch (qv->type) {
...
static iwrc _jql_set_placeholder(JQL q, const char *placeholder, int index, JQVAL *val) {
  ...
  if (!placeholder) {
    char nbuf[32];
    iwitoa(index, nbuf, 32);
    for (JQP_STRING *pv = aux->start_placeholder; pv; pv = pv->placeholder_next) {
      if ((pv->value[0] == '?') && !strcmp(pv->value + 1, nbuf)) {
        if ((pv->flavour & (((jqp_string_flavours_t) 0x04U) | ((jqp_string_flavours_t) 0x200U))) && val->type != JQVAL_STR) {
...
# 132 "/ejdb/src/jql/jql.c" 3 4
                                         (*__errno_location ())
# 132 "/ejdb/src/jql/jql.c"
                                              );
  }
  qv->refs = 0;
  qv->freefn = freefn;
...
iwrc jql_set_json_jbl(JQL q, const char *placeholder, int index, JBL jbl) {
  ...
  if (!pool) {
    return iwrc_set_errno(IW_ERROR_ALLOC, 
# 160 "/ejdb/src/jql/jql.c" 3 4
                                         (*__errno_location ())
# 160 "/ejdb/src/jql/jql.c"
                                              );
  }
  iwrc rc;
  JBL_NODE n;
  rc = jbl_to_node(jbl, &n, 
# 164 "/ejdb/src/jql/jql.c" 3 4
 1
# 164 "/ejdb/src/jql/jql.c"
...
iwrc jql_set_i64(JQL q, const char *placeholder, int index, int64_t val) {
  ...
  if (!qv) {
    return iwrc_set_errno(IW_ERROR_ALLOC, 
# 177 "/ejdb/src/jql/jql.c" 3 4
                                         (*__errno_location ())
# 177 "/ejdb/src/jql/jql.c"
                                              );
  }
  qv->refs = 0;
  qv->freefn = 0;
...
iwrc jql_set_f64(JQL q, const char *placeholder, int index, double val) {
  ...
  if (!qv) {
    return iwrc_set_errno(IW_ERROR_ALLOC, 
# 194 "/ejdb/src/jql/jql.c" 3 4
                                         (*__errno_location ())
# 194 "/ejdb/src/jql/jql.c"
                                              );
  }
  qv->refs = 0;
  qv->freefn = 0;
...
iwrc jql_set_str2(
  ...
  if (!qv) {
    return iwrc_set_errno(IW_ERROR_ALLOC, 
# 221 "/ejdb/src/jql/jql.c" 3 4
                                         (*__errno_location ())
# 221 "/ejdb/src/jql/jql.c"
                                              );
  }
  qv->refs = 0;
  qv->freefn = freefn;
...
iwrc jql_set_str3(JQL q, const char *placeholder, int index, const char *val_, size_t val_len) {
  ...
  if (!val) {
    return iwrc_set_errno(IW_ERROR_ALLOC, 
# 249 "/ejdb/src/jql/jql.c" 3 4
                                         (*__errno_location ())
# 249 "/ejdb/src/jql/jql.c"
                                              );
  }
  return jql_set_str2(q, placeholder, index, val, _freefn_str, 0);
}

iwrc jql_set_bool(JQL q, const char *placeholder, int index, 
# 254 "/ejdb/src/jql/jql.c" 3 4
                                                            _Bool 
# 254 "/ejdb/src/jql/jql.c"
                                                                 val) {
  JQVAL *qv = malloc(sizeof(*qv));
  if (!qv) {
    return iwrc_set_errno(IW_ERROR_ALLOC, 
# 257 "/ejdb/src/jql/jql.c" 3 4
                                         (*__errno_location ())
# 257 "/ejdb/src/jql/jql.c"
                                              );
  }
  qv->refs = 0;
  qv->freefn = 0;
...
iwrc jql_set_regexp2(
  ...
  iwrc rc = 0;
  JQVAL *qv = 0;
  struct iwre *rx = (expr && *expr != '\0') ? iwre_create(expr) : ((void*) (intptr_t) -1);
  if (!rx) {
    rc = JQL_ERROR_REGEXP_INVALID;
    goto finish;
  }
  if (!(qv = malloc(sizeof(*qv)))) { rc = iwrc_set_errno(IW_ERROR_ALLOC, 
# 282 "/ejdb/src/jql/jql.c" 3 4
 (*__errno_location ())
# 282 "/ejdb/src/jql/jql.c"
 ); goto finish; };
  qv->refs = 0;
  if (rx != ((void*) (intptr_t) -1)) {
    qv->freefn = freefn;
    qv->freefn_op = op;
  }
  qv->type = JQVAL_RE;
...
finish:
  if (rc) {
    ...
    free(qv);
  } else if (rx == ((void*) (intptr_t) -1) && freefn) {
      freefn((void*) expr, op);
  }
  return rc;
}
...
iwrc jql_set_null(JQL q, const char *placeholder, int index) {
  ...
  if (!qv) {
    return iwrc_set_errno(IW_ERROR_ALLOC, 
# 314 "/ejdb/src/jql/jql.c" 3 4
                                         (*__errno_location ())
# 314 "/ejdb/src/jql/jql.c"
                                              );
  }
  qv->refs = 0;
  qv->freefn = 0;
...
static 
# 327 "/ejdb/src/jql/jql.c" 3 4
      _Bool 
# 327 "/ejdb/src/jql/jql.c"
           _jql_need_deeper_match(JQP_EXPR_NODE *en, int lvl) {
  for (en = en->chain; en; en = en->next) {
    if (en->type == JQP_EXPR_NODE_TYPE) {
      if (_jql_need_deeper_match(en, lvl)) {
        return 
# 331 "/ejdb/src/jql/jql.c" 3 4
              1
# 331 "/ejdb/src/jql/jql.c"
                  ;
      }
      ...
      if (!fctx->matched && (fctx->last_lvl == lvl)) {
        return 
# 336 "/ejdb/src/jql/jql.c" 3 4
              1
# 336 "/ejdb/src/jql/jql.c"
                  ;
      }
    }
  }
  return 
# 340 "/ejdb/src/jql/jql.c" 3 4
        0
# 340 "/ejdb/src/jql/jql.c"
             ;
}

static void _jql_reset_expression_node(JQP_EXPR_NODE *en, JQP_AUX *aux, 
# 343 "/ejdb/src/jql/jql.c" 3 4
                                                                       _Bool 
# 343 "/ejdb/src/jql/jql.c"
                                                                            reset_match_cache) {
  MENCTX *ectx = en->opaque;
  ectx->matched = 
# 345 "/ejdb/src/jql/jql.c" 3 4
                 0
# 345 "/ejdb/src/jql/jql.c"
                      ;
  for (en = en->chain; en; en = en->next) {
    if (en->type == JQP_EXPR_NODE_TYPE) {
      ...
      MFCTX *fctx = ((JQP_FILTER*) en)->opaque;
      fctx->matched = 
# 351 "/ejdb/src/jql/jql.c" 3 4
                     0
# 351 "/ejdb/src/jql/jql.c"
                          ;
      fctx->last_lvl = -1;
      for (JQP_NODE *n = fctx->nodes; n; n = n->next) {
        ...
        if (reset_match_cache && (unit->type == JQP_EXPR_TYPE)) {
          for (JQP_EXPR *expr = &unit->expr; expr; expr = expr->next) expr->prematched = 
# 358 "/ejdb/src/jql/jql.c" 3 4
                                                                                        0
# 358 "/ejdb/src/jql/jql.c"
                                                                                             ;
        }
      }
    }
...
static iwrc _jql_init_expression_node(JQP_EXPR_NODE *en, JQP_AUX *aux) {
  ...
  if (!en->opaque) {
    return iwrc_set_errno(IW_ERROR_ALLOC, 
# 368 "/ejdb/src/jql/jql.c" 3 4
                                         (*__errno_location ())
# 368 "/ejdb/src/jql/jql.c"
                                              );
  }
  for (en = en->chain; en; en = en->next) {
    if (en->type == JQP_EXPR_NODE_TYPE) {
      ...
      if (!fctx) {
        return iwrc_set_errno(IW_ERROR_ALLOC, 
# 378 "/ejdb/src/jql/jql.c" 3 4
                                             (*__errno_location ())
# 378 "/ejdb/src/jql/jql.c"
                                                  );
      }
      f->opaque = fctx;
      fctx->last_lvl = -1;
...
# 407 "/ejdb/src/jql/jql.c" 3 4
                                       (*__errno_location ())
# 407 "/ejdb/src/jql/jql.c"
                                            );
    goto finish;
  }
  aux->mode = mode;
  ...
  rc = jqp_parse(aux); if (__builtin_expect((!!(rc)), 0)) goto finish;
...
void jql_reset(JQL q, 
# 463 "/ejdb/src/jql/jql.c" 3 4
                     _Bool 
# 463 "/ejdb/src/jql/jql.c"
                          reset_match_cache, 
# 463 "/ejdb/src/jql/jql.c" 3 4
                                             _Bool 
# 463 "/ejdb/src/jql/jql.c"
                                                  reset_placeholders) {
  q->matched = 
# 464 "/ejdb/src/jql/jql.c" 3 4
              0
# 464 "/ejdb/src/jql/jql.c"
                   ;
  q->dirty = 
# 465 "/ejdb/src/jql/jql.c" 3 4
            0
# 465 "/ejdb/src/jql/jql.c"
                 ;
  JQP_AUX *aux = q->aux;
...
void jql_destroy(JQL *qptr) {
  ...
  if (q) {
    ...
    for (JQP_OP *op = aux->start_op; op; op = op->next) {
      if (op->opaque) {
        if (op->value == JQP_OP_RE && op->opaque != ((void*) (intptr_t) -1)) {
...
static inline __attribute__((always_inline)) jqval_type_t _jql_binn_to_jqval(binn *vbinn, JQVAL *qval) {
  switch (vbinn->type) {
    case 0xE2:
    case 0xE1:
    case 0xE0:
...
static inline __attribute__((always_inline)) void _jql_node_to_jqval(JBL_NODE jn, JQVAL *qv) {
  switch (jn->type) {
    case JBV_STR:
      qv->type = JQVAL_STR;
      qv->vstr = jn->vptr;
...
static int _jql_cmp_jqval_pair(const JQVAL *left, const JQVAL *right, iwrc *rcp) {
  ...
  switch (lv->type) {
    ...
    case JQVAL_BINN: {
      if ( (rv->type != JQVAL_JBLNODE)
         || ((rv->vnode->type == JBV_ARRAY) && (lv->vbinn->type != 0xE0))
         || ((rv->vnode->type == JBV_OBJECT) && ((lv->vbinn->type != 0xE2) && (lv->vbinn->type != 0xE1)))) {
      ...
      if (!pool) {
        *rcp = iwrc_set_errno(IW_ERROR_ALLOC, 
# 734 "/ejdb/src/jql/jql.c" 3 4
                                             (*__errno_location ())
# 734 "/ejdb/src/jql/jql.c"
                                                  );
        return 0;
      }
      *rcp = _jbl_node_from_binn(lv->vbinn, &lnode, 
# 737 "/ejdb/src/jql/jql.c" 3 4
                                                   0
# 737 "/ejdb/src/jql/jql.c"
                                                        , pool);
      if (*rcp) {
        iwpool_destroy(pool);
        return 0;
...
static 
# 757 "/ejdb/src/jql/jql.c" 3 4
      _Bool 
# 757 "/ejdb/src/jql/jql.c"
           _jql_match_regexp(
  JQP_AUX *aux,
  ...
  if (lv->type >= JQVAL_JBLNODE) {
    *rcp = _JQL_ERROR_UNMATCHED;
    return 
# 778 "/ejdb/src/jql/jql.c" 3 4
          0
# 778 "/ejdb/src/jql/jql.c"
               ;
  }
  ...
  if (jqop->opaque) {
    ...
    switch (rv->type) {
      ...
      case JQVAL_I64: {
        ...
        if (*rcp) {
          return 
# 798 "/ejdb/src/jql/jql.c" 3 4
                0
# 798 "/ejdb/src/jql/jql.c"
                     ;
        }
        break;
      }
      case JQVAL_F64: {
        ...
        if (*rcp) {
          return 
# 807 "/ejdb/src/jql/jql.c" 3 4
                0
# 807 "/ejdb/src/jql/jql.c"
                     ;
        }
        break;
      }
      ...
      default:
        *rcp = _JQL_ERROR_UNMATCHED;
        return 
# 816 "/ejdb/src/jql/jql.c" 3 4
              0
# 816 "/ejdb/src/jql/jql.c"
                   ;
    }
    ...
    if (__builtin_expect(!!(*expr == '\0'), 0)) {
      ...
      if (!rx) {
        *rcp = JQL_ERROR_REGEXP_INVALID;
        return 
# 825 "/ejdb/src/jql/jql.c" 3 4
              0
# 825 "/ejdb/src/jql/jql.c"
                   ;
      }
    }
    jqop->opaque = rx;
  ...
  switch (lv->type) {
    ...
    default:
      *rcp = _JQL_ERROR_UNMATCHED;
      return 
# 850 "/ejdb/src/jql/jql.c" 3 4
            0
# 850 "/ejdb/src/jql/jql.c"
                 ;
  }

  if (__builtin_expect(!!(rx == ((void*) (intptr_t) -1)), 0)) {
    return *input == '\0';
  } else {
    const char *mpairs[64];
    int mret = iwre_match(rx, input, mpairs, 64);
    return mret > 0;
  }
}

static 
# 862 "/ejdb/src/jql/jql.c" 3 4
      _Bool 
# 862 "/ejdb/src/jql/jql.c"
           _jql_match_in(
  JQVAL *left, JQP_OP *jqop, JQVAL *right,
  iwrc *rcp
  ) {
  ...
  if ((rv->type != JQVAL_JBLNODE) && (rv->vnode->type != JBV_ARRAY)) {
    *rcp = _JQL_ERROR_UNMATCHED;
    return 
# 870 "/ejdb/src/jql/jql.c" 3 4
          0
# 870 "/ejdb/src/jql/jql.c"
               ;
  }
  if (lv->type == JQVAL_JBLNODE) {
    _jql_node_to_jqval(lv->vnode, &sleft);
  ...
  for (JBL_NODE n = rv->vnode->child; n; n = n->next) {
    ...
    if (!_jql_cmp_jqval_pair(lv, &qv, rcp)) {
      if (*rcp) {
        return 
# 886 "/ejdb/src/jql/jql.c" 3 4
              0
# 886 "/ejdb/src/jql/jql.c"
                   ;
      }
      return 
# 888 "/ejdb/src/jql/jql.c" 3 4
            1
# 888 "/ejdb/src/jql/jql.c"
                ;
    }
    if (*rcp) {
      return 
# 891 "/ejdb/src/jql/jql.c" 3 4
            0
# 891 "/ejdb/src/jql/jql.c"
                 ;
    }
  }
  return 
# 894 "/ejdb/src/jql/jql.c" 3 4
        0
# 894 "/ejdb/src/jql/jql.c"
             ;
}

static 
# 897 "/ejdb/src/jql/jql.c" 3 4
      _Bool 
# 897 "/ejdb/src/jql/jql.c"
           _jql_match_ni(
  JQVAL *left, JQP_OP *jqop, JQVAL *right,
  iwrc *rcp
  ) {
  ...
  binn bv;
  binn_iter iter;
  if ((rv->type != JQVAL_BINN) || (rv->vbinn->type != 0xE0)) {
    *rcp = _JQL_ERROR_UNMATCHED;
    return 
# 907 "/ejdb/src/jql/jql.c" 3 4
          0
# 907 "/ejdb/src/jql/jql.c"
               ;
  }
  if (lv->type == JQVAL_JBLNODE) {
    _jql_node_to_jqval(lv->vnode, &sleft);
  ...
  if (lv->type >= JQVAL_JBLNODE) {
    *rcp = _JQL_ERROR_UNMATCHED;
    return 
# 918 "/ejdb/src/jql/jql.c" 3 4
          0
# 918 "/ejdb/src/jql/jql.c"
               ;
  }
  if (!binn_iter_init(&iter, rv->vbinn, rv->vbinn->type)) {
    *rcp = JBL_ERROR_INVALID;
    return 
# 922 "/ejdb/src/jql/jql.c" 3 4
          0
# 922 "/ejdb/src/jql/jql.c"
               ;
  }
  while (binn_list_next(&iter, &bv)) {
    ...
    if (!_jql_cmp_jqval_pair(&qv, lv, rcp)) {
      if (*rcp) {
        return 
# 931 "/ejdb/src/jql/jql.c" 3 4
              0
# 931 "/ejdb/src/jql/jql.c"
                   ;
      }
      return 
# 933 "/ejdb/src/jql/jql.c" 3 4
            1
# 933 "/ejdb/src/jql/jql.c"
                ;
    } else if (*rcp) {
      return 
# 935 "/ejdb/src/jql/jql.c" 3 4
            0
# 935 "/ejdb/src/jql/jql.c"
                 ;
    }
  }
  return 
# 938 "/ejdb/src/jql/jql.c" 3 4
        0
# 938 "/ejdb/src/jql/jql.c"
             ;
}

static 
# 941 "/ejdb/src/jql/jql.c" 3 4
      _Bool 
# 941 "/ejdb/src/jql/jql.c"
           _jql_match_starts(
  JQVAL *left, JQP_OP *jqop, JQVAL *right,
  iwrc *rcp
  ) {
  ...
  switch (lv->type) {
    ...
    default:
      *rcp = _JQL_ERROR_UNMATCHED;
      return 
# 977 "/ejdb/src/jql/jql.c" 3 4
            0
# 977 "/ejdb/src/jql/jql.c"
                 ;
  }
  switch (rv->type) {
    case JQVAL_STR:
    ...
    default:
      *rcp = _JQL_ERROR_UNMATCHED;
      return 
# 998 "/ejdb/src/jql/jql.c" 3 4
            0
# 998 "/ejdb/src/jql/jql.c"
                 ;
  }
  ...
  if (plen > 0) {
    ...
    return 
# 1004 "/ejdb/src/jql/jql.c" 3 4
          1
# 1004 "/ejdb/src/jql/jql.c"
              ;
  }
}

static 
# 1008 "/ejdb/src/jql/jql.c" 3 4
      _Bool 
# 1008 "/ejdb/src/jql/jql.c"
           _jql_match_jqval_pair(
  JQP_AUX *aux,
  JQVAL *left, JQP_OP *jqop, JQVAL *right,
  iwrc *rcp
  ) {
  
# 1013 "/ejdb/src/jql/jql.c" 3 4
 _Bool 
# 1013 "/ejdb/src/jql/jql.c"
      match = 
# 1013 "/ejdb/src/jql/jql.c" 3 4
              0
# 1013 "/ejdb/src/jql/jql.c"
...
finish:
  if (*rcp) {
    ...
    match = 
# 1062 "/ejdb/src/jql/jql.c" 3 4
           0
# 1062 "/ejdb/src/jql/jql.c"
                ;
  }
  if (jqop->negate) {
    match = !match;
...
# 1070 "/ejdb/src/jql/jql.c" 3 4
_Bool 
# 1070 "/ejdb/src/jql/jql.c"
    jql_match_jqval_pair(
  JQP_AUX *aux,
  JQVAL *left, JQP_OP *jqop, JQVAL *right,
  iwrc *rcp
  ) {
  ...
  *rcp = 0;
  switch (unit->type) {
    case JQP_STRING_TYPE: {
      ...
      if (unit->string.flavour & ((jqp_string_flavours_t) 0x01U)) {
        *rcp = JQL_ERROR_INVALID_PLACEHOLDER;
        ...
        if (!qv) {
          *rcp = iwrc_set_errno(IW_ERROR_ALLOC, 
# 1091 "/ejdb/src/jql/jql.c" 3 4
                                               (*__errno_location ())
# 1091 "/ejdb/src/jql/jql.c"
                                                    );
          return 0;
        }
    ...
    case JQP_JSON_TYPE: {
      ...
      if (!qv) {
        *rcp = iwrc_set_errno(IW_ERROR_ALLOC, 
# 1106 "/ejdb/src/jql/jql.c" 3 4
                                             (*__errno_location ())
# 1106 "/ejdb/src/jql/jql.c"
                                                  );
        return 0;
      }
      unit->json.opaque = qv;
    ...
    case JQP_INTEGER_TYPE: {
      ...
      if (!qv) {
        *rcp = iwrc_set_errno(IW_ERROR_ALLOC, 
# 1144 "/ejdb/src/jql/jql.c" 3 4
                                             (*__errno_location ())
# 1144 "/ejdb/src/jql/jql.c"
                                                  );
        return 0;
      }
    ...
    case JQP_DOUBLE_TYPE: {
      ...
      if (!qv) {
        *rcp = iwrc_set_errno(IW_ERROR_ALLOC, 
# 1158 "/ejdb/src/jql/jql.c" 3 4
                                             (*__errno_location ())
# 1158 "/ejdb/src/jql/jql.c"
                                                  );
        return 0;
      }
    ...
    default:
      iwlog3(IWLOG_ERROR, (IW_ERROR_ASSERTION), "/ejdb/src/jql/jql.c", 1167, "");
...
# 1177 "/ejdb/src/jql/jql.c" 3 4
_Bool 
# 1177 "/ejdb/src/jql/jql.c"
    jql_jqval_as_int(JQVAL *jqval, int64_t *out) {
  switch (jqval->type) {
    case JQVAL_I64:
      *out = jqval->vi64;
      return 
# 1181 "/ejdb/src/jql/jql.c" 3 4
            1
# 1181 "/ejdb/src/jql/jql.c"
                ;
    case JQVAL_STR:
      *out = iwatoi(jqval->vstr);
      return 
# 1184 "/ejdb/src/jql/jql.c" 3 4
            1
# 1184 "/ejdb/src/jql/jql.c"
                ;
    case JQVAL_F64:
      *out = jqval->vf64;
      return 
# 1187 "/ejdb/src/jql/jql.c" 3 4
            1
# 1187 "/ejdb/src/jql/jql.c"
                ;
    case JQVAL_BOOL:
      *out = jqval->vbool ? 1 : 0;
      return 
# 1190 "/ejdb/src/jql/jql.c" 3 4
            1
# 1190 "/ejdb/src/jql/jql.c"
                ;
    case JQVAL_JBLNODE: {
      ...
      switch (n->type) {
        case JBV_I64:
          *out = n->vi64;
          return 
# 1196 "/ejdb/src/jql/jql.c" 3 4
                1
# 1196 "/ejdb/src/jql/jql.c"
                    ;
        case JBV_STR:
          *out = iwatoi(jqval->vstr);
          return 
# 1199 "/ejdb/src/jql/jql.c" 3 4
                1
# 1199 "/ejdb/src/jql/jql.c"
                    ;
        case JBV_F64:
          *out = n->vf64;
          return 
# 1202 "/ejdb/src/jql/jql.c" 3 4
                1
# 1202 "/ejdb/src/jql/jql.c"
                    ;
        case JBV_BOOL:
          *out = n->vbool ? 1 : 0;
          return 
# 1205 "/ejdb/src/jql/jql.c" 3 4
                1
# 1205 "/ejdb/src/jql/jql.c"
                    ;
        default:
          *out = 0;
          return 
# 1208 "/ejdb/src/jql/jql.c" 3 4
                0
# 1208 "/ejdb/src/jql/jql.c"
                     ;
      }
    }
    default:
      *out = 0;
      return 
# 1213 "/ejdb/src/jql/jql.c" 3 4
            0
# 1213 "/ejdb/src/jql/jql.c"
                 ;
  }
}

static 
# 1217 "/ejdb/src/jql/jql.c" 3 4
      _Bool 
# 1217 "/ejdb/src/jql/jql.c"
           _jql_match_node_expr_impl(MCTX *mctx, JQP_EXPR *expr, iwrc *rcp) {
  if (expr->prematched) {
    return 
# 1219 "/ejdb/src/jql/jql.c" 3 4
          1
# 1219 "/ejdb/src/jql/jql.c"
              ;
  }
  const 
# 1221 "/ejdb/src/jql/jql.c" 3 4
       _Bool 
# 1221 "/ejdb/src/jql/jql.c"
            negate = (expr->join && expr->join->negate);
  JQPUNIT *left = expr->left;
  JQP_OP *op = expr->op;
  JQPUNIT *right = expr->right;
  if (left->type == JQP_STRING_TYPE) {
    if (left->string.flavour & ((jqp_string_flavours_t) 0x20U)) {
      JQVAL lv, *rv = _jql_unit_to_jqval(mctx->aux, right, rcp);
      if (*rcp) {
        return 
# 1229 "/ejdb/src/jql/jql.c" 3 4
              0
# 1229 "/ejdb/src/jql/jql.c"
                   ;
      }
      lv.type = JQVAL_STR;
      lv.vstr = mctx->key;
      
# 1233 "/ejdb/src/jql/jql.c" 3 4
     _Bool 
# 1233 "/ejdb/src/jql/jql.c"
          ret = _jql_match_jqval_pair(mctx->aux, &lv, op, rv, rcp);
      return negate != (0 == !ret);
    } else if ( !(left->string.flavour & ((jqp_string_flavours_t) 0x80U))
              && (strcmp(mctx->key, left->string.value) != 0)) {
      return negate;
    }
  } else if (left->type == JQP_EXPR_TYPE) {
    if ((left->expr.left->type != JQP_STRING_TYPE) || !(left->expr.left->string.flavour & ((jqp_string_flavours_t) 0x20U))) {
      iwlog3(IWLOG_ERROR, (IW_ERROR_ASSERTION), "/ejdb/src/jql/jql.c", 1241, "");
      *rcp = IW_ERROR_ASSERTION;
      return 
# 1243 "/ejdb/src/jql/jql.c" 3 4
            0
# 1243 "/ejdb/src/jql/jql.c"
                 ;
    }
    JQVAL lv, *rv = _jql_unit_to_jqval(mctx->aux, left->expr.right, rcp);
    if (*rcp) {
      return 
# 1247 "/ejdb/src/jql/jql.c" 3 4
            0
# 1247 "/ejdb/src/jql/jql.c"
                 ;
    }
    lv.type = JQVAL_STR;
    lv.vstr = mctx->key;
  ...
  if (*rcp) {
    return 
# 1257 "/ejdb/src/jql/jql.c" 3 4
          0
# 1257 "/ejdb/src/jql/jql.c"
               ;
  }
  lv.type = JQVAL_BINN;
  lv.vbinn = mctx->bv;
  
# 1261 "/ejdb/src/jql/jql.c" 3 4
 _Bool 
# 1261 "/ejdb/src/jql/jql.c"
      ret = _jql_match_jqval_pair(mctx->aux, &lv, expr->op, rv, rcp);
  return negate != (0 == !ret);
}

static 
# 1265 "/ejdb/src/jql/jql.c" 3 4
      _Bool 
# 1265 "/ejdb/src/jql/jql.c"
           _jql_match_node_expr(MCTX *mctx, JQP_NODE *n, iwrc *rcp) {
  n->start = mctx->lvl;
  n->end = n->start;
  JQPUNIT *unit = n->value;
  if (unit->type != JQP_EXPR_TYPE) {
    iwlog3(IWLOG_ERROR, (IW_ERROR_ASSERTION), "/ejdb/src/jql/jql.c", 1270, "");
    *rcp = IW_ERROR_ASSERTION;
    return 
# 1272 "/ejdb/src/jql/jql.c" 3 4
          0
# 1272 "/ejdb/src/jql/jql.c"
               ;
  }
  
# 1274 "/ejdb/src/jql/jql.c" 3 4
 _Bool 
# 1274 "/ejdb/src/jql/jql.c"
      prev = 
# 1274 "/ejdb/src/jql/jql.c" 3 4
             0
# 1274 "/ejdb/src/jql/jql.c"
...
# 1276 "/ejdb/src/jql/jql.c" 3 4
   _Bool 
# 1276 "/ejdb/src/jql/jql.c"
        matched = _jql_match_node_expr_impl(mctx, expr, rcp);
    if (*rcp) {
      return 
# 1278 "/ejdb/src/jql/jql.c" 3 4
            0
# 1278 "/ejdb/src/jql/jql.c"
                 ;
    }
    ...
    if (!join) {
      ...
      if (join->value == JQP_JOIN_AND) {
        prev = prev && matched;
      } else if (prev || matched) {
        prev = 
# 1287 "/ejdb/src/jql/jql.c" 3 4
              1
# 1287 "/ejdb/src/jql/jql.c"
                  ;
        break;
      }
    }
...
# 1295 "/ejdb/src/jql/jql.c" 3 4
         _Bool 
# 1295 "/ejdb/src/jql/jql.c"
              _jql_match_node_field(MCTX *mctx, JQP_NODE *n, iwrc *rcp) {
  n->start = mctx->lvl;
  n->end = n->start;
  if (n->value->type != JQP_STRING_TYPE) {
    iwlog3(IWLOG_ERROR, (IW_ERROR_ASSERTION), "/ejdb/src/jql/jql.c", 1299, "");
    *rcp = IW_ERROR_ASSERTION;
    return 
# 1301 "/ejdb/src/jql/jql.c" 3 4
          0
# 1301 "/ejdb/src/jql/jql.c"
               ;
  }
  return (strcmp(n->value->string.value, mctx->key) == 0);
}

static inline __attribute__((always_inline)) JQP_NODE* _jql_match_node_anys(MCTX *mctx, JQP_NODE *n, 
# 1306 "/ejdb/src/jql/jql.c" 3 4
                                                                 _Bool 
# 1306 "/ejdb/src/jql/jql.c"
                                                                      *res, iwrc *rcp) {
  if (n->start < 0) {
    n->start = mctx->lvl;
  }
  ...
  *res = 
# 1321 "/ejdb/src/jql/jql.c" 3 4
        1
# 1321 "/ejdb/src/jql/jql.c"
            ;
  return n;
}

static JQP_NODE* _jql_match_node(MCTX *mctx, JQP_NODE *n, 
# 1325 "/ejdb/src/jql/jql.c" 3 4
                                                         _Bool 
# 1325 "/ejdb/src/jql/jql.c"
                                                              *res, iwrc *rcp) {
  switch (n->ntype) {
    case JQP_NODE_FIELD:
    ...
    case JQP_NODE_ANY:
      ...
      n->end = n->start;
      *res = 
# 1336 "/ejdb/src/jql/jql.c" 3 4
            1
# 1336 "/ejdb/src/jql/jql.c"
                ;
      return n;
    case JQP_NODE_ANYS:
...
static 
# 1344 "/ejdb/src/jql/jql.c" 3 4
      _Bool 
# 1344 "/ejdb/src/jql/jql.c"
           _jql_match_filter(JQP_FILTER *f, MCTX *mctx, iwrc *rcp) {
  MFCTX *fctx = f->opaque;
  if (fctx->matched) {
    return 
# 1347 "/ejdb/src/jql/jql.c" 3 4
          1
# 1347 "/ejdb/src/jql/jql.c"
              ;
  }
  
# 1349 "/ejdb/src/jql/jql.c" 3 4
 _Bool 
# 1349 "/ejdb/src/jql/jql.c"
      matched = 
# 1349 "/ejdb/src/jql/jql.c" 3 4
                0
# 1349 "/ejdb/src/jql/jql.c"
  ...
  if (fctx->last_lvl + 1 < lvl) {
    return 
# 1352 "/ejdb/src/jql/jql.c" 3 4
          0
# 1352 "/ejdb/src/jql/jql.c"
               ;
  }
  ...
  for (JQP_NODE *n = fctx->nodes; n; n = n->next) {
    if ((n->start < 0) || ((lvl >= n->start) && (lvl <= n->end))) {
      ...
      if (*rcp) {
        return 
# 1367 "/ejdb/src/jql/jql.c" 3 4
              0
# 1367 "/ejdb/src/jql/jql.c"
                   ;
      }
      if (matched) {
        if (n == fctx->last_node) {
          fctx->matched = 
# 1371 "/ejdb/src/jql/jql.c" 3 4
                         1
# 1371 "/ejdb/src/jql/jql.c"
                             ;
          mctx->q->dirty = 
# 1372 "/ejdb/src/jql/jql.c" 3 4
                          1
# 1372 "/ejdb/src/jql/jql.c"
                              ;
        }
        fctx->last_lvl = lvl;
      }
...
static 
# 1382 "/ejdb/src/jql/jql.c" 3 4
      _Bool 
# 1382 "/ejdb/src/jql/jql.c"
           _jql_match_expression_node(JQP_EXPR_NODE *en, MCTX *mctx, iwrc *rcp) {
  MENCTX *enctx = en->opaque;
  if (enctx->matched) {
    return 
# 1385 "/ejdb/src/jql/jql.c" 3 4
          1
# 1385 "/ejdb/src/jql/jql.c"
              ;
  }
  
# 1387 "/ejdb/src/jql/jql.c" 3 4
 _Bool 
# 1387 "/ejdb/src/jql/jql.c"
      prev = 
# 1387 "/ejdb/src/jql/jql.c" 3 4
             0
# 1387 "/ejdb/src/jql/jql.c"
...
# 1389 "/ejdb/src/jql/jql.c" 3 4
   _Bool 
# 1389 "/ejdb/src/jql/jql.c"
        matched = 
# 1389 "/ejdb/src/jql/jql.c" 3 4
                  0
# 1389 "/ejdb/src/jql/jql.c"
    ...
    if (*rcp) {
      return ((jbn_visitor_cmd_t) 0x01U);
    }
    ...
    if (!join) {
      ...
      if (join->value == JQP_JOIN_AND) {
        ...
        prev = 
# 1408 "/ejdb/src/jql/jql.c" 3 4
              1
# 1408 "/ejdb/src/jql/jql.c"
                  ;
        break;
      }
    }
...
static jbl_visitor_cmd_t _jql_match_visitor(int lvl, binn *bv, const char *key, int idx, JBL_VCTX *vctx, iwrc *rcp) {
  ...
  if (q->dirty) {
    q->dirty = 
# 1437 "/ejdb/src/jql/jql.c" 3 4
              0
# 1437 "/ejdb/src/jql/jql.c"
                   ;
    if (!_jql_need_deeper_match(mctx.aux->expr, lvl)) {
      return ((jbn_visitor_cmd_t) 0x02U);
    }
  }
  return 0;
}

iwrc jql_matched(JQL q, JBL jbl, 
# 1445 "/ejdb/src/jql/jql.c" 3 4
                                _Bool 
# 1445 "/ejdb/src/jql/jql.c"
                                     *out) {
  JBL_VCTX vctx = {
    .bn = &jbl->bn,
    .op = q
  ...
  if (en->flags & 0x01U) {
    q->matched = 
# 1452 "/ejdb/src/jql/jql.c" 3 4
                1
# 1452 "/ejdb/src/jql/jql.c"
                    ;
    *out = 
# 1453 "/ejdb/src/jql/jql.c" 3 4
          1
# 1453 "/ejdb/src/jql/jql.c"
              ;
    return 0;
  }
  *out = 
# 1456 "/ejdb/src/jql/jql.c" 3 4
        0
# 1456 "/ejdb/src/jql/jql.c"
             ;
  jql_reset(q, 
# 1457 "/ejdb/src/jql/jql.c" 3 4
              0
# 1457 "/ejdb/src/jql/jql.c"
                   , 
# 1457 "/ejdb/src/jql/jql.c" 3 4
                     0
# 1457 "/ejdb/src/jql/jql.c"
  ...
  if (en->chain && !en->chain->next && !en->next) {
    ...
    if (en->type == JQP_FILTER_TYPE) {
      ...
      if (n && ((n->ntype == JQP_NODE_ANYS) || (n->ntype == JQP_NODE_ANY)) && !n->next) {
        ...
        q->matched = 
# 1464 "/ejdb/src/jql/jql.c" 3 4
                    1
# 1464 "/ejdb/src/jql/jql.c"
                        ;
        *out = 
# 1465 "/ejdb/src/jql/jql.c" 3 4
              1
# 1465 "/ejdb/src/jql/jql.c"
                  ;
        return 0;
      }
    }
...
# 1493 "/ejdb/src/jql/jql.c" 3 4
_Bool 
# 1493 "/ejdb/src/jql/jql.c"
    jql_has_apply(JQL q) {
  return q->aux->apply || q->aux->apply_placeholder || (q->aux->qmode & (((jqp_query_mode_t) 0x04U) | ((jqp_query_mode_t) 0x10U)));
}


# 1497 "/ejdb/src/jql/jql.c" 3 4
_Bool 
# 1497 "/ejdb/src/jql/jql.c"
    jql_has_apply_upsert(JQL q) {
  return (q->aux->qmode & ((jqp_query_mode_t) 0x10U));
}


# 1501 "/ejdb/src/jql/jql.c" 3 4
_Bool 
# 1501 "/ejdb/src/jql/jql.c"
    jql_has_apply_delete(JQL q) {
  return (q->aux->qmode & ((jqp_query_mode_t) 0x04U));
}


# 1505 "/ejdb/src/jql/jql.c" 3 4
_Bool 
# 1505 "/ejdb/src/jql/jql.c"
    jql_has_projection(JQL q) {
  return q->aux->projection;
}


# 1509 "/ejdb/src/jql/jql.c" 3 4
_Bool 
# 1509 "/ejdb/src/jql/jql.c"
    jql_has_orderby(JQL q) {
  return q->aux->orderby_num > 0;
}


# 1513 "/ejdb/src/jql/jql.c" 3 4
_Bool 
# 1513 "/ejdb/src/jql/jql.c"
    jql_has_aggregate_count(JQL q) {
  return (q->aux->qmode & (((jqp_query_mode_t) 0x01U)));
}
...
static 
# 1571 "/ejdb/src/jql/jql.c" 3 4
      _Bool 
# 1571 "/ejdb/src/jql/jql.c"
           _jql_proj_matched(
  ...
  if (proj->cnt <= lvl) {
    return 
# 1578 "/ejdb/src/jql/jql.c" 3 4
          0
# 1578 "/ejdb/src/jql/jql.c"
               ;
  }
  if (proj->pos >= lvl) {
    proj->pos = lvl - 1;
...
# 1586 "/ejdb/src/jql/jql.c" 3 4
   ((void) (0))
# 1586 "/ejdb/src/jql/jql.c"
             ;
    if (ps->flavour & ((jqp_string_flavours_t) 0x04U)) {
      for (JQP_STRING *sn = ps; sn; sn = sn->subnext) {
        const char *pv = __builtin_expect(!!(sn->flavour & ((jqp_string_flavours_t) 0x01U)), 0) ? ((JQVAL*) sn->opaque)->vstr : sn->value;
      ...
      const char *pv = __builtin_expect(!!(ps->flavour & ((jqp_string_flavours_t) 0x01U)), 0) ? ((JQVAL*) ps->opaque)->vstr : ps->value;
      int pvlen = (int) strlen(pv);
  ...
  return 
# 1605 "/ejdb/src/jql/jql.c" 3 4
        0
# 1605 "/ejdb/src/jql/jql.c"
             ;
}

static 
# 1608 "/ejdb/src/jql/jql.c" 3 4
      _Bool 
# 1608 "/ejdb/src/jql/jql.c"
           _jql_proj_join_matched(
  int16_t lvl, JBL_NODE n,
  const char *key, int keylen,
  JBN_VCTX *vctx, JQP_PROJECTION *proj,
  JBL *out,
...
# 1623 "/ejdb/src/jql/jql.c" 3 4
 _Bool 
# 1623 "/ejdb/src/jql/jql.c"
      ret = 
# 1623 "/ejdb/src/jql/jql.c" 3 4
            0
# 1623 "/ejdb/src/jql/jql.c"
...
# 1626 "/ejdb/src/jql/jql.c" 3 4
 ((void) (0))
# 1626 "/ejdb/src/jql/jql.c"
           ;

  if (ps->flavour & ((jqp_string_flavours_t) 0x04U)) {
    for (JQP_STRING *sn = ps; sn; sn = sn->subnext) {
      pv = __builtin_expect(!!(sn->flavour & ((jqp_string_flavours_t) 0x01U)), 0) ? ((JQVAL*) sn->opaque)->vstr : sn->value;
      spos = strchr(pv, '<');
      if (!spos) {
        if ((strlen(pv) == keylen) && !strncmp(key, pv, keylen)) {
          proj->pos = lvl;
          return 
# 1635 "/ejdb/src/jql/jql.c" 3 4
                1
# 1635 "/ejdb/src/jql/jql.c"
                    ;
        }
      }
    ...
    pv = __builtin_expect(!!(ps->flavour & ((jqp_string_flavours_t) 0x01U)), 0) ? ((JQVAL*) ps->opaque)->vstr : ps->value;
    spos = strchr(pv, '<');
    
# 1646 "/ejdb/src/jql/jql.c" 3 4
   ((void) (0))
# 1646 "/ejdb/src/jql/jql.c"
               ;
    ret = !strncmp(key, pv, spos - pv);
  }
  if (ret) {
    ...
    if (*coll == '\0') {
      return 
# 1656 "/ejdb/src/jql/jql.c" 3 4
            0
# 1656 "/ejdb/src/jql/jql.c"
                 ;
    }
    ...
    if (!jql_jqval_as_int(&jqval, &id)) {
      ...
      return 
# 1661 "/ejdb/src/jql/jql.c" 3 4
            0
# 1661 "/ejdb/src/jql/jql.c"
                 ;
    }
    ...
    if (!pool) {
      ...
      if (!pool) {
        ...
        if (!(pool)) { rc = iwrc_set_errno(IW_ERROR_ALLOC, 
# 1669 "/ejdb/src/jql/jql.c" 3 4
       (*__errno_location ())
# 1669 "/ejdb/src/jql/jql.c"
       ); goto finish; };
        exec_ctx->proj_joined_nodes_pool = pool;
      } else if (cache && (iwpool_used_size(pool) > 10 * 1024 * 1024)) {
        exec_ctx->proj_joined_nodes_cache = 0;
        iwhmap_destroy(exec_ctx->proj_joined_nodes_cache);
        cache = 0;
        iwpool_destroy(pool);
        pool = iwpool_create(1024 * 1024);
        if (!(pool)) { rc = iwrc_set_errno(IW_ERROR_ALLOC, 
# 1677 "/ejdb/src/jql/jql.c" 3 4
       (*__errno_location ())
# 1677 "/ejdb/src/jql/jql.c"
       ); goto finish; };
      }
    }
    if (!cache) {
      if (!(cache = iwhmap_create(jb_proj_node_cache_cmp, jb_proj_node_hash, jb_proj_node_kvfree))) { rc = iwrc_set_errno(IW_ERROR_ALLOC, 
# 1681 "/ejdb/src/jql/jql.c" 3 4
     (*__errno_location ())
# 1681 "/ejdb/src/jql/jql.c"
     ); goto finish; };
    ...
    if (!nn) {
      ...
      if (rc) {
        if ((rc == IW_ERROR_NOT_EXISTS) || (rc == IWKV_ERROR_NOTFOUND)) {
          ...
          rc = 0;
          ret = 
# 1696 "/ejdb/src/jql/jql.c" 3 4
               0
# 1696 "/ejdb/src/jql/jql.c"
                    ;
        }
        goto finish;
      }
      rc = jbl_to_node(jbl, &nn, 
# 1700 "/ejdb/src/jql/jql.c" 3 4
     1
# 1700 "/ejdb/src/jql/jql.c"
     , pool); if (__builtin_expect((!!(rc)), 0)) goto finish;
      struct _JBDOCREF *refkey = malloc(sizeof(*refkey));
      if (!(refkey)) { rc = iwrc_set_errno(IW_ERROR_ALLOC, 
# 1702 "/ejdb/src/jql/jql.c" 3 4
     (*__errno_location ())
# 1702 "/ejdb/src/jql/jql.c"
     ); goto finish; };
      *refkey = ref;
      rc = iwhmap_put(cache, refkey, nn); if (__builtin_expect((!!(rc)), 0)) goto finish;
    }
    jbn_apply_from(n, nn);
    proj->pos = lvl;
  }
...
# 1732 "/ejdb/src/jql/jql.c" 3 4
   _Bool 
# 1732 "/ejdb/src/jql/jql.c"
        matched;
    if (flags & 0x04U) {
    ...
    if (matched) {
      if (flags & 0x01U) {
        return ((jbn_visitor_cmd_t) 0x04U);
      } else if (flags & 0x02U) {
        _jql_proj_mark_up(n, 0x02);
      } else if ((flags & 0x04U) && pctx->q->aux->has_keep_projections) {
        _jql_proj_mark_up(n, 0x02 | 0x04);
      }
    }
  }
  return 0;
...
static jbn_visitor_cmd_t _jql_proj_keep_visitor(
  ...
  if ((lvl < 0) || (n->flags & 0x01)) {
    return 0;
  }
  if (n->flags & 0x02) {
    return (n->flags & 0x04) ? ((jbn_visitor_cmd_t) 0x00U) : ((jbn_visitor_cmd_t) 0x02U);
  }
  return ((jbn_visitor_cmd_t) 0x04U);
}
...
static iwrc _jql_project(JBL_NODE root, JQL q, IWPOOL *pool, JBEXEC *exec_ctx) {
  ...
  rc = jbn_visit(root, 0, &vctx, _jql_proj_visitor); if (__builtin_expect((!!(rc)), 0)) goto finish;
  if (aux->has_keep_projections) {
    rc = jbn_visit(root, 0, &vctx, _jql_proj_keep_visitor); if (__builtin_expect((!!(rc)), 0)) goto finish;
  }
...
iwrc jql_apply_and_project(JQL q, JBL jbl, JBL_NODE *out, void *exec_ctx, IWPOOL *pool) {
  *out = 0;
  ...
  JBL_NODE root;
  iwrc rc = jbl_to_node(jbl, &root, 
# 1843 "/ejdb/src/jql/jql.c" 3 4
                                   0
# 1843 "/ejdb/src/jql/jql.c"
                                        , pool);
  if (__builtin_expect((!!(rc)), 0)) return (rc);
