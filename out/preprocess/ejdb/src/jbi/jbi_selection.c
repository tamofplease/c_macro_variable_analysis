# 0 "/ejdb/src/jbi/jbi_selection.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "/ejdb/src/jbi/jbi_selection.c"
...
# 2 "/ejdb/src/jbi/jbi_selection.c" 2
...
static void _jbi_print_index(struct _JBIDX *idx, IWXSTR *xstr) {
  ...
  if (m & ((ejdb_idx_mode_t) 0x01U)) {
    cnt++;
    iwxstr_cat2(xstr, "UNIQUE");
  }
  if (m & ((ejdb_idx_mode_t) 0x04U)) {
    if (cnt++) {
      iwxstr_cat2(xstr, "|");
    }
    iwxstr_cat2(xstr, "STR");
  }
  if (m & ((ejdb_idx_mode_t) 0x08U)) {
    if (cnt++) {
      iwxstr_cat2(xstr, "|");
    }
    iwxstr_cat2(xstr, "I64");
  }
  if (m & ((ejdb_idx_mode_t) 0x10U)) {
    if (cnt++) {
      iwxstr_cat2(xstr, "|");
    }
  ...
  iwxstr_printf(xstr, "%" 
# 33 "/ejdb/src/jbi/jbi_selection.c" 3 4
                         "l" "d" 
# 33 "/ejdb/src/jbi/jbi_selection.c"
                                " ", idx->rnum);
...
static inline __attribute__((always_inline)) int _jbi_idx_expr_op_weight(struct _JBMIDX *midx) {
...
static 
# 114 "/ejdb/src/jbi/jbi_selection.c" 3 4
      _Bool 
# 114 "/ejdb/src/jbi/jbi_selection.c"
           _jbi_is_solid_node_expression(const JQP_NODE *n) {
  ...
  for (const JQP_EXPR *expr = &unit->expr; expr; expr = expr->next) {
    if ( expr->op->negate
      ...
      return 
# 121 "/ejdb/src/jbi/jbi_selection.c" 3 4
            0
# 121 "/ejdb/src/jbi/jbi_selection.c"
                 ;
    }
    JQPUNIT *left = expr->left;
    if ( (left->type == JQP_EXPR_TYPE)
       || ((left->type == JQP_STRING_TYPE) && (left->string.flavour & ((jqp_string_flavours_t) 0x20U)))) {
      return 
# 126 "/ejdb/src/jbi/jbi_selection.c" 3 4
            0
# 126 "/ejdb/src/jbi/jbi_selection.c"
                 ;
    }
  }
  return 
# 129 "/ejdb/src/jbi/jbi_selection.c" 3 4
        1
# 129 "/ejdb/src/jbi/jbi_selection.c"
            ;
}

static iwrc _jbi_compute_index_rules(JBEXEC *ctx, struct _JBMIDX *mctx) {
  JQP_EXPR *expr = mctx->nexpr;
  if (!expr) {
    return 0;
  }
  ...
  for ( ; expr; expr = expr->next) {
    ...
    switch (rv->type) {
      ...
      case JQVAL_JBLNODE: {
        ...
        if ( (vcnt > 10)
           && ( (vcnt > 500)
              || (mctx->idx->rnum < rv->vbinn->count * 200))) {
    ...
    switch (op) {
      case JQP_OP_EQ:
        ...
        return 0;
      case JQP_OP_PREFIX:
        if (!(mctx->idx->mode & ((ejdb_idx_mode_t) 0x04U))) {
          mctx->expr1 = 0;
          return 0;
        }
...
# 237 "/ejdb/src/jbi/jbi_selection.c" 3 4
     _Bool 
# 237 "/ejdb/src/jbi/jbi_selection.c"
          desc = (aux->orderby_ptrs[0]->op & 1) != 0;
      if (desc) {
        if (mctx->cursor_step != IWKV_CURSOR_NEXT) {
          mctx->orderby_support = 
# 240 "/ejdb/src/jbi/jbi_selection.c" 3 4
                                 0
# 240 "/ejdb/src/jbi/jbi_selection.c"
                                      ;
        }
      } else {
        if (mctx->cursor_step != IWKV_CURSOR_PREV) {
          mctx->orderby_support = 
# 244 "/ejdb/src/jbi/jbi_selection.c" 3 4
                                 0
# 244 "/ejdb/src/jbi/jbi_selection.c"
                                      ;
        }
      }
      if (!mctx->orderby_support && mctx->expr2) {
        ...
        mctx->expr2 = tmp;
        mctx->orderby_support = 
# 251 "/ejdb/src/jbi/jbi_selection.c" 3 4
                               1
# 251 "/ejdb/src/jbi/jbi_selection.c"
                                   ;
        if (desc) {
          mctx->cursor_step = IWKV_CURSOR_NEXT;
        } else {
      ...
      mctx->orderby_support = 
# 259 "/ejdb/src/jbi/jbi_selection.c" 3 4
                             0
# 259 "/ejdb/src/jbi/jbi_selection.c"
                                  ;
    }
  }
...
static iwrc _jbi_collect_indexes(
  ...
  struct _JBMIDX marr[static 127],
  size_t *snp
  ) {
  iwrc rc = 0;
  if (*snp >= 127 - 1) {
  ...
  if (en->type == JQP_EXPR_NODE_TYPE) {
    ...
    for (struct _JBIDX *idx = ctx->jbc->idx; idx && *snp < 127; idx = idx->next) {
      ...
      for (JQP_NODE *n = f->node; n && i < ptr->cnt; n = n->next, ++i) {
        ...
        if ( (i == ptr->cnt - 1)
          ...
          if ((left->type == JQP_STRING_TYPE) && (left->string.flavour & ((jqp_string_flavours_t) 0x80U))) {
            i++;
            j++;
...
# 374 "/ejdb/src/jbi/jbi_selection.c" 3 4
 ((void) (0))
# 374 "/ejdb/src/jbi/jbi_selection.c"
                 ;
...
# 394 "/ejdb/src/jbi/jbi_selection.c" 3 4
 ((void) (0))
# 394 "/ejdb/src/jbi/jbi_selection.c"
            ;
  for (struct _JBIDX *idx = ctx->jbc->idx; idx; idx = idx->next) {
    ...
    if (i == obp->cnt) {
      ...
      ctx->midx.idx = idx;
      ctx->midx.orderby_support = 
# 409 "/ejdb/src/jbi/jbi_selection.c" 3 4
                                 1
# 409 "/ejdb/src/jbi/jbi_selection.c"
                                     ;
      ctx->midx.cursor_init = ctx->cursor_init;
      ctx->midx.cursor_step = ctx->cursor_step;
      ctx->sorting = 
# 412 "/ejdb/src/jbi/jbi_selection.c" 3 4
                    0
# 412 "/ejdb/src/jbi/jbi_selection.c"
                         ;
      return idx;
    }
  }
...
iwrc jbi_selection(JBEXEC *ctx) {
  ...
  if (aux->orderby_num) {
    ctx->sorting = 
# 430 "/ejdb/src/jbi/jbi_selection.c" 3 4
                  1
# 430 "/ejdb/src/jbi/jbi_selection.c"
                      ;
  } else if (aux->qmode & ((jqp_query_mode_t) 0x08U)) {
    ctx->cursor_init = IWKV_CURSOR_AFTER_LAST;
    ctx->cursor_step = IWKV_CURSOR_PREV;
  }

  if (!(aux->qmode & ((jqp_query_mode_t) 0x02U)) && ctx->jbc->idx) {
    ...
    if (snp) {
      ...
      if ((op == JQP_OP_EQ) || (op == JQP_OP_IN) || ((op == JQP_OP_GTE) && (ctx->cursor_init == IWKV_CURSOR_GE))) {
        midx->expr1->prematched = 
# 445 "/ejdb/src/jbi/jbi_selection.c" 3 4
                                 1
# 445 "/ejdb/src/jbi/jbi_selection.c"
                                     ;
      }
      if (ctx->ux->log) {
      ...
      if (midx->orderby_support && (aux->orderby_num == 1)) {
        ...
        ctx->sorting = 
# 453 "/ejdb/src/jbi/jbi_selection.c" 3 4
                      0
# 453 "/ejdb/src/jbi/jbi_selection.c"
                           ;
      } else if (aux->orderby_num) {
        ctx->sorting = 
# 455 "/ejdb/src/jbi/jbi_selection.c" 3 4
                      1
# 455 "/ejdb/src/jbi/jbi_selection.c"
                          ;
      }
    } else if (ctx->sorting) {
