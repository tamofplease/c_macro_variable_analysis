...
static int __user_read_task(struct task *task, addr_t addr, void *buf, size_t count) {
    ...
    addr_t p = addr;
    while (p < addr + count) {
        addr_t chunk_end = (((p) >> 12) + 1) << 12;
        if (chunk_end > addr + count)
            chunk_end = addr + count;
        const char *ptr = mem_ptr(task->mem, p, 0);
        if (ptr == 
# 12 "../kernel/user.c" 3 4
                  ((void *)0)
# 12 "../kernel/user.c"
                      )
            return 1;
...
# 20 "../kernel/user.c"
    ...
    while (p < addr + count) {
        ...
        if (chunk_end > addr + count)
            chunk_end = addr + count;
        char *ptr = mem_ptr(task->mem, p, ptrace ? 2 : 1);
        if (ptr == 
# 28 "../kernel/user.c" 3 4
                  ((void *)0)
# 28 "../kernel/user.c"
                      )
            return 1;
