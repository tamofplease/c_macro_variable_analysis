...
static ssize_t eventfd_read(struct fd *fd, void *buf, size_t bufsize) {
    ...
    __lock(&fd->lock, "../kernel/eventfd.c", 26);
...
static ssize_t eventfd_write(struct fd *fd, const void *buf, size_t bufsize) {
    ...
    if (increment == 
# 50 "../kernel/eventfd.c" 3 4
                    (18446744073709551615UL)
# 50 "../kernel/eventfd.c"
                              )
        return -22;

    __lock(&fd->lock, "../kernel/eventfd.c", 53);
    while (fd->eventfd.val >= 
# 54 "../kernel/eventfd.c" 3 4
                             (18446744073709551615UL) 
# 54 "../kernel/eventfd.c"
                                        - increment) {
        if (fd->flags & (1 << 11)) {
...
static int eventfd_poll(struct fd *fd) {
    __lock(&fd->lock, "../kernel/eventfd.c", 73);
    int types = 0;
    if (fd->eventfd.val > 0)
        types |= 1;
    if (fd->eventfd.val < 
# 77 "../kernel/eventfd.c" 3 4
                         (18446744073709551615UL) 
# 77 "../kernel/eventfd.c"
                                    - 1)
        types |= 4;
