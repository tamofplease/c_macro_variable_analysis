...
static int clockid_to_real(uint_t clock, clockid_t *real) {
    switch (clock) {
        ...
        case 5:
            *real = 
# 19 "../kernel/time.c" 3 4
                   0
# 19 "../kernel/time.c"
                                 ; break;
        case 1: *real = 
# 20 "../kernel/time.c" 3 4
                                      1
# 20 "../kernel/time.c"
                                                     ; break;
        default: return -22;
...
int_t sys_setitimer(int_t which, addr_t new_val_addr, addr_t old_val_addr) {
    ...
    __lock(&group->lock, "../kernel/time.c", 142);
...
uint_t sys_alarm(uint_t seconds) {
    ...
    struct tgroup *group = current->group;
    __lock(&group->lock, "../kernel/time.c", 169);
...
int_t sys_timer_settime(dword_t timer_id, int_t flags, addr_t new_value_addr, addr_t old_value_addr) {
    ...
    __lock(&current->group->lock, "../kernel/time.c", 313);
...
int_t sys_timer_delete(dword_t timer_id) {
    __use(0, timer_id);
    __lock(&current->group->lock, "../kernel/time.c", 336);
    struct posix_timer *timer = &current->group->posix_timers[timer_id];
    if (timer->timer == 
# 338 "../kernel/time.c" 3 4
                       ((void *)0)
# 338 "../kernel/time.c"
                           ) {
        unlock(&current->group->lock);
...
static void timerfd_callback(struct fd *fd) {
    __lock(&fd->lock, "../kernel/time.c", 351);
...
# 376 "../kernel/time.c"
    ...
    __lock(&fd->lock, "../kernel/time.c", 390);
...
static ssize_t timerfd_read(struct fd *fd, void *buf, size_t bufsize) {
    if (bufsize < sizeof(uint64_t))
        return -22;
    __lock(&fd->lock, "../kernel/time.c", 408);
...
static int timerfd_poll(struct fd *fd) {
    int res = 0;
    __lock(&fd->lock, "../kernel/time.c", 428);
