...
static int load_entry(struct prg_header ph, addr_t bias, struct fd *fd) {
    ...
    if (memsize > filesize) {
        ...
        if (tail_size != 0) {
            ...
            __write_wrlock(&current->mem->lock, "../kernel/exec.c", 113);
        }
...
static int elf_exec(struct fd *fd, const char *file, struct exec_args argv, struct exec_args envp) {
    ...
    char *interp_name = 
# 157 "../kernel/exec.c" 3 4
                       ((void *)0)
# 157 "../kernel/exec.c"
                           ;
    struct fd *interp_fd = 
# 158 "../kernel/exec.c" 3 4
                          ((void *)0)
# 158 "../kernel/exec.c"
                              ;
    struct elf_header interp_header;
    struct prg_header *interp_ph = 
# 160 "../kernel/exec.c" 3 4
                                  ((void *)0)
# 160 "../kernel/exec.c"
                                      ;
...
# 206 "../kernel/exec.c"
    __lock(&current->general_lock, "../kernel/exec.c", 206);
    mm_release(current->mm);
    task_set_mm(current, mm_new());
    unlock(&current->general_lock);
    __write_wrlock(&current->mem->lock, "../kernel/exec.c", 210);

    current->mm->exefile = fd_retain(fd);

    addr_t load_addr = 0;
    
# 215 "../kernel/exec.c" 3 4
   _Bool 
# 215 "../kernel/exec.c"
        load_addr_set = 
# 215 "../kernel/exec.c" 3 4
...
out_free_interp:
    if (interp_name != 
# 411 "../kernel/exec.c" 3 4
                      ((void *)0)
# 411 "../kernel/exec.c"
                          )
        free(interp_name);
    if (interp_fd != 
# 413 "../kernel/exec.c" 3 4
                    ((void *)0) 
# 413 "../kernel/exec.c"
                         && !((uintptr_t) (interp_fd) > (uintptr_t) -0xfff))
        fd_close(interp_fd);
    if (interp_ph != 
# 415 "../kernel/exec.c" 3 4
                    ((void *)0)
# 415 "../kernel/exec.c"
                        )
        free(interp_ph);
...
# 433 "../kernel/exec.c"
   "args_end[0] == '\\0'"
# 433 "../kernel/exec.c" 3 4
   , "../kernel/exec.c", 433, __extension__ __PRETTY_FUNCTION__); }))
# 433 "../kernel/exec.c"
                              ;
    args_end++;
...
# 592 "../kernel/exec.c"
    ...
    __lock(&current->general_lock, "../kernel/exec.c", 598);
    const char *basename = strrchr(file, '/');
    if (basename == 
# 600 "../kernel/exec.c" 3 4
                   ((void *)0)
# 600 "../kernel/exec.c"
                       )
        basename = file;
    ...
    __lock(&current->sighand->lock, "../kernel/exec.c", 614);
...
# 623 "../kernel/exec.c"
    ...
    if (current->ptrace.traced) {
        __lock(&pids_lock, "../kernel/exec.c", 627);
