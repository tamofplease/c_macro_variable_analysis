...
pid_t_ sys_getppid() {
    __use(0);
    pid_t_ ppid;
    __lock(&pids_lock, "../kernel/getset.c", 16);
    if (current->parent != 
# 17 "../kernel/getset.c" 3 4
                          ((void *)0)
# 17 "../kernel/getset.c"
                              )
...
dword_t sys_setresuid(uid_t_ ruid, uid_t_ euid, uid_t_ suid) {
    __use(0, ruid, euid, suid);
    if (!(current != 
# 57 "../kernel/getset.c" 3 4
        ((void *)0) 
# 57 "../kernel/getset.c"
        && current->euid == 0)) {
...
int_t sys_getresuid(addr_t ruid_addr, addr_t euid_addr, addr_t suid_addr) {
    __use(0, ruid_addr, euid_addr, suid_addr);
    if (user_write(ruid_addr, &(current->uid), sizeof(current->uid)))
        return -14;
    if (user_write(euid_addr, &(current->euid), sizeof(current->euid)))
        return -14;
    if (user_write(suid_addr, &(current->suid), sizeof(current->suid)))
        return -14;
    return 0;
}
...
dword_t sys_setresgid(uid_t_ rgid, uid_t_ egid, uid_t_ sgid) {
    __use(0, rgid, egid, sgid);
    if (!(current != 
# 122 "../kernel/getset.c" 3 4
        ((void *)0) 
# 122 "../kernel/getset.c"
        && current->euid == 0)) {
...
int_t sys_getresgid(addr_t rgid_addr, addr_t egid_addr, addr_t sgid_addr) {
    __use(0, rgid_addr, egid_addr, sgid_addr);
    if (user_write(rgid_addr, &(current->gid), sizeof(current->gid)))
        return -14;
    if (user_write(egid_addr, &(current->egid), sizeof(current->egid)))
        return -14;
    if (user_write(sgid_addr, &(current->sgid), sizeof(current->sgid)))
        return -14;
    return 0;
}
