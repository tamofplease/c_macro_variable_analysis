...
dword_t sys_access(addr_t path_addr, dword_t mode) {
    return sys_faccessat(-100, path_addr, mode, 0);
}
...
fd_t sys_open(addr_t path_addr, dword_t flags, mode_t_ mode) {
    return sys_openat(-100, path_addr, flags, mode);
}
...
dword_t sys_readlinkat(fd_t at_f, addr_t path_addr, addr_t buf_addr, dword_t bufsize) {
    ...
    if (user_read_string(path_addr, path, sizeof(path)))
        return -14;
    __use(0, at_f, path, buf_addr, bufsize);
    struct fd *at = at_fd(at_f);
    if (at == 
# 95 "../kernel/fs.c" 3 4
             ((void *)0)
...
dword_t sys_readlink(addr_t path_addr, addr_t buf_addr, dword_t bufsize) {
    return sys_readlinkat(-100, path_addr, buf_addr, bufsize);
}
...
dword_t sys_linkat(fd_t src_at_f, addr_t src_addr, fd_t dst_at_f, addr_t dst_addr) {
    ...
    if (user_read_string(dst_addr, dst, sizeof(dst)))
        return -14;
    __use(0, src_at_f, src, dst_at_f, dst);
...
dword_t sys_link(addr_t src_addr, addr_t dst_addr) {
    return sys_linkat(-100, src_addr, -100, dst_addr);
}
...
dword_t sys_unlink(addr_t path_addr) {
    return sys_unlinkat(-100, path_addr, 0);
}
...
dword_t sys_rename(addr_t src_addr, addr_t dst_addr) {
    return sys_renameat2(-100, src_addr, -100, dst_addr, 0);
}
...
dword_t sys_symlink(addr_t target_addr, addr_t link_addr) {
    return sys_symlinkat(target_addr, -100, link_addr);
}
...
dword_t sys_mknod(addr_t path_addr, mode_t_ mode, dev_t_ dev) {
    return sys_mknodat(-100, path_addr, mode, dev);
}
...
# 216 "../kernel/fs.c"
    ...
    if (res >= 0) {
        ...
        __use(0, print_size, buf);
    }
    return res;
}

dword_t sys_read(fd_t fd_no, addr_t buf_addr, dword_t size) {
    __use(0, fd_no, buf_addr, size);
...
# 275 "../kernel/fs.c"
    ...
    if (print_size > 100) print_size = 100;
    __use(0, fd_no, print_size, buf, size);
...
dword_t sys_readv(fd_t fd_no, addr_t iovec_addr, dword_t iovec_count) {
    __use(0, fd_no, iovec_addr, iovec_count);
    struct iovec_ *iovec = read_iovec(iovec_addr, iovec_count);
    if (((uintptr_t) (iovec) > (uintptr_t) -0xfff))
        return (intptr_t) (iovec);
...
# 324 "../kernel/fs.c"
    ...
    for (unsigned i = 0; i < iovec_count; i++) {
        ...
        if (print_size > 100) print_size = 100;
        __use(0, print_size, buf + offset, iovec[i].len);
...
# 386 "../kernel/fs.c"
    ...
    if (!fd->ops->lseek)
        return -29;
    __lock(&fd->lock, "../kernel/fs.c", 390);
    off_t_ off = ((qword_t) off_high << 32) | off_low;
    __use(0, f, off, res_addr, whence);
    off_t_ res = fd->ops->lseek(fd, off, whence);
    __use(0, res);
    unlock(&fd->lock);
    if (res < 0)
        return res;
    if (user_write(res_addr, &(res), sizeof(res)))
        return -14;
    return 0;
}
...
# 405 "../kernel/fs.c"
    ...
    if (!fd->ops->lseek)
        return -29;
    __lock(&fd->lock, "../kernel/fs.c", 409);
...
# 423 "../kernel/fs.c"
        ...
        return -12;
    __lock(&fd->lock, "../kernel/fs.c", 425);
    ssize_t res;
    if (fd->ops->pread) {
        res = fd->ops->pread(fd, buf, size, off);
    } else {
        off_t_ saved_off = fd->ops->lseek(fd, 0, 1);
        if ((res = fd->ops->lseek(fd, off, 0)) < 0) {
            goto out;
        }
        res = fd->ops->read(fd, buf, size);
        ...
        off_t_ lseek_res = fd->ops->lseek(fd, saved_off, 0);
...
# 440 "../kernel/fs.c"
       "lseek_res >= 0"
# 440 "../kernel/fs.c" 3 4
       , "../kernel/fs.c", 440, __extension__ __PRETTY_FUNCTION__); }))
# 440 "../kernel/fs.c"
                             ;
    }
    if (res >= 0) {
...
# 460 "../kernel/fs.c"
    ...
    if (user_read(buf_addr, buf, size))
        return -14;
    __lock(&fd->lock, "../kernel/fs.c", 464);
    ssize_t res;
    if (fd->ops->pwrite) {
...
# 477 "../kernel/fs.c"
           "lseek_res >= 0"
# 477 "../kernel/fs.c" 3 4
           , "../kernel/fs.c", 477, __extension__ __PRETTY_FUNCTION__); }))
# 477 "../kernel/fs.c"
                                 ;
        }
    }
...
static int set_nonblock(struct fd *fd, addr_t nb_addr) {
    dword_t nonblock;
    if (user_read(nb_addr, &(nonblock), sizeof(nonblock)))
        return -14;
...
dword_t sys_getcwd(addr_t buf_addr, dword_t size) {
    __use(0, buf_addr, size);
    __lock(&current->fs->lock, "../kernel/fs.c", 539);
...
static struct fd *open_dir(const char *path) {
    struct statbuf stat;
    int err = generic_statat((struct fd *) -2, path, &stat, 
# 564 "../kernel/fs.c" 3 4
                                                 1
# 564 "../kernel/fs.c"
                                                     );
...
# 567 "../kernel/fs.c"
    ...
    return generic_open(path, 0, 0);
}

void fs_chdir(struct fs_info *fs, struct fd *fd) {
    __lock(&fs->lock, "../kernel/fs.c", 574);
    fd_close(fs->pwd);
    fs->pwd = fd;
    unlock(&fs->lock);
}
...
dword_t sys_chdir(addr_t path_addr) {
    ...
    if (((uintptr_t) (dir) > (uintptr_t) -0xfff))
        return (intptr_t) (dir);
...
dword_t sys_chroot(addr_t path_addr) {
    ...
    if (((uintptr_t) (dir) > (uintptr_t) -0xfff))
        return (intptr_t) (dir);
    __lock(&current->fs->lock, "../kernel/fs.c", 612);
...
dword_t sys_umask(dword_t mask) {
    __use(0, mask);
    struct fs_info *fs = current->fs;
    __lock(&fs->lock, "../kernel/fs.c", 622);
...
static int_t statfs_mount(struct mount *mount, addr_t buf_addr) {
    ...
    if (user_write(buf_addr, &(out_buf), sizeof(out_buf)))
        return -14;
    return 0;
}
...
dword_t sys_statfs(addr_t path_addr, addr_t buf_addr) {
    ...
    if (user_read_string(path_addr, path_raw, sizeof(path_raw)))
        return -14;
    __use(0, path_raw, buf_addr);
    char path[4096];
    int err = path_normalize((struct fd *) -2, path_raw, path, 2);
    if (err < 0)
        return err;
...
dword_t sys_statfs64(addr_t path_addr, dword_t buf_size, addr_t buf_addr) {
    ...
    if (user_read_string(path_addr, path_raw, sizeof(path_raw)))
        return -14;
    __use(0, path_raw, buf_size, buf_addr);
...
static dword_t sys_utime_common(fd_t at_f, addr_t path_addr, struct timespec atime, struct timespec mtime, dword_t flags) {
    ...
    if (path_addr != 0)
        if (user_read_string(path_addr, path, sizeof(path)))
            return -14;
    __use(0, at_f, path, atime.tv_sec, atime.tv_nsec, mtime.tv_sec, mtime.tv_nsec, flags)
                                                                            ;
    struct fd *at = at_fd(at_f);
    if (at == 
# 757 "../kernel/fs.c" 3 4
...
# 768 "../kernel/fs.c"
        ...
        if (user_read(times_addr, &(times), sizeof(times)))
            return -14;
...
# 783 "../kernel/fs.c"
    ...
    return sys_utime_common(-100, path_addr, atime, mtime, 0);
}
...
# 825 "../kernel/fs.c"
                  ;
    return generic_fsetattr(fd, ((struct attr) {.type = attr_mode, .mode = mode}));
}
...
# 837 "../kernel/fs.c"
                  ;
    return generic_setattrat(at, path, ((struct attr) {.type = attr_mode, .mode = mode}), 
# 838 "../kernel/fs.c" 3 4
                                                             1
# 838 "../kernel/fs.c"
                                                                 );
...
dword_t sys_chmod(addr_t path_addr, dword_t mode) {
    return sys_fchmodat(-100, path_addr, mode);
}

dword_t sys_fchown32(fd_t f, uid_t_ owner, uid_t_ group) {
    __use(0, f, owner, group);
    struct fd *fd = f_get(f);
    if (fd == 
# 848 "../kernel/fs.c" 3 4
             ((void *)0)
# 848 "../kernel/fs.c"
    ...
    int err;
    if (owner != (uid_t) -1) {
        err = generic_fsetattr(fd, ((struct attr) {.type = attr_uid, .uid = owner}));
        if (err < 0)
            return err;
    }
    if (group != (uid_t) -1) {
        err = generic_fsetattr(fd, ((struct attr) {.type = attr_gid, .gid = group}));
        if (err < 0)
            return err;
    }
    return 0;
...
dword_t sys_fchownat(fd_t at_f, addr_t path_addr, dword_t owner, dword_t group, int flags) {
    ...
    if (user_read_string(path_addr, path, sizeof(path)))
        return -14;
    __use(0, at_f, path, owner, group, flags);
    struct fd *at = at_fd(at_f);
    if (at == 
# 870 "../kernel/fs.c" 3 4
             ((void *)0)
...
# 873 "../kernel/fs.c"
                                                                  ;
    if (owner != (uid_t) -1) {
        err = generic_setattrat(at, path, ((struct attr) {.type = attr_uid, .uid = owner}), follow_links);
        if (err < 0)
            return err;
    }
    if (group != (uid_t) -1) {
        err = generic_setattrat(at, path, ((struct attr) {.type = attr_gid, .gid = group}), follow_links);
        if (err < 0)
            return err;
    }
    return 0;
...
dword_t sys_chown32(addr_t path_addr, uid_t_ owner, uid_t_ group) {
    return sys_fchownat(-100, path_addr, owner, group, 0);
}

dword_t sys_lchown(addr_t path_addr, uid_t_ owner, uid_t_ group) {
    return sys_fchownat(-100, path_addr, owner, group, 0x100);
}
...
dword_t sys_truncate64(addr_t path_addr, dword_t size_low, dword_t size_high) {
    ...
    if (user_read_string(path_addr, path, sizeof(path)))
        return -14;
    return generic_setattrat(
# 900 "../kernel/fs.c" 3 4
                            ((void *)0)
# 900 "../kernel/fs.c"
                                , path, ((struct attr) {.type = attr_size, .size = size}), 
# 900 "../kernel/fs.c" 3 4
                                                               1
# 900 "../kernel/fs.c"
                                                                   );
...
# 906 "../kernel/fs.c"
        ...
        return -9;
    return generic_fsetattr(fd, ((struct attr) {.type = attr_size, .size = size}));
}

dword_t sys_fallocate(fd_t f, dword_t UNUSED_mode __attribute__((unused)), dword_t offset_low, dword_t offset_high, dword_t len_low, dword_t len_high) {
...
# 915 "../kernel/fs.c"
    ...
    if ((uint64_t) offset + (uint64_t) len > statbuf.size)
        return generic_fsetattr(fd, ((struct attr) {.type = attr_size, .size = offset + len}));
    return 0;
}
...
dword_t sys_mkdir(addr_t path_addr, mode_t_ mode) {
    return sys_mkdirat(-100, path_addr, mode);
}
...
dword_t sys_rmdir(addr_t path_addr) {
    ...
    if (user_read_string(path_addr, path, sizeof(path)))
        return -14;
    __use(0, path);
    return generic_rmdirat((struct fd *) -2, path);
}
...
dword_t sys_sendfile(fd_t UNUSED_out_fd __attribute__((unused)), fd_t UNUSED_in_fd __attribute__((unused)), addr_t UNUSED_offset_addr __attribute__((unused)), dword_t UNUSED_count __attribute__((unused))) {
    return -22;
}
dword_t sys_sendfile64(fd_t UNUSED_out_fd __attribute__((unused)), fd_t UNUSED_in_fd __attribute__((unused)), addr_t UNUSED_offset_addr __attribute__((unused)), dword_t UNUSED_count __attribute__((unused))) {
    return -22;
}
dword_t sys_splice(fd_t UNUSED_in_fd __attribute__((unused)), addr_t UNUSED_in_off_addr __attribute__((unused)), fd_t UNUSED_out_fd __attribute__((unused)), addr_t UNUSED_out_off_addr __attribute__((unused)), dword_t UNUSED_count __attribute__((unused)), dword_t UNUSED_flags __attribute__((unused))) {
    return -22;
}
dword_t sys_copy_file_range(fd_t UNUSED_in_fd __attribute__((unused)), addr_t UNUSED_in_off __attribute__((unused)), fd_t UNUSED_out_fd __attribute__((unused)),
        addr_t UNUSED_out_off __attribute__((unused)), dword_t UNUSED_len __attribute__((unused)), uint_t UNUSED_flags __attribute__((unused))) {
    return -1;
}

dword_t sys_xattr_stub(addr_t UNUSED_path_addr __attribute__((unused)), addr_t UNUSED_name_addr __attribute__((unused)),
        addr_t UNUSED_value_addr __attribute__((unused)), dword_t UNUSED_size __attribute__((unused)), dword_t UNUSED_flags __attribute__((unused))) {
    return -95;
}
