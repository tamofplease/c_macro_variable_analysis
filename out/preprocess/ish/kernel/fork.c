...
static struct tgroup *tgroup_copy(struct tgroup *old_group) {
    ...
    *group = *old_group;
    ...
    if (group->tty) {
        __lock(&group->tty->lock, "../kernel/fork.c", 43);
        group->tty->refcount++;
        unlock(&group->tty->lock);
    }
    group->itimer = 
# 47 "../kernel/fork.c" 3 4
                   ((void *)0)
# 47 "../kernel/fork.c"
                       ;
    group->doing_group_exit = 
# 48 "../kernel/fork.c" 3 4
                             0
# 48 "../kernel/fork.c"
                                  ;
...
# 92 "../kernel/fork.c"
    ...
    __lock(&pids_lock, "../kernel/fork.c", 97);
    __lock(&old_group->lock, "../kernel/fork.c", 98);
    if (!(flags & 0x00010000)) {
...
# 151 "../kernel/fork.c"
    ...
    if (err < 0) {
        ...
        __lock(&pids_lock, "../kernel/fork.c", 159);
        task_destroy(task);
        unlock(&pids_lock);
        return err;
    }
...
# 170 "../kernel/fork.c"
    ...
    if (flags & 0x00004000) {
        __lock(&vfork.lock, "../kernel/fork.c", 185);
        while (!vfork.done)

            wait_for_ignore_signals(&vfork.cond, &vfork.lock, 
# 188 "../kernel/fork.c" 3 4
                                                             ((void *)0)
# 188 "../kernel/fork.c"
                                                                 );
        unlock(&vfork.lock);
        __lock(&task->general_lock, "../kernel/fork.c", 190);
        task->vfork = 
# 191 "../kernel/fork.c" 3 4
                     ((void *)0)
# 191 "../kernel/fork.c"
                         ;
...
void vfork_notify(struct task *task) {
    __lock(&task->general_lock, "../kernel/fork.c", 208);
    if (task->vfork) {
        __lock(&task->vfork->lock, "../kernel/fork.c", 210);
        task->vfork->done = 
# 211 "../kernel/fork.c" 3 4
                           1
# 211 "../kernel/fork.c"
                               ;
