...
syscall_t syscall_table[] = {
    ...
    [383] = (syscall_t) syscall_silent_stub,
    [384] = (syscall_t) sys_arch_prctl,
    [439] = (syscall_t) syscall_silent_stub,
};
...
# 258 "../kernel/calls.c"
        ...
        void *ptr = mem_ptr(current->mem, cpu->segfault_addr, cpu->segfault_was_write ? 1 : 0);
        read_wrunlock(&current->mem->lock);
        if (ptr == 
# 275 "../kernel/calls.c" 3 4
                  ((void *)0)
# 275 "../kernel/calls.c"
                      ) {
        ...
        __lock(&pids_lock, "../kernel/calls.c", 298);
        ...
        __lock(&pids_lock, "../kernel/calls.c", 305);
        send_signal(current, 5, (struct siginfo_) {
            .sig = 5,
            .code = 2,
        });
    ...
    __lock(&group->lock, "../kernel/calls.c", 318);
    while (group->stopped)
        wait_for_ignore_signals(&group->stopped_cond, &group->lock, 
# 320 "../kernel/calls.c" 3 4
                                                                   ((void *)0)
# 320 "../kernel/calls.c"
                                                                       );
    unlock(&group->lock);
