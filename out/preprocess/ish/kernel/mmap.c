...
struct mm *mm_new() {
    ...
    if (mm == 
# 12 "../kernel/mmap.c" 3 4
             ((void *)0)
# 12 "../kernel/mmap.c"
                 )
        return 
# 13 "../kernel/mmap.c" 3 4
              ((void *)0)
# 13 "../kernel/mmap.c"
                  ;
    mem_init(&mm->mem);
...
struct mm *mm_copy(struct mm *mm) {
    ...
    if (new_mm == 
# 23 "../kernel/mmap.c" 3 4
                 ((void *)0)
# 23 "../kernel/mmap.c"
                     )
        return 
# 24 "../kernel/mmap.c" 3 4
              ((void *)0)
# 24 "../kernel/mmap.c"
                  ;
    *new_mm = *mm;
    ...
    __write_wrlock(&mm->mem.lock, "../kernel/mmap.c", 31);
...
static addr_t do_mmap(addr_t addr, dword_t len, dword_t prot, dword_t flags, fd_t fd_no, dword_t offset) {
    ...
    if (flags & 0x20) {
        ...
        if (fd == 
# 78 "../kernel/mmap.c" 3 4
                 ((void *)0)
# 78 "../kernel/mmap.c"
                     )
            return -9;
        if (fd->ops->mmap == 
# 80 "../kernel/mmap.c" 3 4
                            ((void *)0)
# 80 "../kernel/mmap.c"
                                )
            return -19;
...
static addr_t mmap_common(addr_t addr, dword_t len, dword_t prot, dword_t flags, fd_t fd_no, dword_t offset) {
    ...
    __write_wrlock(&current->mem->lock, "../kernel/mmap.c", 99);
...
int_t sys_munmap(addr_t addr, uint_t len) {
    ...
    if (len == 0)
        return -22;
    __write_wrlock(&current->mem->lock, "../kernel/mmap.c", 126);
...
int_t sys_mprotect(addr_t addr, uint_t len, int_t prot) {
    ...
    if (prot & ~((1 << 0) | (1 << 1) | (1 << 2)))
        return -22;
    pages_t pages = ((((len) + (1 << 12) - 1) >> 12));
    __write_wrlock(&current->mem->lock, "../kernel/mmap.c", 188);
...
addr_t sys_brk(addr_t new_brk) {
    ...
    __write_wrlock(&mm->mem.lock, "../kernel/mmap.c", 216);
