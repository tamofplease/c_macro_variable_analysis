...
dword_t sys_setpgid(pid_t_ id, pid_t_ pgid) {
    ...
    if (pgid == 0)
        pgid = id;
    __lock(&pids_lock, "../kernel/group.c", 13);
    struct pid *pid = pid_get(id);
    err = -3;
    if (pid == 
# 16 "../kernel/group.c" 3 4
              ((void *)0)
# 16 "../kernel/group.c"
                  )
        goto out;
...
pid_t_ sys_getpgid(pid_t_ pid) {
    __use(0, pid);
    __lock(&pids_lock, "../kernel/group.c", 64);
...
void task_leave_session(struct task *task) {
    ...
    if (group->tty) {
        __lock(&ttys_lock, "../kernel/group.c", 85);
        if (list_empty(&pid_get(group->sid)->session)) {
            __lock(&group->tty->lock, "../kernel/group.c", 87);
            group->tty->session = 0;
            unlock(&group->tty->lock);
        }
        tty_release(group->tty);
        group->tty = 
# 92 "../kernel/group.c" 3 4
                    ((void *)0)
# 92 "../kernel/group.c"
                        ;
        unlock(&ttys_lock);
...
pid_t_ task_setsid(struct task *task) {
    __lock(&pids_lock, "../kernel/group.c", 98);
    struct tgroup *group = task->group;
...
dword_t sys_getsid() {
    __use(0);
    __lock(&pids_lock, "../kernel/group.c", 126);
