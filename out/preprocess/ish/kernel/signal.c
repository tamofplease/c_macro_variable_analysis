...
# 163 "../kernel/signal.h"
   "sig >= 1 && sig < NUM_SIGS"
# 163 "../kernel/signal.h" 3 4
   , "../kernel/signal.h", 163, __extension__ __PRETTY_FUNCTION__); }))
# 163 "../kernel/signal.h"
                                     ;
    return 1l << (sig - 1);
}
...
# 55 "../kernel/task.h" 3 4
       _Bool 
# 55 "../kernel/task.h"
            traced;
        
# 56 "../kernel/task.h" 3 4
       _Bool 
# 56 "../kernel/task.h"
            stopped;
        int signal;
...
# 72 "../kernel/task.h" 3 4
   _Bool 
# 72 "../kernel/task.h"
        zombie;
    
# 73 "../kernel/task.h" 3 4
   _Bool 
# 73 "../kernel/task.h"
        exiting;
...
# 26 "/usr/include/aarch64-linux-gnu/bits/types/struct_iovec.h" 3 4
struct iovec
  {
    void *iov_base;
    size_t iov_len;
...
int __attribute__((warn_unused_result)) user_write_task_ptrace(struct task *task, addr_t addr, const void *buf, size_t count);
...
# 38 "../kernel/calls.h" 3 4
_Noreturn 
# 38 "../kernel/calls.h"
        void do_exit(int status);

# 39 "../kernel/calls.h" 3 4
_Noreturn 
# 39 "../kernel/calls.h"
...
# 1 "../tools/ptraceomatic-config.h" 1
# 4 "../kernel/vdso.h" 2
...
static 
# 18 "../kernel/signal.c" 3 4
      _Bool 
# 18 "../kernel/signal.c"
           is_on_altstack(dword_t sp, struct sighand *sighand);
...
retry:
        __lock(&task->waiting_cond_lock, "../kernel/signal.c", 74);
        if (task->waiting_cond != 
# 75 "../kernel/signal.c" 3 4
                                 ((void *)0)
# 75 "../kernel/signal.c"
                                     ) {
            
# 76 "../kernel/signal.c" 3 4
           _Bool 
# 76 "../kernel/signal.c"
                mine = 
# 76 "../kernel/signal.c" 3 4
...
# 76 "../kernel/signal.c"
                            ;
            if (trylock(task->waiting_lock, "../kernel/signal.c", 77) == 
# 77 "../kernel/signal.c" 3 4
                                              16
# 77 "../kernel/signal.c"
                                                   ) {
                if (pthread_equal(task->waiting_lock->owner, pthread_self()))
                    mine = 
# 79 "../kernel/signal.c" 3 4
                          1
# 79 "../kernel/signal.c"
                              ;
                if (!mine) {
        ...
        unlock(&task->waiting_cond_lock);
        __lock(&task->sighand->lock, "../kernel/signal.c", 90);
    }
}

void deliver_signal(struct task *task, int sig, struct siginfo_ info) {
    __lock(&task->sighand->lock, "../kernel/signal.c", 95);
...
void send_signal(struct task *task, int sig, struct siginfo_ info) {
    ...
    __lock(&sighand->lock, "../kernel/signal.c", 108);
    if (signal_action(sighand, sig) != 0) {
        deliver_signal_unlocked(task, sig, info);
    }
    unlock(&sighand->lock);

    if (sig == 18 || sig == 9) {
        __lock(&task->group->lock, "../kernel/signal.c", 115);
        task->group->stopped = 
# 116 "../kernel/signal.c" 3 4
                              0
# 116 "../kernel/signal.c"
                                   ;
...
# 122 "../kernel/signal.c" 3 4
_Bool 
# 122 "../kernel/signal.c"
    try_self_signal(int sig) {
    
# 123 "../kernel/signal.c" 3 4
   ((void) sizeof ((
# 123 "../kernel/signal.c"
   sig == 21 || sig == 22
# 123 "../kernel/signal.c" 3 4
...
# 123 "../kernel/signal.c"
   "sig == SIGTTIN_ || sig == SIGTTOU_"
# 123 "../kernel/signal.c" 3 4
   , "../kernel/signal.c", 123, __extension__ __PRETTY_FUNCTION__); }))
# 123 "../kernel/signal.c"
                                             ;

    struct sighand *sighand = current->sighand;
    __lock(&sighand->lock, "../kernel/signal.c", 126);
    
# 127 "../kernel/signal.c" 3 4
   _Bool 
# 127 "../kernel/signal.c"
        can_send = signal_action(sighand, sig) != 0 &&
...
int send_group_signal(dword_t pgid, int sig, struct siginfo_ info) {
    __lock(&pids_lock, "../kernel/signal.c", 136);
    struct pid *pid = pid_get(pgid);
    if (pid == 
# 138 "../kernel/signal.c" 3 4
              ((void *)0)
# 138 "../kernel/signal.c"
                  ) {
        unlock(&pids_lock);
    ...
    for (tgroup = ((typeof(*tgroup) *) ((char *) ((&pid->pgroup)->next) - 
# 143 "../kernel/signal.c" 3 4
   __builtin_offsetof (
# 143 "../kernel/signal.c"
   typeof(*tgroup)
# 143 "../kernel/signal.c" 3 4
...
# 143 "../kernel/signal.c"
   )); &tgroup->pgroup != (&pid->pgroup); tgroup = ((typeof(*tgroup) *) ((char *) (tgroup->pgroup.next) - 
# 143 "../kernel/signal.c" 3 4
   __builtin_offsetof (
# 143 "../kernel/signal.c"
   typeof(*tgroup)
...
static void receive_signal(struct sighand *sighand, struct siginfo_ *info) {
    ...
    switch (signal_action(sighand, sig)) {
        ...
        case 3:
            __lock(&current->group->lock, "../kernel/signal.c", 227);
            current->group->stopped = 
# 228 "../kernel/signal.c" 3 4
                                     1
# 228 "../kernel/signal.c"
                                         ;
...
# 239 "../kernel/signal.c" 3 4
   _Bool 
# 239 "../kernel/signal.c"
        need_siginfo = action->flags & 4;
    ...
    if (!(action->flags & 0x40000000))
        sigset_add(&current->blocked, info->sig);
    current->blocked |= action->mask;


    if (need_siginfo) {
        frame.rt_sigframe.pinfo = sp + 
# 284 "../kernel/signal.c" 3 4
                                      __builtin_offsetof (
# 284 "../kernel/signal.c"
                                      struct rt_sigframe_
# 284 "../kernel/signal.c" 3 4
...
# 284 "../kernel/signal.c"
                                                                         ;
        frame.rt_sigframe.puc = sp + 
# 285 "../kernel/signal.c" 3 4
                                    __builtin_offsetof (
# 285 "../kernel/signal.c"
                                    struct rt_sigframe_
# 285 "../kernel/signal.c" 3 4
...
# 285 "../kernel/signal.c"
    ...
    if (user_write(sp, &frame, frame_size)) {
        ish_printk("failed to install frame for %d at %#x\n", info->sig, sp);
        deliver_signal(current, 11, SIGINFO_NIL);
    }

    if (action->flags & 0x80000000)
        *action = (struct sigaction_) {.handler = 0};
}

void signal_delivery_stop(int sig, struct siginfo_ *info) {
    __lock(&current->ptrace.lock, "../kernel/signal.c", 301);
    current->ptrace.stopped = 
# 302 "../kernel/signal.c" 3 4
                             1
# 302 "../kernel/signal.c"
                                 ;
    ...
    __lock(&current->ptrace.lock, "../kernel/signal.c", 311);
    while (current->ptrace.stopped) {
        wait_for_ignore_signals(&current->ptrace.cond, &current->ptrace.lock, 
# 313 "../kernel/signal.c" 3 4
                                                                             ((void *)0)
# 313 "../kernel/signal.c"
                                                                                 );
        __lock(&current->sighand->lock, "../kernel/signal.c", 314);
        
# 315 "../kernel/signal.c" 3 4
       _Bool 
# 315 "../kernel/signal.c"
            got_sigkill = sigset_has(current->pending, 9);
        unlock(&current->sighand->lock);
        if (got_sigkill) {
            __use(0, current->pid);
            unlock(&current->ptrace.lock);
    ...
    unlock(&current->ptrace.lock);
    __lock(&current->sighand->lock, "../kernel/signal.c", 324);
}

void receive_signals() {
    __lock(&current->group->lock, "../kernel/signal.c", 328);
    
# 329 "../kernel/signal.c" 3 4
   _Bool 
# 329 "../kernel/signal.c"
        was_stopped = current->group->stopped;
    unlock(&current->group->lock);

    struct sighand *sighand = current->sighand;
    __lock(&sighand->lock, "../kernel/signal.c", 333);
    ...
    if (current->has_saved_mask) {
        ...
        current->has_saved_mask = 
# 342 "../kernel/signal.c" 3 4
                                 0
# 342 "../kernel/signal.c"
                                      ;
    ...
    for (sigqueue = ((typeof(*(sigqueue)) *) ((char *) ((&current->queue)->next) - 
# 347 "../kernel/signal.c" 3 4
   __builtin_offsetof (
# 347 "../kernel/signal.c"
   typeof(*(sigqueue))
# 347 "../kernel/signal.c" 3 4
...
# 347 "../kernel/signal.c"
   )), tmp = ((typeof(*(sigqueue)) *) ((char *) ((sigqueue)->queue.next) - 
# 347 "../kernel/signal.c" 3 4
   __builtin_offsetof (
# 347 "../kernel/signal.c"
   typeof(*(sigqueue))
...
# 347 "../kernel/signal.c"
   )); &sigqueue->queue != (&current->queue); sigqueue = tmp, tmp = ((typeof(*(sigqueue)) *) ((char *) ((sigqueue)->queue.next) - 
# 347 "../kernel/signal.c" 3 4
   __builtin_offsetof (
# 347 "../kernel/signal.c"
   typeof(*(sigqueue))
...
# 347 "../kernel/signal.c"
        ...
        if (current->ptrace.traced && sig != 9) {
    ...
    if (!was_stopped) {
        __lock(&current->group->lock, "../kernel/signal.c", 370);
        
# 371 "../kernel/signal.c" 3 4
       _Bool 
# 371 "../kernel/signal.c"
            now_stopped = current->group->stopped;
        unlock(&current->group->lock);
        if (now_stopped) {
            __lock(&pids_lock, "../kernel/signal.c", 374);
            notify(&current->parent->group->child_exit);
...
dword_t sys_rt_sigreturn() {
    ...
    if (user_read(cpu->esp - 
# 404 "../kernel/signal.c" 3 4
       __builtin_offsetof (
# 404 "../kernel/signal.c"
       struct rt_sigframe_
# 404 "../kernel/signal.c" 3 4
...
# 404 "../kernel/signal.c"
    ...
    __lock(&current->sighand->lock, "../kernel/signal.c", 410);

    if (!is_on_altstack(cpu->esp, current->sighand) &&
            frame.uc.stack.size >= 2048) {
...
# 426 "../kernel/signal.c"
    ...
    __lock(&current->sighand->lock, "../kernel/signal.c", 432);
...
struct sighand *sighand_new() {
    ...
    if (sighand == 
# 441 "../kernel/signal.c" 3 4
                  ((void *)0)
# 441 "../kernel/signal.c"
                      )
        return 
# 442 "../kernel/signal.c" 3 4
              ((void *)0)
# 442 "../kernel/signal.c"
                  ;
...
struct sighand *sighand_copy(struct sighand *sighand) {
    ...
    if (new_sighand == 
# 451 "../kernel/signal.c" 3 4
                      ((void *)0)
# 451 "../kernel/signal.c"
                          )
        return 
# 452 "../kernel/signal.c" 3 4
              ((void *)0)
# 452 "../kernel/signal.c"
                  ;
...
static int do_sigaction(int sig, const struct sigaction_ *action, struct sigaction_ *oldaction) {
    ...
    struct sighand *sighand = current->sighand;
    __lock(&sighand->lock, "../kernel/signal.c", 470);
    if (oldaction)
...
dword_t sys_rt_sigaction(dword_t signum, addr_t action_addr, addr_t oldaction_addr, dword_t sigset_size) {
    ...
    if (action_addr != 0)
        if (user_read(action_addr, &(action), sizeof(action)))
            return -14;
    __use(0, signum, action_addr, action.handler, action.flags, action.restorer, (unsigned long long) action.mask, oldaction_addr, sigset_size)

                                                                          ;

    int err = do_sigaction(signum,
            action_addr ? &action : 
# 491 "../kernel/signal.c" 3 4
                                   ((void *)0)
# 491 "../kernel/signal.c"
                                       ,
            oldaction_addr ? &oldaction : 
# 492 "../kernel/signal.c" 3 4
                                         ((void *)0)
# 492 "../kernel/signal.c"
                                             );
    if (err < 0)
    ...
    if (oldaction_addr != 0)
        if (user_write(oldaction_addr, &(oldaction), sizeof(oldaction)))
            return -14;
    return err;
}
...
void sigmask_set_temp(sigset_t_ mask) {
    __lock(&current->sighand->lock, "../kernel/signal.c", 517);
...
dword_t sys_rt_sigprocmask(dword_t how, addr_t set_addr, addr_t oldset_addr, dword_t size) {
    ...
    sigset_t_ set;
    if (set_addr != 0)
        if (user_read(set_addr, &(set), sizeof(set)))
            return -14;
    __use(0, how == 0 ? "SIG_BLOCK" : how == 1 ? "SIG_UNBLOCK" : how == 2 ? "SIG_SETMASK" : "??", set_addr != 0 ? (long long) set : -1, oldset_addr, size)
    ...
    if (oldset_addr != 0)
        if (user_write(oldset_addr, &(current->blocked), sizeof(current->blocked)))
            return -14;
    if (set_addr != 0) {
        struct sighand *sighand = current->sighand;
        __lock(&sighand->lock, "../kernel/signal.c", 553);
...
int_t sys_rt_sigpending(addr_t set_addr) {
    ...
    if (user_write(set_addr, &(pending), sizeof(pending)))
        return -14;
    return 0;
}

static 
# 571 "../kernel/signal.c" 3 4
      _Bool 
# 571 "../kernel/signal.c"
           is_on_altstack(dword_t sp, struct sighand *sighand) {
...
dword_t sys_sigaltstack(addr_t ss_addr, addr_t old_ss_addr) {
    __use(0, ss_addr, old_ss_addr);
    struct sighand *sighand = current->sighand;
    __lock(&sighand->lock, "../kernel/signal.c", 588);
    if (old_ss_addr != 0) {
        struct stack_t_ old_ss;
        altstack_to_user(sighand, &old_ss);
        if (user_write(old_ss_addr, &(old_ss), sizeof(old_ss))) {
            unlock(&sighand->lock);
            return -14;
        }
    }
    if (ss_addr != 0) {
        ...
        struct stack_t_ ss;
        if (user_read(ss_addr, &(ss), sizeof(ss))) {
            unlock(&sighand->lock);
            return -14;
        }
        if (ss.flags & 2) {
...
int_t sys_rt_sigsuspend(addr_t mask_addr, uint_t size) {
    if (size != sizeof(sigset_t_))
        return -22;
    sigset_t_ mask;
    if (user_read(mask_addr, &(mask), sizeof(mask)))
        return -14;
    __use(0, (long long) mask);

    __lock(&current->sighand->lock, "../kernel/signal.c", 628);
    sigmask_set_temp_unlocked(mask);
    while (wait_for(&current->pause, &current->sighand->lock, 
# 630 "../kernel/signal.c" 3 4
                                                             ((void *)0)
# 630 "../kernel/signal.c"
                                                                 ) != -4)
        continue;
...
int_t sys_pause() {
    __lock(&current->sighand->lock, "../kernel/signal.c", 638);
    while (wait_for(&current->pause, &current->sighand->lock, 
# 639 "../kernel/signal.c" 3 4
                                                             ((void *)0)
# 639 "../kernel/signal.c"
                                                                 ) != -4)
        continue;
...
int_t sys_rt_sigtimedwait(addr_t set_addr, addr_t info_addr, addr_t timeout_addr, uint_t set_size) {
    ...
    if (timeout_addr != 0) {
        ...
        if (user_read(timeout_addr, &(fake_timeout), sizeof(fake_timeout)))
            return -14;
        timeout.tv_sec = fake_timeout.sec;
        timeout.tv_nsec = fake_timeout.nsec;
    }
    __use(0, (long long) set, info_addr, timeout_addr);

    __lock(&current->sighand->lock, "../kernel/signal.c", 661);
    
# 662 "../kernel/signal.c" 3 4
   ((void) sizeof ((
# 662 "../kernel/signal.c"
   current->waiting == 0
# 662 "../kernel/signal.c" 3 4
...
# 662 "../kernel/signal.c"
   "current->waiting == 0"
# 662 "../kernel/signal.c" 3 4
   , "../kernel/signal.c", 662, __extension__ __PRETTY_FUNCTION__); }))
# 662 "../kernel/signal.c"
                                ;
    current->waiting = set;
    int err;
    do {
        err = wait_for(&current->pause, &current->sighand->lock, timeout_addr == 0 ? 
# 666 "../kernel/signal.c" 3 4
                                                                                    ((void *)0) 
# 666 "../kernel/signal.c"
                                                                                         : &timeout);
    } while (err == 0);
...
# 677 "../kernel/signal.c"
   )); &sigqueue->queue != (&current->queue); sigqueue = ((typeof(*sigqueue) *) ((char *) (sigqueue->queue.next) - 
# 677 "../kernel/signal.c" 3 4
   __builtin_offsetof (
# 677 "../kernel/signal.c"
   typeof(*sigqueue)
...
static int kill_everything(dword_t sig) {
    ...
    for (int i = 2; i < (1 << 15); i++) {
        ...
        if (task == 
# 733 "../kernel/signal.c" 3 4
                   ((void *)0) 
# 733 "../kernel/signal.c"
                        || task == current || !task_is_leader(task))
            continue;
...
static int do_kill(pid_t_ pid, dword_t sig, pid_t_ tgid) {
    ...
    int err;
    __lock(&pids_lock, "../kernel/signal.c", 750);
