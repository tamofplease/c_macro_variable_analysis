...
void mem_destroy(struct mem *mem) {
    __write_wrlock(&mem->lock, "../emu/memory.c", 35);
...
# 102 "../emu/memory.c" 3 4
_Bool 
# 102 "../emu/memory.c"
    pt_is_hole(struct mem *mem, page_t start, pages_t pages) {
    for (page_t page = start; page < start + pages; page++) {
        if (mem_pt(mem, page) != 
# 104 "../emu/memory.c" 3 4
                                ((void *)0)
# 104 "../emu/memory.c"
                                    )
            return 
# 105 "../emu/memory.c" 3 4
                  0
# 105 "../emu/memory.c"
                       ;
    }
    return 
# 107 "../emu/memory.c" 3 4
          1
# 107 "../emu/memory.c"
              ;
}
...
# 115 "../emu/memory.c"
   "(uintptr_t) memory % real_page_size == 0 || memory == vdso_data"
# 115 "../emu/memory.c" 3 4
   , "../emu/memory.c", 115, __extension__ __PRETTY_FUNCTION__); }))
# 115 "../emu/memory.c"
                                                                          ;

    struct data *data = malloc(sizeof(struct data));
    if (data == 
# 118 "../emu/memory.c" 3 4
               ((void *)0)
# 118 "../emu/memory.c"
                   )
        return -12;
...
# 164 "../emu/memory.c" 3 4
                                                                                      (*__errno_location ())
# 164 "../emu/memory.c"
                                                                                           ));
            }
            if (data->fd != 
# 166 "../emu/memory.c" 3 4
                           ((void *)0)
# 166 "../emu/memory.c"
                               ) {
                fd_close(data->fd);
...
int pt_map_nothing(struct mem *mem, page_t start, pages_t pages, unsigned flags) {
    if (pages == 0) return 0;
    void *memory = mmap(
# 178 "../emu/memory.c" 3 4
                       ((void *)0)
# 178 "../emu/memory.c"
                           , pages * (1 << 12),
            
# 179 "../emu/memory.c" 3 4
           0x1 
# 179 "../emu/memory.c"
                     | 
# 179 "../emu/memory.c" 3 4
...
# 185 "../emu/memory.c"
    ...
    for (page_t page = start; page < start + pages; page++) {
        ...
        if ((flags & ~old_flags) & ((1 << 0)|(1 << 1))) {
            ...
            int prot = 
# 196 "../emu/memory.c" 3 4
                      0x1
# 196 "../emu/memory.c"
                               ;
            if (flags & (1 << 1)) prot |= 
# 197 "../emu/memory.c" 3 4
                                        0x2
# 197 "../emu/memory.c"
                                                  ;
...
static void *mem_ptr_nofault(struct mem *mem, addr_t addr, int type) {
    struct pt_entry *entry = mem_pt(mem, ((addr) >> 12));
    if (entry == 
# 234 "../emu/memory.c" 3 4
                ((void *)0)
# 234 "../emu/memory.c"
                    )
        return 
# 235 "../emu/memory.c" 3 4
              ((void *)0)
# 235 "../emu/memory.c"
                  ;
    if (type == 1 && !(entry->flags & (1 << 1) && !(entry->flags & (1 << 4))))
        return 
# 237 "../emu/memory.c" 3 4
              ((void *)0)
# 237 "../emu/memory.c"
                  ;
...
void *mem_ptr(struct mem *mem, addr_t addr, int type) {
    void *old_ptr = mem_ptr_nofault(mem, addr, type);

    page_t page = ((addr) >> 12);
    struct pt_entry *entry = mem_pt(mem, page);

    if (entry == 
# 247 "../emu/memory.c" 3 4
                ((void *)0)
# 247 "../emu/memory.c"
                    ) {
...
# 251 "../emu/memory.c"
        ...
        if (p >= (1 << 20))
            return 
# 254 "../emu/memory.c" 3 4
                  ((void *)0)
# 254 "../emu/memory.c"
                      ;
        if (!(mem_pt(mem, p)->flags & (1 << 3)))
            return 
# 256 "../emu/memory.c" 3 4
                  ((void *)0)
# 256 "../emu/memory.c"
                      ;
        ...
        __write_wrlock(&mem->lock, "../emu/memory.c", 265);
...
# 289 "../emu/memory.c"
            ...
            read_wrunlock(&mem->lock);
            __write_wrlock(&mem->lock, "../emu/memory.c", 293);
            memcpy(copy, data, (1 << 12));
...
# 302 "../emu/memory.c"
   "old_ptr == NULL || old_ptr == ptr || type == MEM_WRITE_PTRACE"
# 302 "../emu/memory.c" 3 4
   , "../emu/memory.c", 302, __extension__ __PRETTY_FUNCTION__); }))
# 302 "../emu/memory.c"
                                                                        ;
    return ptr;
}
...
static void *mem_mmu_translate(struct mmu *mmu, addr_t addr, int type) {
    return mem_ptr_nofault(((struct mem *) ((char *) (mmu) - 
# 307 "../emu/memory.c" 3 4
                          __builtin_offsetof (
# 307 "../emu/memory.c"
                          struct mem
# 307 "../emu/memory.c" 3 4
