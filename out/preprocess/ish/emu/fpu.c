...
void fpu_stm80(struct cpu_state *cpu, float80 *f) {
    ...
    memcpy(f, &cpu->fp[(cpu->top + 0) % 8], 10);
}
# 101 "../emu/fpu.c"
void fpu_cmovb(struct cpu_state *cpu, int i) { if (cpu->cf) cpu->fp[(cpu->top + 0) % 8] = cpu->fp[(cpu->top + i) % 8]; }
void fpu_cmove(struct cpu_state *cpu, int i) { if (cpu->zf) cpu->fp[(cpu->top + 0) % 8] = cpu->fp[(cpu->top + i) % 8]; }
void fpu_cmovbe(struct cpu_state *cpu, int i) { if (cpu->cf | cpu->zf) cpu->fp[(cpu->top + 0) % 8] = cpu->fp[(cpu->top + i) % 8]; }
void fpu_cmovu(struct cpu_state *cpu, int i) { if (cpu->pf) cpu->fp[(cpu->top + 0) % 8] = cpu->fp[(cpu->top + i) % 8]; }
void fpu_cmovnb(struct cpu_state *cpu, int i) { if (!cpu->cf) cpu->fp[(cpu->top + 0) % 8] = cpu->fp[(cpu->top + i) % 8]; }
void fpu_cmovne(struct cpu_state *cpu, int i) { if (!cpu->zf) cpu->fp[(cpu->top + 0) % 8] = cpu->fp[(cpu->top + i) % 8]; }
void fpu_cmovnbe(struct cpu_state *cpu, int i) { if (!(cpu->cf | cpu->zf)) cpu->fp[(cpu->top + 0) % 8] = cpu->fp[(cpu->top + i) % 8]; }
void fpu_cmovnu(struct cpu_state *cpu, int i) { if (!cpu->pf) cpu->fp[(cpu->top + 0) % 8] = cpu->fp[(cpu->top + i) % 8]; }
...
void fpu_save32(struct cpu_state *cpu, struct fpu_state32 *state) {
    ...
    for (int i = 0; i < 8; i++)
        memcpy(state->regs[i], &cpu->fp[(cpu->top + i) % 8], 10);
}

void fpu_restore32(struct cpu_state *cpu, struct fpu_state32 *state) {
    fpu_ldenv32(cpu, &state->env);
    for (int i = 0; i < 8; i++)
        memcpy(&cpu->fp[(cpu->top + i) % 8], state->regs[i], 10);
}
