...
# 90 "/usr/include/netinet/tcp.h" 2 3 4
...
# 91 "/usr/include/netinet/tcp.h" 2 3 4
...
static uint32_t unix_socket_next_id() {
    ...
    static lock_t next_id_lock = {
# 83 "../fs/sock.c" 3 4
                                { { 0, 0, 0, 0, PTHREAD_MUTEX_TIMED_NP, 0, { 0, 0 } } }
# 83 "../fs/sock.c"
                                , 0};
    __lock(&next_id_lock, "../fs/sock.c", 84);
    uint32_t id = ++next_id;
    unlock(&next_id_lock);
    return id;
}
...
static int unix_socket_get(const char *path_raw, struct fd *bind_fd, uint32_t *socket_id) {
    char path[4096];
    int err = path_normalize((struct fd *) -2, path_raw, path, 1);
    if (err < 0)
        return err;
...
# 100 "../fs/sock.c"
        ...
        if (err < 0) {
            ...
            __lock(&fs->lock, "../fs/sock.c", 110);
            mode &= ~fs->umask;
            unlock(&fs->lock);
            err = mount->fs->mknod(mount, path, 
# 113 "../fs/sock.c" 3 4
                                               0140000 
# 113 "../fs/sock.c"
                                                        | mode, 0);
...
# 127 "../fs/sock.c"
    ...
    __lock(&inode->lock, "../fs/sock.c", 134);
...
static struct list abstract_hash[1024];
static lock_t unix_abstract_lock = {
# 173 "../fs/sock.c" 3 4
                                  { { 0, 0, 0, 0, PTHREAD_MUTEX_TIMED_NP, 0, { 0, 0 } } }
# 173 "../fs/sock.c"
                                  , 0};
...
static int unix_abstract_get(const char *name, struct fd *bind_fd, uint32_t *socket_id) {
    ...
    __lock(&unix_abstract_lock, "../fs/sock.c", 177);
    struct unix_abstract *sock_tmp;
    struct unix_abstract *sock = 
# 179 "../fs/sock.c" 3 4
                                ((void *)0)
# 179 "../fs/sock.c"
                                    ;
    struct list *bucket = &abstract_hash[hash % 1024];
    if (list_null(bucket))
        list_init(bucket);
    for (sock_tmp = ((typeof(*sock_tmp) *) ((char *) ((bucket)->next) - 
# 183 "../fs/sock.c" 3 4
   __builtin_offsetof (
# 183 "../fs/sock.c"
   typeof(*sock_tmp)
...
# 183 "../fs/sock.c"
   )); &sock_tmp->links != (bucket); sock_tmp = ((typeof(*sock_tmp) *) ((char *) (sock_tmp->links.next) - 
# 183 "../fs/sock.c" 3 4
   __builtin_offsetof (
# 183 "../fs/sock.c"
   typeof(*sock_tmp)
...
static void unix_abstract_release(struct unix_abstract *name) {
    __lock(&unix_abstract_lock, "../fs/sock.c", 216);
    if (--name->refcount == 0) {
...
# 292 "../fs/sock.c"
                                  ;
    int err = sockaddr_read_bind(sockaddr_addr, sockaddr, sockaddr_len, 
# 293 "../fs/sock.c" 3 4
                                                                       ((void *)0)
# 293 "../fs/sock.c"
                                                                           );
...
static int sockaddr_write(addr_t sockaddr_addr, void *sockaddr, uint_t buffer_len, uint_t *sockaddr_len) {
    ...
    switch (fake_addr->family) {
        case 1: {
            ...
            static struct sockaddr_ unix_domain_null = {.family = 1};
...
int_t sys_bind(fd_t sock_fd, addr_t sockaddr_addr, uint_t sockaddr_len) {
    __use(0, sock_fd, sockaddr_addr, sockaddr_len);
...
# 334 "../fs/sock.c"
    ...
    if (err < 0) {
        ...
        if (sock->socket.unix_name_abstract != 
# 342 "../fs/sock.c" 3 4
                                              ((void *)0)
# 342 "../fs/sock.c"
                                                  )
...
# 359 "../fs/sock.c"
    ...
    if (sock->socket.domain == 1) {
        fill_cred(&sock->socket.unix_cred);
        
# 372 "../fs/sock.c" 3 4
       ((void) sizeof ((
# 372 "../fs/sock.c"
       sock->socket.unix_peer == 
# 372 "../fs/sock.c" 3 4
...
# 372 "../fs/sock.c" 3 4
       , "../fs/sock.c", 372, __extension__ __PRETTY_FUNCTION__); }))
# 372 "../fs/sock.c"
                                             ;
        ...
        if (res == sizeof(struct fd *)) {
            ...
            __lock(&peer_lock, "../fs/sock.c", 377);
            while (sock->socket.unix_peer == 
# 378 "../fs/sock.c" 3 4
                                            ((void *)0)
# 378 "../fs/sock.c"
                                                )
                wait_for_ignore_signals(&sock->socket.unix_got_peer, &peer_lock, 
# 379 "../fs/sock.c" 3 4
                                                                                ((void *)0)
# 379 "../fs/sock.c"
                                                                                    );
...
int_t sys_accept(fd_t sock_fd, addr_t sockaddr_addr, addr_t sockaddr_len_addr) {
    __use(0, sock_fd, sockaddr_addr, sockaddr_len_addr);
    struct fd *sock = sock_getfd(sock_fd);
    if (sock == 
# 402 "../fs/sock.c" 3 4
               ((void *)0)
# 402 "../fs/sock.c"
    ...
    if (sockaddr_addr != 0) {
        if (user_read(sockaddr_len_addr, &(sockaddr_len), sizeof(sockaddr_len)))
            return -14;
    }
...
# 416 "../fs/sock.c"
                                                            ,
                sockaddr_addr != 0 ? &sockaddr_len : 
# 417 "../fs/sock.c" 3 4
                                                    ((void *)0)
# 417 "../fs/sock.c"
                                                        );
        sockrestart_end_listen_wait(sock);
    } while (sockrestart_should_restart_listen_wait() && 
# 419 "../fs/sock.c" 3 4
                                                        (*__errno_location ()) 
# 419 "../fs/sock.c"
                                                              == 
...
# 419 "../fs/sock.c"
    ...
    if (sockaddr_addr != 0) {
        ...
        if (err < 0)
            return client;
        if (user_write(sockaddr_len_addr, &(sockaddr_len), sizeof(sockaddr_len)))
            return -14;
    }
    ...
    if (sock->socket.domain == 1) {
        __lock(&peer_lock, "../fs/sock.c", 437);
...
int_t sys_socketpair(dword_t domain, dword_t type, dword_t protocol, addr_t sockets_addr) {
    __use(0, domain, type, protocol, sockets_addr);
    int real_domain = sock_family_to_real(domain);
    if (real_domain < 0)
        return -22;
    int real_type = sock_type_to_real(type, protocol);
    ...
    __lock(&peer_lock, "../fs/sock.c", 537);
    int fake_sockets[2];
    ...
    err = -14;
    if (user_write(sockets_addr, &(fake_sockets), sizeof(fake_sockets)))
        goto close_fake_1;

    __use(0, fake_sockets[0], fake_sockets[1]);
    return 0;
...
# 574 "../fs/sock.c"
    ...
    if (user_read(buffer_addr, buffer, len))
        return -14;
    buffer[len] = '\0';
    __use(0, sock_fd, buffer, len, flags, sockaddr_addr, sockaddr_len);
...
int_t sys_recvfrom(fd_t sock_fd, addr_t buffer_addr, dword_t len, dword_t flags, addr_t sockaddr_addr, addr_t sockaddr_len_addr) {
    __use(0, sock_fd, buffer_addr, len, flags, sockaddr_addr, sockaddr_len_addr);
    struct fd *sock = sock_getfd(sock_fd);
    if (sock == 
# 607 "../fs/sock.c" 3 4
               ((void *)0)
...
# 620 "../fs/sock.c"
                                                        ,
            sockaddr_len_addr != 0 ? &sockaddr_len : 
# 621 "../fs/sock.c" 3 4
                                                    ((void *)0)
# 621 "../fs/sock.c"
                                                        );
...
int_t sys_setsockopt(fd_t sock_fd, dword_t level, dword_t option, addr_t value_addr, dword_t value_len) {
    __use(0, sock_fd, level, option, value_addr, value_len);
    struct fd *sock = sock_getfd(sock_fd);
    if (sock == 
# 667 "../fs/sock.c" 3 4
               ((void *)0)
...
int_t sys_getsockopt(fd_t sock_fd, dword_t level, dword_t option, addr_t value_addr, dword_t len_addr) {
    __use(0, sock_fd, level, option, value_addr, len_addr);
    struct fd *sock = sock_getfd(sock_fd);
    if (sock == 
# 709 "../fs/sock.c" 3 4
               ((void *)0)
# 709 "../fs/sock.c"
        ...
        return -9;
    dword_t value_len;
    if (user_read(len_addr, &(value_len), sizeof(value_len)))
        return -14;
    char value[value_len];
    if (user_read(value_addr, value, value_len))
        return -14;

    if (level == 1 && (option == 39 || option == 3 || option == 38)) {
    ...
    } else if (level == 1 && option == 17) {
        struct ucred_ *cred = (struct ucred_ *) value;
        if (value_len != sizeof(*cred))
            return -22;
        __lock(&peer_lock, "../fs/sock.c", 732);
        if (sock->socket.domain != 1 || sock->socket.unix_peer == 
# 733 "../fs/sock.c" 3 4
                                                                         ((void *)0)
# 733 "../fs/sock.c"
                                                                             ) {
...
# 749 "../fs/sock.c"
                                   && option == 13) {
        value_len = strlen("cubic");
        memcpy(value, "cubic", value_len);
# 799 "../fs/sock.c"
    } else {
        int real_opt = sock_opt_to_real(option, level);
        if (real_opt < 0)
    ...
    if (user_write(len_addr, &(value_len), sizeof(value_len)))
        return -14;
    if (user_write(value_addr, &(value), sizeof(value)))
        return -14;
    return 0;
}
...
int_t sys_sendmsg(fd_t sock_fd, addr_t msghdr_addr, int_t flags) {
    int err;
    __use(0, sock_fd, msghdr_addr, flags);
    struct fd *sock = sock_getfd(sock_fd);
    if (sock == 
# 829 "../fs/sock.c" 3 4
               ((void *)0)
# 829 "../fs/sock.c"
    ...
    if (user_read(msghdr_addr, &(msg_fake), sizeof(msg_fake)))
        return -14;
...
# 846 "../fs/sock.c"
    ...
    if (user_read(msg_fake.msg_iov, &(msg_iov_fake), sizeof(msg_iov_fake)))
        return -14;
...
# 882 "../fs/sock.c"
    ...
    if (sock->socket.domain == 1 && msg_control != 
# 883 "../fs/sock.c" 3 4
                                                          ((void *)0) 
# 883 "../fs/sock.c"
        ...
        for (cmsg = (void *) msg_control; cmsg != 
# 888 "../fs/sock.c" 3 4
                                                 ((void *)0)
# 888 "../fs/sock.c"
                                                     ; cmsg = ((cmsg)->len < sizeof (struct cmsghdr_) || (((cmsg)->len + sizeof(dword_t) - 1) & ~(dword_t)(sizeof(dword_t) - 1)) + sizeof(struct cmsghdr_) >= (size_t) (mhdr_end - (uint8_t *)(cmsg)) ? 
# 888 "../fs/sock.c" 3 4
                                                              ((void *)0) 
# 888 "../fs/sock.c"
                                                              : (struct cmsghdr_ *)((uint8_t *)(cmsg) + (((cmsg)->len + sizeof(dword_t) - 1) & ~(dword_t)(sizeof(dword_t) - 1))))) {
            if (cmsg->level != 1)
                continue;
            if (cmsg->type != 1)
                return -22;
...
# 918 "../fs/sock.c"
            ...
            __lock(&peer_lock, "../fs/sock.c", 927);
...
# 929 "../fs/sock.c"
            ...
            __lock(&peer->lock, "../fs/sock.c", 934);
...
# 957 "../fs/sock.c"
        ...
        __lock(&peer_lock, "../fs/sock.c", 958);
        struct fd *peer = sock->socket.unix_peer;
        if (peer != 
# 960 "../fs/sock.c" 3 4
                   ((void *)0)
# 960 "../fs/sock.c"
            ...
            __lock(&peer->lock, "../fs/sock.c", 961);
...
# 1048 "../fs/sock.c"
        ...
        __lock(&sock->lock, "../fs/sock.c", 1051);
        
# 1052 "../fs/sock.c" 3 4
       ((void) sizeof ((
# 1052 "../fs/sock.c"
       !list_empty(&sock->socket.unix_scm)
# 1052 "../fs/sock.c" 3 4
       ) ? 1 : 0), __extension__ ({ if (
# 1052 "../fs/sock.c"
       !list_empty(&sock->socket.unix_scm)
...
# 1052 "../fs/sock.c"
       "!list_empty(&sock->socket.unix_scm)"
# 1052 "../fs/sock.c" 3 4
       , "../fs/sock.c", 1052, __extension__ __PRETTY_FUNCTION__); }))
# 1052 "../fs/sock.c"
                                                  ;
        struct scm *scm = ((struct scm *) ((char *) ((&sock->socket.unix_scm)->next) - 
# 1053 "../fs/sock.c" 3 4
                         __builtin_offsetof (
# 1053 "../fs/sock.c"
                         struct scm
...
# 1053 "../fs/sock.c"
    ...
    if (user_write(msghdr_addr, &(msg_fake), sizeof(msg_fake)))
        return -14;
    return res;
}
...
# 1108 "../fs/sock.c"
                                                                        ;
            if (user_write(msg_len_addr, &(res), sizeof(res)))
                res = -14;
        }
        if (res < 0) {
...
static void sock_translate_err(struct fd *fd, int *err) {
    ...
    if (*err == -107) {
        ...
        if (getpeername(fd->real_fd, &addr, &len) < 0 && 
# 1137 "../fs/sock.c" 3 4
                                                        (*__errno_location ()) 
# 1137 "../fs/sock.c"
                                                              == 
...
static int sock_close(struct fd *fd) {
    ...
    if (fd->socket.unix_name_abstract != 
# 1159 "../fs/sock.c" 3 4
                                        ((void *)0)
# 1159 "../fs/sock.c"
                                            )
        unix_abstract_release(fd->socket.unix_name_abstract);
    __lock(&peer_lock, "../fs/sock.c", 1161);
...
# 1163 "../fs/sock.c"
        ...
        peer->socket.unix_peer = 
# 1164 "../fs/sock.c" 3 4
                                ((void *)0)
# 1164 "../fs/sock.c"
                                    ;
    unlock(&peer_lock);
    if (fd->socket.domain == 1) {
        __lock(&fd->lock, "../fs/sock.c", 1167);
        struct scm *scm, *tmp;
        for (scm = ((typeof(*(scm)) *) ((char *) ((&fd->socket.unix_scm)->next) - 
# 1169 "../fs/sock.c" 3 4
       __builtin_offsetof (
# 1169 "../fs/sock.c"
       typeof(*(scm))
...
# 1169 "../fs/sock.c"
       )), tmp = ((typeof(*(scm)) *) ((char *) ((scm)->queue.next) - 
# 1169 "../fs/sock.c" 3 4
       __builtin_offsetof (
# 1169 "../fs/sock.c"
       typeof(*(scm))
...
# 1169 "../fs/sock.c"
       )); &scm->queue != (&fd->socket.unix_scm); scm = tmp, tmp = ((typeof(*(scm)) *) ((char *) ((scm)->queue.next) - 
# 1169 "../fs/sock.c" 3 4
       __builtin_offsetof (
# 1169 "../fs/sock.c"
       typeof(*(scm))
