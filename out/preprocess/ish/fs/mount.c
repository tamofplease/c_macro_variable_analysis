...
# 23 "../fs/mount.c"
   "!\"reached filesystem limit\""
# 23 "../fs/mount.c" 3 4
   , "../fs/mount.c", 23, __extension__ __PRETTY_FUNCTION__); }))
# 23 "../fs/mount.c"
                                      ;
}
...
# 27 "../fs/mount.c"
   "path_is_normalized(path)"
# 27 "../fs/mount.c" 3 4
   , "../fs/mount.c", 27, __extension__ __PRETTY_FUNCTION__); }))
# 27 "../fs/mount.c"
                                   ;
    __lock(&mounts_lock, "../fs/mount.c", 28);
    struct mount *mount = 
# 29 "../fs/mount.c" 3 4
                         ((void *)0)
# 29 "../fs/mount.c"
...
# 30 "../fs/mount.c"
   "!list_empty(&mounts)"
# 30 "../fs/mount.c" 3 4
   , "../fs/mount.c", 30, __extension__ __PRETTY_FUNCTION__); }))
# 30 "../fs/mount.c"
                               ;
...
void mount_retain(struct mount *mount) {
    __lock(&mounts_lock, "../fs/mount.c", 42);
    mount->refcount++;
    unlock(&mounts_lock);
}

void mount_release(struct mount *mount) {
    __lock(&mounts_lock, "../fs/mount.c", 48);
    mount->refcount--;
    unlock(&mounts_lock);
}
...
# 161 "../fs/mount.c"
    ...
    __lock(&mounts_lock, "../fs/mount.c", 169);
...
dword_t sys_umount2(addr_t target_addr, dword_t flags) {
    ...
    __lock(&mounts_lock, "../fs/mount.c", 187);
