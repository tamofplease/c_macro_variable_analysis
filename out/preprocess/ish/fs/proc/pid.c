...
static struct task *proc_get_task(struct proc_entry *entry) {
    __lock(&pids_lock, "../fs/proc/pid.c", 17);
    struct task *task = pid_get_task(entry->pid);
    if (task == 
# 19 "../fs/proc/pid.c" 3 4
               ((void *)0)
# 19 "../fs/proc/pid.c"
                   )
        unlock(&pids_lock);
...
static int proc_pid_stat_show(struct proc_entry *entry, struct proc_data *buf) {
    ...
    if (task == 
# 29 "../fs/proc/pid.c" 3 4
               ((void *)0)
# 29 "../fs/proc/pid.c"
                   )
        return -3;
    __lock(&task->general_lock, "../fs/proc/pid.c", 31);
    __lock(&task->group->lock, "../fs/proc/pid.c", 32);
    __lock(&task->sighand->lock, "../fs/proc/pid.c", 33);
...
static int proc_pid_auxv_show(struct proc_entry *entry, struct proc_data *buf) {
    struct task *task = proc_get_task(entry);
    if (task == 
# 108 "../fs/proc/pid.c" 3 4
               ((void *)0)
# 108 "../fs/proc/pid.c"
                   )
        return -3;
    int err = 0;
    __lock(&task->general_lock, "../fs/proc/pid.c", 111);
    if (task->mm == 
# 112 "../fs/proc/pid.c" 3 4
                   ((void *)0)
# 112 "../fs/proc/pid.c"
                       )
        goto out_free_task;
...
static int proc_pid_cmdline_show(struct proc_entry *entry, struct proc_data *buf) {
    struct task *task = proc_get_task(entry);
    if (task == 
# 133 "../fs/proc/pid.c" 3 4
               ((void *)0)
# 133 "../fs/proc/pid.c"
                   )
        return -3;
    int err = 0;
    __lock(&task->general_lock, "../fs/proc/pid.c", 136);
    if (task->mm == 
# 137 "../fs/proc/pid.c" 3 4
                   ((void *)0)
# 137 "../fs/proc/pid.c"
                       )
        goto out_free_task;
...
# 192 "../fs/proc/pid.c" 3 4
                                ((void *)0)
# 192 "../fs/proc/pid.c"
                                    ) {
            strcpy(path, data->name);
        } else if (data->fd != 
# 194 "../fs/proc/pid.c" 3 4
                              ((void *)0)
# 194 "../fs/proc/pid.c"
                                  ) {
...
static int proc_pid_fd_readlink(struct proc_entry *entry, char *buf) {
    struct task *task = proc_get_task(entry);
    if (task == 
# 261 "../fs/proc/pid.c" 3 4
               ((void *)0)
# 261 "../fs/proc/pid.c"
                   )
        return -3;
    __lock(&task->files->lock, "../fs/proc/pid.c", 263);
...
static int proc_pid_exe_readlink(struct proc_entry *entry, char *buf) {
    struct task *task = proc_get_task(entry);
    if (task == 
# 273 "../fs/proc/pid.c" 3 4
               ((void *)0)
# 273 "../fs/proc/pid.c"
                   )
        return -3;
    __lock(&task->general_lock, "../fs/proc/pid.c", 275);
...
static 
# 293 "../fs/proc/pid.c" 3 4
      _Bool 
# 293 "../fs/proc/pid.c"
           proc_pid_task_readdir(struct proc_entry *entry, unsigned long *index, struct proc_entry *next_entry) {
...
struct proc_children proc_pid_children = { .count = sizeof((struct proc_dir_entry[]){ {"auxv", .show = proc_pid_auxv_show}, {"cmdline", .show = proc_pid_cmdline_show}, {"exe", 
# 300 "../fs/proc/pid.c" 3 4
                                        0120000
# 300 "../fs/proc/pid.c"
                                        , .readlink = proc_pid_exe_readlink}, {"fd", 
# 300 "../fs/proc/pid.c" 3 4
                                        0040000
# 300 "../fs/proc/pid.c"
                                        , .readdir = proc_pid_fd_readdir}, {"maps", .show = proc_pid_maps_show}, {"mem", .pread = proc_pid_mem_pread, .pwrite = proc_pid_mem_pwrite}, {"stat", .show = proc_pid_stat_show}, {"task", 
# 300 "../fs/proc/pid.c" 3 4
                                        0040000
# 300 "../fs/proc/pid.c"
                                        , .readdir = proc_pid_task_readdir}, }) / sizeof(struct proc_dir_entry), .entries = { {"auxv", .show = proc_pid_auxv_show}, {"cmdline", .show = proc_pid_cmdline_show}, {"exe", 
# 300 "../fs/proc/pid.c" 3 4
                                        0120000
# 300 "../fs/proc/pid.c"
                                        , .readlink = proc_pid_exe_readlink}, {"fd", 
...
# 300 "../fs/proc/pid.c" 3 4
                                        0040000
# 300 "../fs/proc/pid.c"
                                        , .readdir = proc_pid_task_readdir}, } }
# 309 "../fs/proc/pid.c"
  ;

struct proc_dir_entry proc_pid = {
# 311 "../fs/proc/pid.c" 3 4
                                 ((void *)0)
# 311 "../fs/proc/pid.c"
                                     , 
# 311 "../fs/proc/pid.c" 3 4
