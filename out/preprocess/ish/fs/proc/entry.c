...
mode_t_ proc_entry_mode(struct proc_entry *entry) {
    ...
    if ((mode & 
# 9 "../fs/proc/entry.c" 3 4
               0170000
# 9 "../fs/proc/entry.c"
                     ) == 0)
        mode |= 
# 10 "../fs/proc/entry.c" 3 4
               0100000
# 10 "../fs/proc/entry.c"
                      ;
    if ((mode & 0777) == 0) {
        if (
# 12 "../fs/proc/entry.c" 3 4
           ((((
# 12 "../fs/proc/entry.c"
           mode
# 12 "../fs/proc/entry.c" 3 4
...
int proc_entry_stat(struct proc_entry *entry, struct statbuf *stat) {
    ...
    __lock(&pids_lock, "../fs/proc/entry.c", 26);
    struct task *task = pid_get_task(entry->pid);

    if (task != 
# 29 "../fs/proc/entry.c" 3 4
               ((void *)0)
# 29 "../fs/proc/entry.c"
                   ) {
        stat->uid = task->uid;
...
# 46 "../fs/proc/entry.c" 3 4
       ((void) sizeof ((
# 46 "../fs/proc/entry.c"
       !"missing name in proc entry"
# 46 "../fs/proc/entry.c" 3 4
       ) ? 1 : 0), __extension__ ({ if (
# 46 "../fs/proc/entry.c"
       !"missing name in proc entry"
...
# 46 "../fs/proc/entry.c"
       "!\"missing name in proc entry\""
# 46 "../fs/proc/entry.c" 3 4
       , "../fs/proc/entry.c", 46, __extension__ __PRETTY_FUNCTION__); }))
# 46 "../fs/proc/entry.c"
                                            ;
}


# 49 "../fs/proc/entry.c" 3 4
_Bool 
# 49 "../fs/proc/entry.c"
    proc_dir_read(struct proc_entry *entry, unsigned long *index, struct proc_entry *next_entry) {
...
# 55 "../fs/proc/entry.c"
        ...
        (*index)++;
        return 
# 59 "../fs/proc/entry.c" 3 4
              1
# 59 "../fs/proc/entry.c"
                  ;
    }
    
# 61 "../fs/proc/entry.c" 3 4
   ((void) sizeof ((
# 61 "../fs/proc/entry.c"
   !"read from invalid proc directory"
# 61 "../fs/proc/entry.c" 3 4
   ) ? 1 : 0), __extension__ ({ if (
# 61 "../fs/proc/entry.c"
   !"read from invalid proc directory"
...
# 61 "../fs/proc/entry.c"
   "!\"read from invalid proc directory\""
# 61 "../fs/proc/entry.c" 3 4
   , "../fs/proc/entry.c", 61, __extension__ __PRETTY_FUNCTION__); }))
# 61 "../fs/proc/entry.c"
                                              ;
}
...
void proc_entry_cleanup(struct proc_entry *entry) {
    if (entry->name != 
# 71 "../fs/proc/entry.c" 3 4
                      ((void *)0)
# 71 "../fs/proc/entry.c"
                          )
        free(entry->name);
    if (entry->child_names != 
# 73 "../fs/proc/entry.c" 3 4
                             ((void *)0)
# 73 "../fs/proc/entry.c"
                                 )
