...
static int proc_show_cpuinfo(struct proc_entry *UNUSED_entry __attribute__((unused)), struct proc_data *buf) {
    unsigned cpus = sysconf(
# 22 "../fs/proc/root.c" 3 4
                           _SC_NPROCESSORS_ONLN
# 22 "../fs/proc/root.c"
                                               );
...
struct proc_dir_entry proc_root_entries[] = {
    ...
    {"ish", 
# 112 "../fs/proc/root.c" 3 4
           0040000
# 112 "../fs/proc/root.c"
                  , .children = &proc_ish_children},
    {"meminfo", .show = proc_show_meminfo},
    {"mounts", .show = proc_show_mounts},
    {"self", 
# 115 "../fs/proc/root.c" 3 4
            0120000
# 115 "../fs/proc/root.c"
...
# 122 "../fs/proc/root.c"
    ...
    if (*index < sizeof(proc_root_entries)/sizeof(proc_root_entries[0])) {
        *next_entry = (struct proc_entry) {&proc_root_entries[*index], *index, 
# 124 "../fs/proc/root.c" 3 4
                                                                              ((void *)0)
# 124 "../fs/proc/root.c"
                                                                                  , 
# 124 "../fs/proc/root.c" 3 4
...
# 126 "../fs/proc/root.c"
    ...
    if (pid <= (1 << 15)) {
        __lock(&pids_lock, "../fs/proc/root.c", 131);
        do {
            pid++;
        } while (pid <= (1 << 15) && pid_get_task(pid) == 
# 134 "../fs/proc/root.c" 3 4
                                                       ((void *)0)
# 134 "../fs/proc/root.c"
                                                           );
        unlock(&pids_lock);
