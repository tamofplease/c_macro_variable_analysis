...
# 93 "../fs/fd.c"
   "size > table->size"
# 93 "../fs/fd.c" 3 4
   , "../fs/fd.c", 93, __extension__ __PRETTY_FUNCTION__); }))
# 93 "../fs/fd.c"
                             ;
...
# 103 "../fs/fd.c"
    ...
    memset(cloexec, 0, ((((size) - 1) / 8) + 1));
    if (table->cloexec)
        memcpy(cloexec, table->cloexec, ((((table->size) - 1) / 8) + 1));
...
struct fdtable *fdtable_copy(struct fdtable *table) {
    ...
    memcpy(new_table->cloexec, table->cloexec, ((((size) - 1) / 8) + 1));
    unlock(&table->lock);
    return new_table;
}
...
struct fd *f_get(fd_t f) {
    __lock(&current->files->lock, "../fs/fd.c", 152);
...
# 159 "../fs/fd.c"
   "start >= 0"
# 159 "../fs/fd.c" 3 4
   , "../fs/fd.c", 159, __extension__ __PRETTY_FUNCTION__); }))
# 159 "../fs/fd.c"
                     ;
...
fd_t f_install(struct fd *fd, int flags) {
    __lock(&current->files->lock, "../fs/fd.c", 185);
...
int f_close(fd_t f) {
    __lock(&current->files->lock, "../fs/fd.c", 210);
