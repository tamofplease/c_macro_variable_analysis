...
# 81 "../fs/lock.c"
       "lock->owner == request->owner"
# 81 "../fs/lock.c" 3 4
       , "../fs/lock.c", 81, __extension__ __PRETTY_FUNCTION__); }))
# 81 "../fs/lock.c"
                                            ;
...
# 119 "../fs/lock.c"
               "lock->end >= request->start"
# 119 "../fs/lock.c" 3 4
               , "../fs/lock.c", 119, __extension__ __PRETTY_FUNCTION__); }))
# 119 "../fs/lock.c"
                                                  ;
...
# 124 "../fs/lock.c"
               "lock->start <= request->end"
# 124 "../fs/lock.c" 3 4
               , "../fs/lock.c", 124, __extension__ __PRETTY_FUNCTION__); }))
# 124 "../fs/lock.c"
                                                  ;
...
int fcntl_getlk(struct fd *fd, struct flock_ *flock) {
    ...
    __lock(&inode->lock, "../fs/lock.c", 196);
...
# 213 "../fs/lock.c"
    ...
    struct inode_data *inode = fd->inode;
    __lock(&inode->lock, "../fs/lock.c", 223);
...
void file_lock_remove_owned_by(struct fd *fd, void *owner) {
    struct inode_data *inode = fd->inode;
    __lock(&inode->lock, "../fs/lock.c", 243);
