...
static void tty_poll_wakeup(struct tty *tty, int events) {
    unlock(&tty->lock);
    struct fd *fd;
    __lock(&tty->fds_lock, "../fs/tty.c", 86);
...
# 87 "../fs/tty.c"
    ...
    unlock(&tty->fds_lock);
    __lock(&tty->lock, "../fs/tty.c", 91);
}

void tty_release(struct tty *tty) {
    __lock(&tty->lock, "../fs/tty.c", 95);
    if (--tty->refcount == 0) {
...
# 110 "../fs/tty.c"
            ...
            __lock(&master->lock, "../fs/tty.c", 111);
...
static void tty_set_controlling(struct tgroup *group, struct tty *tty) {
    __lock(&group->lock, "../fs/tty.c", 120);
    if (group->tty == 
# 121 "../fs/tty.c" 3 4
                     ((void *)0)
# 121 "../fs/tty.c"
...
int tty_open(struct tty *tty, struct fd *fd) {
    ...
    __lock(&tty->fds_lock, "../fs/tty.c", 137);
    ...
    if (!(fd->flags & (1 << 8))) {
        ...
        __lock(&pids_lock, "../fs/tty.c", 145);
        __lock(&tty->lock, "../fs/tty.c", 146);
...
static int tty_device_open(int major, int minor, struct fd *fd) {
    ...
    if (major == 5) {
        if (minor == 0) {
            __lock(&ttys_lock, "../fs/tty.c", 160);
            __lock(&current->group->lock, "../fs/tty.c", 161);
...
# 181 "../fs/tty.c"
       "driver != NULL"
# 181 "../fs/tty.c" 3 4
       , "../fs/tty.c", 181, __extension__ __PRETTY_FUNCTION__); }))
# 181 "../fs/tty.c"
                             ;
...
# 201 "../fs/tty.c"
        ...
        __lock(&fd->tty->fds_lock, "../fs/tty.c", 202);
        list_remove_safe(&fd->tty_other_fds);
        unlock(&fd->tty->fds_lock);
        __lock(&ttys_lock, "../fs/tty.c", 205);
        tty_release(fd->tty);
        unlock(&ttys_lock);
    }
    return 0;
...
# 362 "../fs/tty.c"
           "tty->bufsize < sizeof(tty->buf)"
# 362 "../fs/tty.c" 3 4
           , "../fs/tty.c", 362, __extension__ __PRETTY_FUNCTION__); }))
# 362 "../fs/tty.c"
                                                  ;
...
# 370 "../fs/tty.c"
   "tty->bufsize <= sizeof(tty->buf)"
# 370 "../fs/tty.c" 3 4
   , "../fs/tty.c", 370, __extension__ __PRETTY_FUNCTION__); }))
# 370 "../fs/tty.c"
                                           ;
    unlock(&tty->lock);
...
# 387 "../fs/tty.c"
   "bufsize <= tty->bufsize"
# 387 "../fs/tty.c" 3 4
   , "../fs/tty.c", 387, __extension__ __PRETTY_FUNCTION__); }))
# 387 "../fs/tty.c"
                                  ;
...
# 404 "../fs/tty.c"
    ...
    __lock(&slave->lock, "../fs/tty.c", 408);
...
# 414 "../fs/tty.c"
    ...
    __lock(&current->group->lock, "../fs/tty.c", 415);
...
static ssize_t tty_read(struct fd *fd, void *buf, size_t bufsize) {
    ...
    __lock(&pids_lock, "../fs/tty.c", 443);
    __lock(&tty->lock, "../fs/tty.c", 444);
    if (tty->hung_up) {
        unlock(&pids_lock);
        goto error;
    }
...
static int tiocsctty(struct tty *tty, int force) {
    ...
    __lock(&pids_lock, "../fs/tty.c", 624);
    __lock(&tty->lock, "../fs/tty.c", 625);
...
# 640 "../fs/tty.c"
                ...
                __lock(&tgroup->lock, "../fs/tty.c", 641);
                if (tgroup->tty == tty) {
                    tgroup->tty = 
# 643 "../fs/tty.c" 3 4
                                 ((void *)0)
...
static int tty_ioctl(struct fd *fd, int cmd, void *arg) {
    ...
    switch (cmd) {
        ...
        case 0x5410:
            ...
            unlock(&tty->lock);
            __lock(&pids_lock, "../fs/tty.c", 743);
            __lock(&tty->lock, "../fs/tty.c", 744);
