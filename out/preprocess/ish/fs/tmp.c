...
# 153 "../fs/tmp.c"
    ...
    while (path_next_component(&path, component, &err)) {
        ...
        __lock(&dirent->lock, "../fs/tmp.c", 163);
...
# 190 "../fs/tmp.c"
   file->stat.mode
# 190 "../fs/tmp.c" 3 4
   )) & 0170000) == (0100000))) ? 1 : 0), __extension__ ({ if (((((
# 190 "../fs/tmp.c"
   file->stat.mode
# 190 "../fs/tmp.c" 3 4
   )) & 0170000) == (0100000))) ; else __assert_fail (
# 190 "../fs/tmp.c"
   "S_ISREG(file->stat.mode)"
# 190 "../fs/tmp.c" 3 4
   , "../fs/tmp.c", 190, __extension__ __PRETTY_FUNCTION__); }))
# 190 "../fs/tmp.c"
                                   ;
...
static struct fd *tmpfs_open(struct mount *mount, const char *path, int flags, int mode) {
    ...
    if (flags & (1 << 6)) {
        ...
        __lock(&parent->lock, "../fs/tmp.c", 259);
        int err = 0;
...
# 301 "../fs/tmp.c"
                           ;
    __lock(&dirent->lock, "../fs/tmp.c", 302);
...
static int tmpfs_stat(struct mount *mount, const char *path, struct statbuf *stat) {
    ...
    __lock(&inode->lock, "../fs/tmp.c", 315);
...
static int tmpfs_mkdir(struct mount *mount, const char *path, mode_t_ mode) {
    ...
    __lock(&parent->lock, "../fs/tmp.c", 334);
...
static int tmpfs_fstat(struct fd *fd, struct statbuf *stat) {
    ...
    __lock(&inode->lock, "../fs/tmp.c", 379);
    *stat = inode->stat;
    unlock(&inode->lock);
    return 0;
}
...
static ssize_t tmpfs_read(struct fd *fd, void *buf, size_t bufsize) {
    ssize_t res;
    struct tmp_inode *inode = tmpfs_fd_inode(fd);
    __lock(&inode->lock, "../fs/tmp.c", 388);
    res = -21;
    if (
# 390 "../fs/tmp.c" 3 4
       ((((
...
# 392 "../fs/tmp.c" 3 4
   , "../fs/tmp.c", 392, __extension__ __PRETTY_FUNCTION__); }))
# 392 "../fs/tmp.c"
                                    ;
...
static ssize_t tmpfs_write(struct fd *fd, const void *buf, size_t bufsize) {
    ssize_t res;
    struct tmp_inode *inode = tmpfs_fd_inode(fd);
    __lock(&inode->lock, "../fs/tmp.c", 411);
    res = -21;
    if (
# 413 "../fs/tmp.c" 3 4
       ((((
...
# 415 "../fs/tmp.c"
   "S_ISREG(inode->stat.mode)"
# 415 "../fs/tmp.c" 3 4
   , "../fs/tmp.c", 415, __extension__ __PRETTY_FUNCTION__); }))
# 415 "../fs/tmp.c"
                                    ;
...
static off_t_ tmpfs_lseek(struct fd *fd, off_t_ off, int whence) {
    ...
    if (whence == 2) {
        struct tmp_inode *inode = tmpfs_fd_inode(fd);
        __lock(&inode->lock, "../fs/tmp.c", 435);
...
# 450 "../fs/tmp.c"
    ...
    __lock(&fd->lock, "../fs/tmp.c", 453);
    __lock(&parent->lock, "../fs/tmp.c", 454);
...
# 484 "../fs/tmp.c"
   dir->inode->stat.mode
# 484 "../fs/tmp.c" 3 4
   )) & 0170000) == (0040000))) ? 1 : 0), __extension__ ({ if (((((
# 484 "../fs/tmp.c"
   dir->inode->stat.mode
# 484 "../fs/tmp.c" 3 4
   )) & 0170000) == (0040000))) ; else __assert_fail (
# 484 "../fs/tmp.c"
   "S_ISDIR(dir->inode->stat.mode)"
# 484 "../fs/tmp.c" 3 4
   , "../fs/tmp.c", 484, __extension__ __PRETTY_FUNCTION__); }))
# 484 "../fs/tmp.c"
                                         ;
