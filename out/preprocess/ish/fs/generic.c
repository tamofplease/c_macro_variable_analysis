...
_Bool 
# 23 "../fs/generic.c"
    contains_mount_point(const char *path) {
    struct mount *mount;
...
struct fd *generic_openat(struct fd *at, const char *path_raw, int flags, int mode) {
    ...
    char path[4096];
    int err = path_normalize(at, path_raw, path, 1 |
            (flags & (1 << 6) ? 4 : 0));
    if (err < 0)
        return (void *) (intptr_t) (err);
    struct mount *mount = find_mount_and_trim_path(path);
    __lock(&inodes_lock, "../fs/generic.c", 45);
...
# 64 "../fs/generic.c"
    ...
    int accmode;
    if (flags & (1 << 1)) accmode = 4 | 2;
    else if (flags & (1 << 0)) accmode = 2;
    else accmode = 4;
...
# 75 "../fs/generic.c"
   fd->type
# 75 "../fs/generic.c" 3 4
   )) & 0170000) == (0120000))) ? 1 : 0), __extension__ ({ if (
# 75 "../fs/generic.c"
   !
# 75 "../fs/generic.c" 3 4
   ((((
# 75 "../fs/generic.c"
   fd->type
# 75 "../fs/generic.c" 3 4
   )) & 0170000) == (0120000))) ; else __assert_fail (
# 75 "../fs/generic.c"
   "!S_ISLNK(fd->type)"
# 75 "../fs/generic.c" 3 4
   , "../fs/generic.c", 75, __extension__ __PRETTY_FUNCTION__); }))
# 75 "../fs/generic.c"
                             ;
    if (
# 76 "../fs/generic.c" 3 4
...
struct fd *generic_open(const char *path, int flags, int mode) {
    return generic_openat((struct fd *) -2, path, flags, mode);
}
...
int generic_getpath(struct fd *fd, char *buf) {
    ...
    if (err < 0)
        return err;
    if (strlen(buf) + strlen(fd->mount->point) >= 4096)
        return -36;
...
int generic_accessat(struct fd *dirfd, const char *path_raw, int mode) {
    char path[4096];
    int err = path_normalize(dirfd, path_raw, path, 1);
    if (err < 0)
        return err;
...
int generic_linkat(struct fd *src_at, const char *src_raw, struct fd *dst_at, const char *dst_raw) {
    char src[4096];
    int err = path_normalize(src_at, src_raw, src, 2);
    if (err < 0)
        return err;
    char dst[4096];
    err = path_normalize(dst_at, dst_raw, dst, 2 | 4);
    if (err < 0)
        return err;
    ...
    if (mount != dst_mount)
        err = -18;
    else if (mount->fs->link == 
# 147 "../fs/generic.c" 3 4
                               ((void *)0)
# 147 "../fs/generic.c"
                                   )
        err = -1;
...
int generic_unlinkat(struct fd *at, const char *path_raw) {
    char path[4096];
    int err = path_normalize(at, path_raw, path, 2);
    if (err < 0)
        return err;
    struct mount *mount = find_mount_and_trim_path(path);
    err = -1;
...
int generic_renameat(struct fd *src_at, const char *src_raw, struct fd *dst_at, const char *dst_raw) {
    ...
    if (mount != dst_mount)
        err = -18;
    else if (mount->fs->rename == 
# 184 "../fs/generic.c" 3 4
                                 ((void *)0)
# 184 "../fs/generic.c"
                                     )
        err = -1;
...
int generic_symlinkat(const char *target, struct fd *at, const char *link_raw) {
    char link[4096];
    int err = path_normalize(at, link_raw, link, 2 | 4);
    if (err < 0)
        return err;
...
# 209 "../fs/generic.c"
    ...
    char path[4096];
    int err = path_normalize(at, path_raw, path, 2 | 4);
    if (err < 0)
        return err;
    struct mount *mount = find_mount_and_trim_path(path);
    err = -1;
...
int generic_setattrat(struct fd *at, const char *path_raw, struct attr attr, 
# 224 "../fs/generic.c" 3 4
                                                                            _Bool 
# 224 "../fs/generic.c"
                                                                                 follow_links) {
    char path[4096];
...
int generic_mkdirat(struct fd *at, const char *path_raw, mode_t_ mode) {
    char path[4096];
    int err = path_normalize(at, path_raw, path, 1 | 4);
    if (err < 0)
        return err;
    struct mount *mount = find_mount_and_trim_path(path);
    err = -1;
