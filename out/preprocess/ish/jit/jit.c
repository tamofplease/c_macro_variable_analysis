...
struct jit *jit_new(struct mmu *mmu) {
    ...
    jit->mmu = mmu;
    jit_resize_hash(jit, (1 << 10));
    jit->page_hash = calloc((1 << 10), sizeof(*jit->page_hash));
...
static inline struct list *blocks_list(struct jit *jit, page_t page, int i) {
    ...
    return &jit->page_hash[page % (1 << 10)].blocks[i];
}

void jit_invalidate_range(struct jit *jit, page_t start, page_t end) {
    __lock(&jit->lock, "../jit/jit.c", 50);
    struct jit_block *block, *tmp;
...
# 134 "../jit/jit.c" 3 4
   , "../jit/jit.c", 134, __extension__ __PRETTY_FUNCTION__); }))
# 134 "../jit/jit.c"
                                     ;
...
static int cpu_step_to_interrupt(struct cpu_state *cpu, struct tlb *tlb) {
    ...
    struct jit_block **cache = calloc((1 << 10), sizeof(*cache));
...
# 187 "../jit/jit.c"
   "jit->mmu == cpu->mmu"
# 187 "../jit/jit.c" 3 4
   , "../jit/jit.c", 187, __extension__ __PRETTY_FUNCTION__); }))
# 187 "../jit/jit.c"
                               ;
...
# 215 "../jit/jit.c"
        ...
        if (interrupt == -1 && __atomic_exchange_n(cpu->poked_ptr, 
# 233 "../jit/jit.c" 3 4
                                                                        0
# 233 "../jit/jit.c"
                                                                             , 5))
...
# 264 "../jit/jit.c"
    ...
    struct jit *jit = cpu->mmu->jit;
    __lock(&jit->lock, "../jit/jit.c", 271);
    if (!list_empty(&jit->jetsam)) {



        unlock(&jit->lock);
        __write_wrlock(&jit->jetsam_lock, "../jit/jit.c", 277);
        __lock(&jit->lock, "../jit/jit.c", 278);
        jit_free_jetsam(jit);
...
void cpu_poke(struct cpu_state *cpu) {
    __atomic_store_n(cpu->poked_ptr, 
# 288 "../jit/jit.c" 3 4
                                    1
# 288 "../jit/jit.c"
                                        , 5);
