# 1 "/workspace/acl/lib_protocol/src/icmp/icmp_host.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 2 "/workspace/acl/lib_protocol/src/icmp/icmp_host.c" 2
...
# 3 "/workspace/acl/lib_protocol/src/icmp/icmp_host.c" 2
...
# 4 "/workspace/acl/lib_protocol/src/icmp/icmp_host.c" 2
...
# 5 "/workspace/acl/lib_protocol/src/icmp/icmp_host.c" 2
...
void icmp_host_free(ICMP_HOST *host)
{
 ...
 do { if (host) { acl_free_glue("/workspace/acl/lib_protocol/src/icmp/icmp_host.c", 13, (host)); (host) = 
# 13 "/workspace/acl/lib_protocol/src/icmp/icmp_host.c" 3 4
((void *)0)
# 13 "/workspace/acl/lib_protocol/src/icmp/icmp_host.c"
; } } while (0);
}
...
ICMP_HOST *icmp_host_alloc(ICMP_CHAT *chat, const char *domain, const char *ip)
{
 ...
 if (ip == 
# 21 "/workspace/acl/lib_protocol/src/icmp/icmp_host.c" 3 4
          ((void *)0) 
# 21 "/workspace/acl/lib_protocol/src/icmp/icmp_host.c"
               || *ip == 0)
  acl_msg_fatal("%s(%d): ip null", __FUNCTION__, 22);

 host = (ICMP_HOST*) acl_calloc_glue("/workspace/acl/lib_protocol/src/icmp/icmp_host.c", 24, 1, sizeof(ICMP_HOST));
 if (domain && *domain)
  do { if (sizeof(host->domain) > 0) { size_t _n = strlen(domain); _n = _n > (size_t ) sizeof(host->domain) - 1? (size_t) sizeof(host->domain) - 1 : _n; memcpy(host->domain, domain, _n); host->domain[_n] = 0; } } while (0);
 do { if (sizeof(host->dest_ip) > 0) { size_t _n = strlen(ip); _n = _n > (size_t ) sizeof(host->dest_ip) - 1? (size_t) sizeof(host->dest_ip) - 1 : _n; memcpy(host->dest_ip, ip, _n); host->dest_ip[_n] = 0; } } while (0);
 ...
 if (chat->aio != 
# 33 "/workspace/acl/lib_protocol/src/icmp/icmp_host.c" 3 4
                 ((void *)0)
# 33 "/workspace/acl/lib_protocol/src/icmp/icmp_host.c"
                     )
  icmp_chat_aio_add(chat, host);
 acl_ring_prepend(&chat->host_head, &host->host_ring);
 host->chat = chat;
 host->nsent = 0;
 host->dest.sin_family = 
# 38 "/workspace/acl/lib_protocol/src/icmp/icmp_host.c" 3 4
                             2
# 38 "/workspace/acl/lib_protocol/src/icmp/icmp_host.c"
                                    ;
 ...
 size_t npkt, size_t dlen, int delay, int timeout)
{
 ...
 host->dlen = dlen;
 host->npkt = npkt;
 host->pkts = (ICMP_PKT**) acl_calloc_glue("/workspace/acl/lib_protocol/src/icmp/icmp_host.c", 58, host->npkt, sizeof(ICMP_PKT*));

 for (i = 0; i < npkt; i++) {
  ICMP_PKT *pkt = icmp_pkt_alloc();
  icmp_pkt_client(host, pkt, type, code, 
# 62 "/workspace/acl/lib_protocol/src/icmp/icmp_host.c" 3 4
                                        ((void *)0)
# 62 "/workspace/acl/lib_protocol/src/icmp/icmp_host.c"
                                            , dlen);
  host->pkts[i] = pkt;
 }
}
 ...
 size_t npkt, size_t dlen, int delay, int timeout)
{
 ...
 icmp_host_init(host, 8, 0, npkt, dlen, delay, timeout);
 return host;
}
