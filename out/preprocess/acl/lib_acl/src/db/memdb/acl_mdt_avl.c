# 1 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 2 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c" 2
# 22 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c"
# 1 "/workspace/acl/lib_acl/src/db/memdb/define.h" 1
# 23 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c" 2
...
# 24 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c" 2
...
# 25 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c" 2
...
static ACL_MDT_IDX *mdt_idx_create(ACL_MDT *mdt, size_t init_capacity __attribute__ ((__unused__)),
 const char *name, unsigned int flag)
{
 ACL_MDT_IDX_AVL *idx;

 unsigned int slice_align = (1 << 11);



 unsigned int rtgc_off = 0;

 idx = (ACL_MDT_IDX_AVL*) acl_calloc_glue("/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c", 72, 1, sizeof(ACL_MDT_IDX_AVL));
 avl_create(&idx->avl, cmp_fn, sizeof(TREE_NODE), 
# 73 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c" 3 4
                                                 __builtin_offsetof (
# 73 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c"
                                                 TREE_NODE
# 73 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c" 3 4
                                                 , 
# 73 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c"
...
# 73 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c"
 ...
 if ((mdt->tbl_flag & (1 << 13)))
  rtgc_off = 1;

 if ((mdt->tbl_flag & (1 << 10)))
  idx->slice = acl_slice_create("ACL_MDT_IDX_AVL->slice", 0,
   sizeof(TREE_NODE), (1 << 0) | slice_align | rtgc_off);
 else if ((mdt->tbl_flag & (1 << 11)))
  idx->slice = acl_slice_create("ACL_MDT_IDX_AVL->slice", 0,
   sizeof(TREE_NODE), (1 << 1) | slice_align | rtgc_off);
 else if ((mdt->tbl_flag & (1 << 12)))
  idx->slice = acl_slice_create("ACL_MDT_IDX_AVL->slice", 0,
   sizeof(TREE_NODE), (1 << 2) | slice_align | rtgc_off);

 idx->idx.name = acl_strdup_glue("/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c", 88, (name));
 idx->idx.flag = flag;
...
static void mdt_idx_free(ACL_MDT_IDX *idx)
{
 ...
 while (1) {
  ...
  if (pnode == 
# 100 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c" 3 4
              ((void *)0)
# 100 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c"
                  )
   break;

  avl_remove(&idx_avl->avl, pnode);
  if (!(idx->flag & (1 << 1)))
   do { if (pnode->key.key) { acl_free_glue("/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c", 105, (pnode->key.key)); (pnode->key.key) = 
# 105 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c" 3 4
  ((void *)0)
# 105 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c"
  ; } } while (0);

  if (idx_avl->slice)
   acl_slice_free2(idx_avl->slice, pnode);
  else
   do { if (pnode) { acl_free_glue("/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c", 110, (pnode)); (pnode) = 
# 110 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c" 3 4
  ((void *)0)
# 110 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c"
  ; } } while (0);
 }

 avl_destroy(&idx_avl->avl);
 do { if (idx->name) { acl_free_glue("/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c", 114, (idx->name)); (idx->name) = 
# 114 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c" 3 4
((void *)0)
# 114 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c"
; } } while (0);
 if (idx_avl->slice)
  acl_slice_destroy(idx_avl->slice);
 do { if (idx_avl) { acl_free_glue("/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c", 117, (idx_avl)); (idx_avl) = 
# 117 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c" 3 4
((void *)0)
# 117 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c"
; } } while (0);
}
# 127 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c"
static void mdt_idx_add(ACL_MDT_IDX *idx, const char *key, ACL_MDT_REC *rec)
{
 ...
 if (idx_avl->slice)
  ...
  pnode = (TREE_NODE*) acl_malloc_glue("/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c", 135, sizeof(TREE_NODE));
 if (idx->flag & (1 << 1))
  pnode->key.c_key = key;
 else
  pnode->key.key = acl_strdup_glue("/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c", 139, (key));
...
static ACL_MDT_REC *mdt_idx_get(ACL_MDT_IDX *idx, const char *key)
{
 ...
 node.key.c_key = key;
 pnode = (TREE_NODE*) avl_find(&idx_avl->avl, &node, 
# 158 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c" 3 4
                                                    ((void *)0)
# 158 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c"
                                                        );
 return (pnode ? pnode->rec : 
# 159 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c" 3 4
                             ((void *)0)
# 159 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c"
                                 );
}
...
static void mdt_idx_del(ACL_MDT_IDX *idx, const char *key)
{
 ...
 node.key.c_key = key;
 pnode = (TREE_NODE*) avl_find(&idx_avl->avl, &node, 
# 175 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c" 3 4
                                                    ((void *)0)
# 175 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c"
                                                        );
 if (pnode == 
# 176 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c" 3 4
             ((void *)0)
# 176 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c"
                 )
  acl_msg_fatal("%s: key(%s) not exist", myname, key);
 avl_remove(&idx_avl->avl, pnode);
 if (!(idx->flag & (1 << 1)))
  do { if (pnode->key.key) { acl_free_glue("/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c", 180, (pnode->key.key)); (pnode->key.key) = 
# 180 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c" 3 4
 ((void *)0)
# 180 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c"
 ; } } while (0);
 if (idx_avl->slice)
  acl_slice_free2(idx_avl->slice, pnode);
 else
  do { if (pnode) { acl_free_glue("/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c", 184, (pnode)); (pnode) = 
# 184 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c" 3 4
 ((void *)0)
# 184 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c"
 ; } } while (0);
}
...
static void mdt_avl_free(ACL_MDT *mdt)
{
 ...
 do { if (mdt_avl) { acl_free_glue("/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c", 194, (mdt_avl)); (mdt_avl) = 
# 194 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c" 3 4
((void *)0)
# 194 "/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c"
; } } while (0);
}
...
ACL_MDT *acl_mdt_avl_create()
{
 ...
 mdt = (ACL_MDT_AVL *) acl_calloc_glue("/workspace/acl/lib_acl/src/db/memdb/acl_mdt_avl.c", 201, 1, sizeof(ACL_MDT_AVL));
