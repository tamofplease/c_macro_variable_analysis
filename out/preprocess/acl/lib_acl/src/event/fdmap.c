# 0 "/acl/lib_acl/src/event/fdmap.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "/acl/lib_acl/src/event/fdmap.c"
...
# 2 "/acl/lib_acl/src/event/fdmap.c" 2
# 16 "/acl/lib_acl/src/event/fdmap.c"
...
# 17 "/acl/lib_acl/src/event/fdmap.c" 2
...
ACL_FD_MAP *acl_fdmap_create(int size)
{
 ...
 map = (ACL_FD_MAP *) acl_calloc_glue("/acl/lib_acl/src/event/fdmap.c", 37, 1, sizeof(ACL_FD_MAP));
 if (map == 
# 38 "/acl/lib_acl/src/event/fdmap.c" 3 4
           ((void *)0)
# 38 "/acl/lib_acl/src/event/fdmap.c"
               )
  acl_msg_fatal("%s(%d): calloc error(%s)",
   myname, 40, acl_last_serror());

 map->size = size;
 map->table = (FD_ENTRY *) acl_calloc_glue("/acl/lib_acl/src/event/fdmap.c", 43, map->size, sizeof(FD_ENTRY));
 if (map->table == 
# 44 "/acl/lib_acl/src/event/fdmap.c" 3 4
                  ((void *)0)
# 44 "/acl/lib_acl/src/event/fdmap.c"
                      )
  acl_msg_fatal("%s(%d): calloc error(%s)",
...
void acl_fdmap_add(ACL_FD_MAP *map, int fd, void *ctx)
{
 ...
 if (fd >= map->size) {
  ...
  map->size = fd + 1024;
  map->table = (FD_ENTRY *)
   acl_realloc_glue("/acl/lib_acl/src/event/fdmap.c", 60, (map->table), map->size * sizeof(FD_ENTRY));
 }
 map->table[fd].fd = fd;
...
{
 if (map) {
  do { if (map->table) { acl_free_glue("/acl/lib_acl/src/event/fdmap.c", 90, (map->table)); (map->table) = 
# 90 "/acl/lib_acl/src/event/fdmap.c" 3 4
 ((void *)0)
# 90 "/acl/lib_acl/src/event/fdmap.c"
 ; } } while (0);
  do { if (map) { acl_free_glue("/acl/lib_acl/src/event/fdmap.c", 91, (map)); (map) = 
# 91 "/acl/lib_acl/src/event/fdmap.c" 3 4
 ((void *)0)
# 91 "/acl/lib_acl/src/event/fdmap.c"
 ; } } while (0);
 }
}
