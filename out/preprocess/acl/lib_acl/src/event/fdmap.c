# 1 "/workspace/acl/lib_acl/src/event/fdmap.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 2 "/workspace/acl/lib_acl/src/event/fdmap.c" 2
# 16 "/workspace/acl/lib_acl/src/event/fdmap.c"
...
# 17 "/workspace/acl/lib_acl/src/event/fdmap.c" 2
...
ACL_FD_MAP *acl_fdmap_create(int size)
{
 ...
 map = (ACL_FD_MAP *) acl_calloc_glue("/workspace/acl/lib_acl/src/event/fdmap.c", 37, 1, sizeof(ACL_FD_MAP));
 if (map == 
# 38 "/workspace/acl/lib_acl/src/event/fdmap.c" 3 4
           ((void *)0)
# 38 "/workspace/acl/lib_acl/src/event/fdmap.c"
               )
  acl_msg_fatal("%s(%d): calloc error(%s)",
   myname, 40, acl_last_serror());

 map->size = size;
 map->table = (FD_ENTRY *) acl_calloc_glue("/workspace/acl/lib_acl/src/event/fdmap.c", 43, map->size, sizeof(FD_ENTRY));
 if (map->table == 
# 44 "/workspace/acl/lib_acl/src/event/fdmap.c" 3 4
                  ((void *)0)
# 44 "/workspace/acl/lib_acl/src/event/fdmap.c"
                      )
  acl_msg_fatal("%s(%d): calloc error(%s)",
   myname, 46, acl_last_serror());
...
void acl_fdmap_add(ACL_FD_MAP *map, int fd, void *ctx)
{
 ...
 if (fd >= map->size) {
  acl_msg_warn("%s(%d): fd(%d) >= map's size(%d), extend it to %d",
   myname, 57, fd, map->size, fd + 1024);
  map->size = fd + 1024;
  map->table = (FD_ENTRY *)
   acl_realloc_glue("/workspace/acl/lib_acl/src/event/fdmap.c", 60, (map->table), map->size * sizeof(FD_ENTRY));
 }
 map->table[fd].fd = fd;
...
{
 if (map) {
  do { if (map->table) { acl_free_glue("/workspace/acl/lib_acl/src/event/fdmap.c", 90, (map->table)); (map->table) = 
# 90 "/workspace/acl/lib_acl/src/event/fdmap.c" 3 4
 ((void *)0)
# 90 "/workspace/acl/lib_acl/src/event/fdmap.c"
 ; } } while (0);
  do { if (map) { acl_free_glue("/workspace/acl/lib_acl/src/event/fdmap.c", 91, (map)); (map) = 
# 91 "/workspace/acl/lib_acl/src/event/fdmap.c" 3 4
 ((void *)0)
# 91 "/workspace/acl/lib_acl/src/event/fdmap.c"
 ; } } while (0);
 }
}
