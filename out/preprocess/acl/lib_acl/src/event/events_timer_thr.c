# 0 "/acl/lib_acl/src/event/events_timer_thr.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "/acl/lib_acl/src/event/events_timer_thr.c"
...
# 2 "/acl/lib_acl/src/event/events_timer_thr.c" 2
# 21 "/acl/lib_acl/src/event/events_timer_thr.c"
...
# 22 "/acl/lib_acl/src/event/events_timer_thr.c" 2
 ...
 long long int delay, int keep __attribute__ ((__unused__)))
{
 ...
 do { acl_pthread_mutex_t *mutex_ptr = (&event_thr->tm_mutex); int status = pthread_mutex_lock(mutex_ptr); if (status != 0) acl_msg_fatal("%s(%d): lock error(%s)", "/acl/lib_acl/src/event/events_timer_thr.c", 36, strerror(status)); } while (0);

 when = event_timer_request(eventp, callback, context, delay, 0);

 do { acl_pthread_mutex_t *mutex_ptr = (&event_thr->tm_mutex); int status = pthread_mutex_unlock(mutex_ptr); if (status != 0) acl_msg_fatal("%s(%d): unlock error(%s)", "/acl/lib_acl/src/event/events_timer_thr.c", 40, strerror(status)); } while (0);
 return when;
}
 ...
 ACL_EVENT_NOTIFY_TIME callback, void *context)
{
 ...
 do { acl_pthread_mutex_t *mutex_ptr = (&event_thr->tm_mutex); int status = pthread_mutex_lock(mutex_ptr); if (status != 0) acl_msg_fatal("%s(%d): lock error(%s)", "/acl/lib_acl/src/event/events_timer_thr.c", 52, strerror(status)); } while (0);

 time_left = event_timer_cancel(eventp, callback, context);

 do { acl_pthread_mutex_t *mutex_ptr = (&event_thr->tm_mutex); int status = pthread_mutex_unlock(mutex_ptr); if (status != 0) acl_msg_fatal("%s(%d): unlock error(%s)", "/acl/lib_acl/src/event/events_timer_thr.c", 56, strerror(status)); } while (0);
 return time_left;
}
...
# 80 "/acl/lib_acl/src/event/events_timer_thr.c"
void event_timer_trigger_thr(EVENT_THR *event)
{
 do { acl_pthread_mutex_t *mutex_ptr = (&event->tm_mutex); int status = pthread_mutex_lock(mutex_ptr); if (status != 0) acl_msg_fatal("%s(%d): lock error(%s)", "/acl/lib_acl/src/event/events_timer_thr.c", 82, strerror(status)); } while (0);

 event_timer_trigger(&event->event);

 do { acl_pthread_mutex_t *mutex_ptr = (&event->tm_mutex); int status = pthread_mutex_unlock(mutex_ptr); if (status != 0) acl_msg_fatal("%s(%d): unlock error(%s)", "/acl/lib_acl/src/event/events_timer_thr.c", 86, strerror(status)); } while (0);
}
