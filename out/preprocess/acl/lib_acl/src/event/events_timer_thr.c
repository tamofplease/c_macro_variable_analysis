# 1 "/workspace/acl/lib_acl/src/event/events_timer_thr.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 2 "/workspace/acl/lib_acl/src/event/events_timer_thr.c" 2
# 21 "/workspace/acl/lib_acl/src/event/events_timer_thr.c"
...
# 22 "/workspace/acl/lib_acl/src/event/events_timer_thr.c" 2



long long int event_timer_request_thr(ACL_EVENT *eventp,
 ACL_EVENT_NOTIFY_TIME callback, void *context,
 long long int delay, int keep __attribute__ ((__unused__)))
{
 ...
 do { acl_pthread_mutex_t *mutex_ptr = (&event_thr->tm_mutex); int status = pthread_mutex_lock(mutex_ptr); if (status != 0) acl_msg_fatal("%s(%d): lock error(%s)", "/workspace/acl/lib_acl/src/event/events_timer_thr.c", 36, strerror(status)); } while (0);

 when = event_timer_request(eventp, callback, context, delay, 0);

 do { acl_pthread_mutex_t *mutex_ptr = (&event_thr->tm_mutex); int status = pthread_mutex_unlock(mutex_ptr); if (status != 0) acl_msg_fatal("%s(%d): unlock error(%s)", "/workspace/acl/lib_acl/src/event/events_timer_thr.c", 40, strerror(status)); } while (0);
 return when;
}



long long int event_timer_cancel_thr(ACL_EVENT *eventp,
 ACL_EVENT_NOTIFY_TIME callback, void *context)
{
 EVENT_THR *event_thr = (EVENT_THR *) eventp;
 long long int time_left;

 do { acl_pthread_mutex_t *mutex_ptr = (&event_thr->tm_mutex); int status = pthread_mutex_lock(mutex_ptr); if (status != 0) acl_msg_fatal("%s(%d): lock error(%s)", "/workspace/acl/lib_acl/src/event/events_timer_thr.c", 52, strerror(status)); } while (0);

 time_left = event_timer_cancel(eventp, callback, context);

 do { acl_pthread_mutex_t *mutex_ptr = (&event_thr->tm_mutex); int status = pthread_mutex_unlock(mutex_ptr); if (status != 0) acl_msg_fatal("%s(%d): unlock error(%s)", "/workspace/acl/lib_acl/src/event/events_timer_thr.c", 56, strerror(status)); } while (0);
 return time_left;
}

void event_timer_keep_thr(ACL_EVENT *eventp __attribute__ ((__unused__)),
 ACL_EVENT_NOTIFY_TIME callback __attribute__ ((__unused__)),
 void *context __attribute__ ((__unused__)), int keep __attribute__ ((__unused__)))
{
}

int event_timer_ifkeep_thr(ACL_EVENT *eventp __attribute__ ((__unused__)),
 ACL_EVENT_NOTIFY_TIME callback __attribute__ ((__unused__)),
 void *context __attribute__ ((__unused__)))
{
 return 0;
}
# 80 "/workspace/acl/lib_acl/src/event/events_timer_thr.c"
void event_timer_trigger_thr(EVENT_THR *event)
{
 do { acl_pthread_mutex_t *mutex_ptr = (&event->tm_mutex); int status = pthread_mutex_lock(mutex_ptr); if (status != 0) acl_msg_fatal("%s(%d): lock error(%s)", "/workspace/acl/lib_acl/src/event/events_timer_thr.c", 82, strerror(status)); } while (0);

 event_timer_trigger(&event->event);

 do { acl_pthread_mutex_t *mutex_ptr = (&event->tm_mutex); int status = pthread_mutex_unlock(mutex_ptr); if (status != 0) acl_msg_fatal("%s(%d): unlock error(%s)", "/workspace/acl/lib_acl/src/event/events_timer_thr.c", 86, strerror(status)); } while (0);
}
