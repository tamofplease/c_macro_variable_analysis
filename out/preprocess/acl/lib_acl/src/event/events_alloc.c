# 1 "/workspace/acl/lib_acl/src/event/events_alloc.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 2 "/workspace/acl/lib_acl/src/event/events_alloc.c" 2
# 13 "/workspace/acl/lib_acl/src/event/events_alloc.c"
...
# 14 "/workspace/acl/lib_acl/src/event/events_alloc.c" 2

static void event_default_loop(ACL_EVENT *eventp)
{
 const char *myname = "event_default_loop";
 acl_msg_fatal("%s(%d): Not supported in %s",
  myname, 19, eventp->name);
}

static void event_default_enable_read(ACL_EVENT *eventp,
 ACL_VSTREAM *stream __attribute__ ((__unused__)), int timeout __attribute__ ((__unused__)),
 ACL_EVENT_NOTIFY_RDWR callback __attribute__ ((__unused__)), void *context __attribute__ ((__unused__)))
{
 const char *myname = "event_default_enable_read";
 acl_msg_fatal("%s(%d): Not supported in %s",
  myname, 28, eventp->name);
}
 ...
 ACL_VSTREAM *stream __attribute__ ((__unused__)))
{
 ...
 acl_msg_fatal("%s(%d): Not supported %s",
  myname, 87, eventp->name);
 return (0);
}

static long long int event_default_timer_request(ACL_EVENT *eventp,
 ACL_EVENT_NOTIFY_TIME callback __attribute__ ((__unused__)), void *context __attribute__ ((__unused__)),
 long long int delay __attribute__ ((__unused__)), int keep __attribute__ ((__unused__)))
{
 const char *myname = "event_default_request_timer";
 acl_msg_fatal("%s(%d): Not supported in %s",
  myname, 97, eventp->name);
 return (0);
}

static long long int event_default_timer_cancel(ACL_EVENT *eventp,
 ACL_EVENT_NOTIFY_TIME callback __attribute__ ((__unused__)), void *context __attribute__ ((__unused__)))
{
 const char *myname = "event_default_cancel_timer";
 acl_msg_fatal("%s(%d): Not supported in %s",
  myname, 106, eventp->name);
 return (0);
}

static void event_default_timer_keep(ACL_EVENT *eventp,
 ACL_EVENT_NOTIFY_TIME callback __attribute__ ((__unused__)),
 void * ctx __attribute__ ((__unused__)), int keep __attribute__ ((__unused__)))
{
 const char *myname = "event_default_cancel_timer";
 acl_msg_fatal("%s(%d): Not supported in %s",
  myname, 116, eventp->name);
}

static int event_default_timer_ifkeep(ACL_EVENT *eventp,
 ACL_EVENT_NOTIFY_TIME callback __attribute__ ((__unused__)), void * ctx __attribute__ ((__unused__)))
{
 const char *myname = "event_default_timer_ifkeep";
 acl_msg_fatal("%s(%d): Not supported in %s",
  myname, 124, eventp->name);
 return 0;
}
...
static void event_default_add_dog(ACL_EVENT *eventp)
{
 ...
 acl_msg_fatal("%s(%d): Not supported in %s",
  myname, 132, eventp->name);
}

static void event_default_free(ACL_EVENT *eventp)
{
 do { if (eventp) { acl_free_glue("/workspace/acl/lib_acl/src/event/events_alloc.c", 137, (eventp)); (eventp) = 
# 137 "/workspace/acl/lib_acl/src/event/events_alloc.c" 3 4
((void *)0)
# 137 "/workspace/acl/lib_acl/src/event/events_alloc.c"
; } } while (0);
}
...
ACL_EVENT *event_alloc(size_t size)
{
 ...
 if (size < sizeof(ACL_EVENT))
  acl_msg_fatal("%s(%d): size(%d) too small",
   myname, 147, (int) size);
 eventp = acl_calloc_glue("/workspace/acl/lib_acl/src/event/events_alloc.c", 148, 1, size);

 do { if (sizeof(eventp->name) > 0) { size_t _n = strlen("events_alloc"); _n = _n > (size_t ) sizeof(eventp->name) - 1? (size_t) sizeof(eventp->name) - 1 : _n; memcpy(eventp->name, "events_alloc", _n); eventp->name[_n] = 0; } } while (0);
 eventp->init_fn = 
# 151 "/workspace/acl/lib_acl/src/event/events_alloc.c" 3 4
                               ((void *)0)
# 151 "/workspace/acl/lib_acl/src/event/events_alloc.c"
                                   ;
