# 1 "/workspace/acl/lib_acl/src/stdlib/sys/acl_process.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 2 "/workspace/acl/lib_acl/src/stdlib/sys/acl_process.c" 2
# 22 "/workspace/acl/lib_acl/src/stdlib/sys/acl_process.c"
static char *get_tls_buf(void)
{
 const char *myname = "get_tls_buf";
 static acl_pthread_key_t buf_key = (acl_pthread_key_t) 0xffffffff;
 char *buf;
 static char buf_unsafe[4096];

 buf = (char*) acl_pthread_tls_get(&buf_key);
 if (buf != 
# 30 "/workspace/acl/lib_acl/src/stdlib/sys/acl_process.c" 3 4
           ((void *)0)
# 30 "/workspace/acl/lib_acl/src/stdlib/sys/acl_process.c"
               )
  return (buf);

 if (buf_key == (acl_pthread_key_t) 0xffffffff) {
  acl_msg_warn("%s(%d): acl_pthread_tls_get error(%s), "
   "use unsafe buf", myname, 35, acl_last_serror());
  buf = buf_unsafe;
 } else {
  buf = (char*) acl_calloc_glue("/workspace/acl/lib_acl/src/stdlib/sys/acl_process.c", 38, 1, 4096);
  acl_pthread_tls_set(buf_key, buf,
   (void (*)(void*)) acl_free_fn_glue);
 }
 return (buf);
}
...
const char *acl_process_path()
{
 ...
 ret = readlink("/proc/self/exe", buf_ptr, 4096);
 if (ret < 0) {
  acl_msg_error("%s(%d): readlink error(%s)",
   myname, 55, acl_last_serror());
  return (
# 56 "/workspace/acl/lib_acl/src/stdlib/sys/acl_process.c" 3 4
         ((void *)0)
# 56 "/workspace/acl/lib_acl/src/stdlib/sys/acl_process.c"
             );
 }
 return (buf_ptr);
}
...
const char *acl_getcwd()
{
 ...
 ptr = getcwd(buf_ptr, 4096);
 if (ptr == 
# 68 "/workspace/acl/lib_acl/src/stdlib/sys/acl_process.c" 3 4
           ((void *)0)
# 68 "/workspace/acl/lib_acl/src/stdlib/sys/acl_process.c"
               ) {
  acl_msg_error("%s(%d): getcwd error(%s)",
   myname, 70, acl_last_serror());
 }
 return (ptr);
}
