...
struct ACL_CHAIN {
 long long int from_next;
 long long int off_begin;
...
{
 char *tmpbuf, *ptr;
 long long int to = from + dlen, n, chunk_to = chunk->from + chunk->dlen;
...
void acl_chain_set_from_next(ACL_CHAIN *chain, long long int from_next)
{
 ...
 { (&chain->sbuf)->vbuf.ptr = (&chain->sbuf)->vbuf.data; ((&(&chain->sbuf)->vbuf)->flags &= ~((1<<0) | (1<<1) | (1<<2))); };
}
...
int acl_chain_data_len(ACL_CHAIN *chain)
{
 return (int) ((size_t) ((&chain->sbuf)->vbuf.ptr - (&chain->sbuf)->vbuf.data));
}
 ...
 long long int from, int dlen)
{
 ...
 long long int to, n, chunk_to;

 to = from + dlen;

 if (to < chain->from_next) {
  acl_msg_warn("%s(%d): past data, to(" "%lld" ") < from_next("
   "%lld" "), from=" "%lld" ", dlen=%d",
   myname, 208, to, chain->from_next, from, dlen);
  return;
 }
...
void acl_chain_list(ACL_CHAIN *chain)
{
 ACL_RING_ITER ring_iter;
 ACL_CHUNK *chunk;
 long long int chunk_to;

 if ((size_t) ((&chain->sbuf)->vbuf.ptr - (&chain->sbuf)->vbuf.data) > 0)
  printf("from=" "%lld" ", next to=" "%lld" " , dlen=%d\r\n",
   chain->off_begin, chain->from_next, (int) (size_t) ((&chain->sbuf)->vbuf.ptr - (&chain->sbuf)->vbuf.data));
