...
#define SAVE_DATA(buff) do { assert(data && dlen > 0 && dlen_ > 0); for (; dlen_ > 0 && dlen > 0;) { buff += *data++; --dlen_; --dlen; } if (dlen_ > 0) { assert(dlen == 0); return dlen; } } while (0)
...
#define SAVE_LENGTH(x) do { assert(data && dlen > 0); unsigned short n; for (; x < HDR_LEN_LEN && dlen > 0;) { buff_[x++] = *data++; dlen--; } if (x < HDR_LEN_LEN) { assert(dlen == 0); return dlen; } if (!this->unpack_short(&buff_[0], 2, n)) { logger_error("unpack cid len error"); return -1; } x = n; } while (0)
