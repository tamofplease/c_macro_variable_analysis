...
#define logger_fatal(fmt,args...) acl::log::fatal4(__FILE__, __LINE__, __FUNCTION__, fmt, ##args)
...
#define PARSE_CONTENT_TYPE_HEADER(state,ptr) header_token(state->token, MIME_MAX_TOKEN, state->token_buffer, ptr, RFC2045_TSPECIALS, ';')
...
#define logger_debug(section,level,fmt,args...) acl::log::msg6(section, level, __FUNCTION__, __LINE__, __FUNCTION__, fmt, ##args)
...
#define logger_warn(fmt,args...) acl::log::warn4(__FILE__, __LINE__, __FUNCTION__, fmt, ##args)
...
#define REC_TYPE_WARN_FORMAT "%15ld"
#define LONG_BIT 64
...
#define PARSE_CONTENT_DISPOSITION(state,ptr) header_token(state->token, MIME_MAX_TOKEN, state->token_buffer, ptr, RFC2045_TSPECIALS, ';')
...
#define TOKEN_MATCH(tok,text) ((tok).type == HEADER_TOK_TOKEN && EQUAL((tok).u.value, (text)))
#define __INT8_C(c) c
...
#define REC_TYPE_SIZE_CAST5 long
...
#define logger(fmt,args...) acl::log::msg4(__FILE__, __LINE__, __FUNCTION__, fmt, ##args)
...
#define REC_TYPE_PTR_PAYL_SIZE 15
#define PF_XDP 44
...
#define REC_TYPE_DTXT 'w'
#define __statx_timestamp_defined 1
#define REC_TYPE_CTIME 'c'
...
#define REC_TYPE_TIME_SCAN(cp,tv) do { const char *_p = cp; (tv).tv_sec = atol(_p); while (ISDIGIT(*_p)) _p++; (tv).tv_usec = atol(_p); } while (0)
...
#define logger_error(fmt,args...) acl::log::error4(__FILE__, __LINE__, __FUNCTION__, fmt, ##args)
...
#define REC_TYPE_INSP 'I'
#define POSIX_FADV_RANDOM 1
#define REC_TYPE_ORCP 'O'
...
#define QP_ENCODE(buffer,ch) { buffer += '='; buffer += (char) hexchars[(ch >> 4) & 0xff]; buffer += (char) hexchars[ch & 0xf]; }
#define HDR_OPT_SENDER (1<<1)
