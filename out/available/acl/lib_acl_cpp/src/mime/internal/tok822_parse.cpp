...
#define RESET_NONADDR_LENGTH { start = (ssize_t) ACL_VSTRING_LEN(vp); addr = 0; addr_len = 0; }
...
#define ENFORCE_NONADDR_LENGTH do { if (addr && (ssize_t) ACL_VSTRING_LEN(vp) - addr_len > start + MAX_NONADDR_LENGTH) strip_address(vp, start, addr->head); } while(0)
...
#define NON_OPERATOR(x) (x->type == TOK822_ATOM || x->type == TOK822_QSTRING || x->type == TOK822_COMMENT || x->type == TOK822_DOMLIT || x->type == TOK822_ADDR)
...
#define QUOTE_FLAG_8BITCLEAN (1<<0)
...
#define quote_822_local(dst,src) quote_822_local_flags((dst), (src), QUOTE_FLAG_8BITCLEAN)
...
#define SKIP(tp,cond) { while (tp->type && (cond)) tp = tp->prev; }
...
#define MOVE_COMMENT_AND_CONTINUE(tp,right) { TOK822 *prev = tok822_unlink(tp); right = tok822_prepend(right, tp); tp = prev; continue; }
...
#define SKIP_MOVE_COMMENT(tp,cond,right) { while (tp->type && (cond)) { if (tp->type == TOK822_COMMENT) MOVE_COMMENT_AND_CONTINUE(tp, right); tp = tp->prev; } }
...
#define WNOHANG 1
#define COLLECT(t,s,c,cond) { while ((c = *(unsigned char *) s) != 0) { if (c == '\\') { if ((c = *(unsigned char *)++s) == 0) break; } else if (!(cond)) { break; } ACL_VSTRING_ADDCH(t->vstr, IS_SPACE_TAB_CR_LF(c) ? ' ' : c); s++; } ACL_VSTRING_TERMINATE(t->vstr); }
#define _XOPEN_SHM 1
#define PF_IPX 4
...
#define QUOTE_FLAG_EXPOSE_AT (1<<1)
