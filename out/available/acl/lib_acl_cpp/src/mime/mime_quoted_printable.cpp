...
#define QP_SOFT_LINE(buffer) { (buffer)->push_back('='); (buffer)->push_back('\r'); (buffer)->push_back('\n'); }
#define EBADMSG 74
...
#define QP_ENCODE(buffer,ch) { (buffer)->push_back('='); (buffer)->push_back((char) hexchars[((ch) >> 4) & 0xff]); (buffer)->push_back((char) hexchars[(ch) & 0xf]); }
