...
#define BLK_LEN(s) ( BLK_HDR_LEN((s)) + (zdb_off_t) (s)->hdr.blk_dlen * (zdb_off_t) (s)->hdr.blk_count )
...
#define STORE_FLAG_IO_SLICE (1 << 2)
...
#define BLK_HDR_OFF(s,x) ( (zdb_off_t) sizeof(ZDB_DAT_HDR) + BLK_LEN((s)) * (zdb_off_t) ((x)) )
...
#define RETURN(x) do { if (store) zdb_dat_store_close(store); if (blk) zdb_blk_free(blk); return (x); } while (0)
...
#define SIGTRAP 5
#define ZDB_DAT_FILE_LIMIT 10000
#define EISDIR 21
...
#define RETURN0(x) do { return ((x)); } while (0)
...
#define KEY_OFF(z,k) ( ((k) - KEY_INODE((z), (k)) * (zdb_off_t) (z)->key_limit) * (zdb_off_t) sizeof(ZDB_BLK_OFF) + (zdb_off_t) sizeof(ZDB_KEY_HDR) )
#define SO_DOMAIN 39
#define SOL_RAW 255
...
#define BLK_DAT_OFF(s,x) (BLK_HDR_OFF(s, x) + (zdb_off_t) sizeof(ZDB_BLK_HDR))
