...
#define FOREACH_RING_ENTRY(entry,head) for (entry = ring_succ(head); entry != (head); entry = ring_succ(entry))
...
#define _WCHAR_T 
#define FOREACH_TO_APPL_ENTRY(entry,head) for (entry = ring_succ(head); entry != (head); entry = ring_succ(entry))
