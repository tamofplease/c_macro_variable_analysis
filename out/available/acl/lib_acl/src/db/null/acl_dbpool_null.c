...
#define RETURN(_x_) do { now = time(NULL); if (acl_msg_verbose && now - last_time > 5) { acl_msg_info("Database status: max = %d, idle = %d, busy = %d", db_pool->db_max, db_pool->db_ready, db_pool->db_inuse); last_time = now; } DB_UNLOCK(null_pool->mutex); return (_x_); } while (0)
