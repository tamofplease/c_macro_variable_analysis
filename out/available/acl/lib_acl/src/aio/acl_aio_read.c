...
#define LOCK_DESTROY(mutex_in) do { acl_pthread_mutex_t *mutex_ptr = (mutex_in); (void) acl_pthread_mutex_destroy(mutex_ptr); } while (0)
...
#define EVENT_REG_ADD_WRITE(er,eh,fh,ctx) EVENT_REG_ADD_OP((er), (eh), (fh), (ctx), EPOLLOUT)
...
#define EVENT_REG_ADD_RDWR(er,eh,fh,ctx) EVENT_REG_ADD_OP((er), (eh), (fh), (ctx), EPOLLIN | EPOLLOUT)
#define AF_BLUETOOTH PF_BLUETOOTH
#define ACL_SET_EUGID_INCLUDE_H 
#define EVENT_REG_MOD_WRITE(er,eh,fh,ctx) EVENT_REG_MOD_OP((er), (eh), (fh), (ctx), EPOLLOUT)
#define L_tmpnam 20
...
#define READ_SAFE_DISABLE(x) do { if (((x)->flag & ACL_AIO_FLAG_ISRD) != 0) { (x)->flag &= ~ACL_AIO_FLAG_ISRD; (x)->can_read_fn = NULL; (x)->can_read_ctx = NULL; (x)->aio->event->disable_read_fn((x)->aio->event, (x)->stream); } } while (0)
...
#define TH_SYN 0x02
#define __unix__ 1
#define THREAD_LOCK(mutex_in) do { acl_pthread_mutex_t *mutex_ptr = (mutex_in); int status = acl_pthread_mutex_lock(mutex_ptr); if (status != 0) acl_msg_fatal("%s(%d): lock error(%s)", __FILE__, __LINE__, strerror(status)); } while (0)
#define IPV6_MTU 24
...
#define EVENT_REG_ADD_READ(er,eh,fh,ctx) EVENT_REG_ADD_OP((er), (eh), (fh), (ctx), EPOLLIN)
...
#define EVENT_REG_MOD_RDWR(er,eh,fh,ctx) EVENT_REG_MOD_OP((er), (eh), (fh), (ctx), EPOLLIN | EPOLLOUT)
#define PF_INET6 10
#define PATH_SEP_S "/"
...
#define SIGCONT 18
#define EVENT_BUFFER_READ(ev_cnt,eh,ev_buf,buflen,delay) do { (ev_cnt) = epoll_wait(eh, (ev_buf), (buflen), (delay) < 0 ? -1 : (delay)); } while (0)
#define MADV_FREE 8
...
#define EVENT_REG_MOD_READ(er,eh,fh,ctx) EVENT_REG_MOD_OP((er), (eh), (fh), (ctx), EPOLLIN)
...
#define LOCK_INIT(mutex_in) do { acl_pthread_mutexattr_t attr; acl_pthread_mutex_t *mutex_ptr = (mutex_in); acl_pthread_mutexattr_init(&attr); acl_pthread_mutexattr_settype(&attr, ACL_PTHREAD_MUTEX_RECURSIVE); acl_pthread_mutex_init(mutex_ptr, &attr); acl_pthread_mutexattr_destroy(&attr); } while (0)
...
#define EVENT_REG_ADD_OP(er,eh,fh,ctx,ev) EVENT_REG_FD_OP((er), (eh), (fh), (ctx), (ev), EPOLL_CTL_ADD)
#define F_EXLCK 4
...
#define FD_ENTRY_CLR(_fdp_) do { _fdp_->stream = NULL; _fdp_->r_callback = NULL; _fdp_->w_callback = NULL; _fdp_->r_context = NULL; _fdp_->w_context = NULL; _fdp_->r_ttl = 0; _fdp_->w_ttl = 0; _fdp_->r_timeout = 0; _fdp_->w_timeout = 0; _fdp_->listener = 0; _fdp_->event_type = 0; _fdp_->flag = 0; _fdp_->fdidx = -1; _fdp_->fdidx_ready = -1; _fdp_->fifo_info = NULL; } while (0);
...
#define EVENT_REG_INIT_HANDLE(eh,n) do { eh = epoll_create(n); } while (0)
...
#define EVENT_REG_DEL_BOTH(er,eh,fh) EVENT_REG_DEL_OP((er), (eh), (fh), 0)
...
#define READ_IOCP_CLOSE(x) do { READ_SAFE_DISABLE((x)); (x)->flag |= ACL_AIO_FLAG_IOCP_CLOSE; acl_aio_iocp_close((x)); } while (0)
#define LOCK_SH 1
...
#define EVENT_REG_DEL_OP(er,eh,fh,ev) EVENT_REG_FD_OP((er), (eh), (fh), NULL, (ev), EPOLL_CTL_DEL)
...
#define READ_SAFE_ENABLE(x,callback) do { if (((x)->flag & ACL_AIO_FLAG_ISRD) == 0) { (x)->flag |= ACL_AIO_FLAG_ISRD; (x)->aio->event->enable_read_fn((x)->aio->event, (x)->stream, (x)->timeout, callback, (x)); } } while (0)
...
#define EVENT_REG_MOD_OP(er,eh,fh,ctx,ev) EVENT_REG_FD_OP((er), (eh), (fh), (ctx), (ev), EPOLL_CTL_MOD)
...
#define EVENT_REG_FD_OP(er,eh,fh,ctx,ev,op) do { struct epoll_event dummy; memset(&dummy, 0, sizeof(dummy)); dummy.events = (ev) | EPOLLHUP | EPOLLERR; dummy.data.ptr = ctx; (er) = epoll_ctl(eh, (op), (fh), &dummy); } while (0)
...
#define SET_TIME(x) { struct timeval _tv; gettimeofday(&_tv, NULL); (x) = ((acl_int64) _tv.tv_sec) * 1000000 + ((acl_int64) _tv.tv_usec); }
...
#define THREAD_UNLOCK(mutex_in) do { acl_pthread_mutex_t *mutex_ptr = (mutex_in); int status = acl_pthread_mutex_unlock(mutex_ptr); if (status != 0) acl_msg_fatal("%s(%d): unlock error(%s)", __FILE__, __LINE__, strerror(status)); } while (0)
