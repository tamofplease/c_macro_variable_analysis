...
#define WRITE_SAFE_DIABLE(x) do { if (((x)->flag & ACL_AIO_FLAG_ISWR) != 0) { (x)->flag &= ~ACL_AIO_FLAG_ISWR; (x)->can_write_fn = NULL; (x)->can_write_ctx = NULL; acl_event_disable_write((x)->aio->event, (x)->stream); } } while (0)
...
#define WRITE_SAFE_ENABLE(x,callback) do { if (((x)->flag & ACL_AIO_FLAG_ISWR) == 0) { (x)->flag |= ACL_AIO_FLAG_ISWR; acl_event_enable_write((x)->aio->event, (x)->stream, (x)->timeout, callback, (x)); } } while (0)
...
#define WRITE_IOCP_CLOSE(x) do { WRITE_SAFE_DIABLE((x)); (x)->flag |= ACL_AIO_FLAG_IOCP_CLOSE; acl_aio_iocp_close((x)); } while (0)
