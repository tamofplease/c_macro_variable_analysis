...
#define RETURN(x) do { if (log->lock) { thread_mutex_unlock(log->lock); } return (x); } while (0)
...
#define PRIVATE_FIFO_PUSH(_fifo,_data) do { ACL_FIFO_INFO *_info; _info = (ACL_FIFO_INFO *) acl_default_malloc(__FILE__, __LINE__, sizeof(*_info)); _info->data = (_data); if ((_fifo)->tail == NULL) { _info->prev = _info->next = NULL; (_fifo)->head = (_fifo)->tail = _info; } else { (_fifo)->tail->next = _info; _info->prev = (_fifo)->tail; _info->next = NULL; (_fifo)->tail = _info; } (_fifo)->cnt++; } while (0)
