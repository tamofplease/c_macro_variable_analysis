...
#define SLICE_MBUF_SPACE(slice_in,incr,incr_real,mbuf_type) do { if (slice_in->mbufs == NULL) { incr_real = incr > 16 ? incr : 16; slice_in->mbufs = (mbuf_type**) __slice_malloc_fn(__FILE__, __LINE__, sizeof(mbuf_type*) * incr_real); slice_in->capacity = incr_real; slice_in->imbuf_avail = 0; } else if (slice_in->slice.nbuf + incr >= slice_in->capacity) { incr_real = incr > 16 ? incr : 16; slice_in->capacity += incr_real; slice_in->mbufs = (mbuf_type**) __slice_realloc_fn(__FILE__, __LINE__, slice_in->mbufs, sizeof(mbuf_type*) * slice_in->capacity); } } while (0)
...
#define ring_init(ring_in) do { RING *ring = ring_in; ring->pred = ring->succ = ring; } while (0)
#define _FCNTL_H 1
...
#define MBUF_SLOTS_SPACE(slice,mslots_in,incr,incr_real) do { MBUF_SLOTS *mslots = mslots_in; if (mslots->slots == NULL) { incr_real = incr < slice->page_nslots ? slice->page_nslots : incr; mslots->slots = (void **) __slice_malloc_fn(__FILE__, __LINE__, sizeof(void*) * incr_real); mslots->nslots = incr_real; mslots->islots = 0; } else if (mslots->islots + incr >= mslots->nslots) { incr_real = incr < slice->page_nslots ? slice->page_nslots : incr; mslots->nslots += incr_real; mslots->slots = (void **) __slice_realloc_fn(__FILE__, __LINE__, mslots->slots, sizeof(void*) * mslots->nslots); } } while (0)
...
#define ELNRNG 48
#define EAI_FAIL -4
#define FOREACH_RING_FORWARD(entry,head) for (entry = ring_succ(head); entry != (head); entry = ring_succ(entry))
...
#define FIRST_APPL(head,app_type,ring_member) (ring_succ(head) != (head) ? RING_TO_APPL(ring_succ(head), app_type, ring_member) : 0)
...
#define ring_append(ring_in,entry_in) do { RING *ring = ring_in; RING *entry = entry_in; entry->succ = ring->succ; entry->pred = ring; ring->succ->pred = entry; ring->succ = entry; } while (0)
...
#define ring_prepend(ring_in,entry_in) do { RING *ring = ring_in; RING *entry = entry_in; entry->pred = ring->pred; entry->succ = ring; ring->pred->succ = entry; ring->pred = entry; } while (0)
...
#define FOREACH_RING_BACKWARD(entry,head) for (entry = ring_pred(head); entry != (head); entry = ring_pred(entry))
#define TCP_INFO 11
#define PF_RDS 21
...
#define IPOPT_RR 7
#define ring_detach(entry_in) do { RING *entry = entry_in; RING *succ; RING *pred; succ = entry->succ; pred = entry->pred; if (succ != NULL && pred != NULL) { pred->succ = succ; succ->pred = pred; entry->succ = entry->pred = 0; } } while (0)
