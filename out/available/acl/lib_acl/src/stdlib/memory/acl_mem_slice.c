...
#define CHECK_OUT_PTR(_ptr,_real_ptr,_mem_slice,_len) { _real_ptr->signature = SIGNATURE; _real_ptr->mem_slice = _mem_slice; _real_ptr->length = _len; _ptr = _real_ptr->u.payload; }
...
#define CHECK_PTR(_ptr,_real_ptr,_len,_fname,_line) { if (_ptr == 0) { acl_msg_panic("%s(%d), %s: in %s(%d), null pointer input", __FILE__, __LINE__, __FUNCTION__, _fname, _line); } _real_ptr = (MBLOCK *) ((char *) _ptr - offsetof(MBLOCK, u.payload[0])); if (_real_ptr->signature != SIGNATURE) { acl_msg_panic("%s(%d), %s: in %s(%d), corrupt or unallocated memory block(%d, 0x%x, 0x%x)", __FILE__, __LINE__, __FUNCTION__, _fname, _line, (int) _real_ptr->length, _real_ptr->signature, SIGNATURE); } if ((_len = _real_ptr->length) < 1) { acl_msg_panic("%s(%d), %s: in %s(%d), corrupt memory block length", __FILE__, __LINE__, __FUNCTION__, _fname, _line); } }
...
#define PRIVATE_ARRAY_PUSH(a,ptr) do { if ((a)->count >= (a)->capacity) private_array_grow((a), (a)->count + 16); (a)->items[(a)->count++] = (ptr); } while (0)
...
#define CHECK_IN_PTR(_ptr,_real_ptr,_len,_fname,_line) { if (_ptr == 0) { acl_msg_panic("%s(%d), %s: in %s(%d), null pointer input", __FILE__, __LINE__, __FUNCTION__, _fname, _line); } _real_ptr = (MBLOCK *) ((char *) _ptr - offsetof(MBLOCK, u.payload[0])); if (_real_ptr->signature != SIGNATURE) { acl_msg_panic("%s(%d), %s: in %s(%d), corrupt or unallocated memory block(%d, 0x%x, 0x%x)", __FILE__, __LINE__, __FUNCTION__, _fname, _line, (int) _real_ptr->length, _real_ptr->signature, SIGNATURE); } _real_ptr->signature = 0; if ((_len = _real_ptr->length) < 1) { acl_msg_panic("%s(%d), %s: in %s(%d), corrupt memory block length", __FILE__, __LINE__, __FUNCTION__, _fname, _line); } }
...
#define WSTOPPED 2
#define CHECK_IN_PTR2(_ptr,_real_ptr,_len,_fname,_line) { if (_ptr == 0) { acl_msg_panic("%s(%d), %s: in %s(%d), null pointer input", __FILE__, __LINE__, __FUNCTION__, _fname, _line); } _real_ptr = (MBLOCK *) ((char *) _ptr - offsetof(MBLOCK, u.payload[0])); if (_real_ptr->signature != SIGNATURE) { acl_msg_panic("%s(%d)(CHECK_IN_PTR2): corrupt or unallocated memory block(%d, 0x%x, 0x%x)", _fname, _line, (int) _real_ptr->length, _real_ptr->signature, SIGNATURE); } if ((_len = _real_ptr->length) < 1) { acl_msg_panic("%s(%d), %s: in %s(%d) corrupt memory block length", __FILE__, __LINE__, __FUNCTION__, _fname, _line); } }
...
#define PRIVATE_ARRAY_POP(a,ptr) do { if ((a)->count > 0) { (a)->count--; (ptr) = (a)->items[(a)->count]; (a)->items[(a)->count] = NULL; } else { (ptr) = NULL; } } while (0)
