...
#define CHECK_PTR(_ptr_,_real_ptr_,_len_,_fname_,_line_) { if (_ptr_ == 0) acl_msg_fatal("%s(%d): null pointer input", _fname_, _line_); _real_ptr_ = (MBLOCK *) (((char*)_ptr_) - offsetof(MBLOCK, u.payload[0])); if (_real_ptr_->signature != SIGNATURE) acl_msg_fatal("%s(%d): corrupt or unallocated block(%d, 0x%x, 0x%x)", _fname_, _line_, (int) _real_ptr_->length, (int) _real_ptr_->signature, SIGNATURE); if ((_len_ = _real_ptr_->length) < 1) acl_msg_fatal("%s(%d): corrupt memory block length", _fname_, _line_); }
...
#define SET_FILE(_ptr_,_filename_) do { _ptr_ = strrchr(_filename_, '/'); if (_ptr_ == NULL) _ptr_ = filename; else _ptr_++; } while (0)
...
#define CHECK_OUT_PTR(_ptr_,_real_ptr_,_len_) { _real_ptr_->signature = SIGNATURE; _real_ptr_->length = _len_; _ptr_ = _real_ptr_->u.payload; }
...
#define CHECK_IN_PTR(_ptr_,_real_ptr_,_len_,_fname_,_line_) { if (_ptr_ == 0) acl_msg_fatal("%s(%d): null pointer input", _fname_, _line_); _real_ptr_ = (MBLOCK *) (((char*)_ptr_) - offsetof(MBLOCK, u.payload[0])); if (_real_ptr_->signature != SIGNATURE) acl_msg_fatal("%s(%d): corrupt or unallocated block(%d, 0x%x, 0x%x)", _fname_, _line_, (int) _real_ptr_->length, (int) _real_ptr_->signature, SIGNATURE); _real_ptr_->signature = 0; if ((_len_ = _real_ptr_->length) < 1) acl_msg_fatal("%s(%d): corrupt memory block length", _fname_, _line_); }
