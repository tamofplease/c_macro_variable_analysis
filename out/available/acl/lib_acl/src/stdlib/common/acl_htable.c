...
#define LOCK_TABLE_READ(_table) do { int _ret; if (_table->rwlock && (_ret = _RWLOCK_RDLOCK(_table->rwlock))) { acl_msg_fatal("%s(%d): read lock error(%s)", __FILE__, __LINE__, strerror(_ret)); } } while (0)
...
#define RETURN(x) do { if (keybuf) { if (table->slice) acl_slice_pool_free(__FILE__, __LINE__, keybuf); else acl_myfree(keybuf); } return (x); } while (0)
...
#define UNLOCK_TABLE(_table) do { int _ret; if (_table->rwlock && (_ret = _RWLOCK_UNLOCK(_table->rwlock))) { acl_msg_fatal("%s(%d): unlock error(%s)", __FILE__, __LINE__, strerror(_ret)); } } while (0)
...
#define htable_link(_table,_element,_n) { ACL_HTABLE_INFO **_h = _table->data + _n; _element->prev = 0; if ((_element->next = *_h) != 0) (*_h)->prev = _element; *_h = _element; _table->used++; }
...
#define LOCK_TABLE_WRITE(_table) do { int _ret; if (_table->rwlock && (_ret = _RWLOCK_WRLOCK(_table->rwlock))) { acl_msg_fatal("%s(%d): write lock error(%s)", __FILE__, __LINE__, strerror(_ret)); } } while (0)
