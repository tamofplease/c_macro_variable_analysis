...
#define LOCK_FS do { int __ret; if ((__flags & ACL_FHANDLE_F_LOCK ) != 0 && (__ret = MUTEX_LOCK(&__fhandle_mutex)) != 0) acl_msg_fatal("%s: lock fs error(%d)", myname, __ret); } while (0)
...
#define _STDDEF_H_ 
#define UNLOCK_FS do { int __ret; if ((__flags & ACL_FHANDLE_F_LOCK ) != 0 && (__ret = MUTEX_UNLOCK(&__fhandle_mutex)) != 0) acl_msg_fatal("%s: unlock fs error(%d)", myname, __ret); } while (0)
