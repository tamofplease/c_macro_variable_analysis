...
#define ERETURN(x) do { if (cfg_line == NULL) return (x); if (cfg_line->value == NULL) { acl_myfree(cfg_line); return (x); } for (i = 0; i < cfg_line->ncount; i++) { if (cfg_line->value[i] == NULL) break; acl_myfree(cfg_line->value[i]); } acl_myfree(cfg_line->value); acl_myfree(cfg_line); return (x); } while (0);
...
#define RETURN(x) do { if (filefd != ACL_FILE_INVALID) acl_file_close(filefd); return (x); } while (0);
...
#define SKIP(var,cond) for (; *var && (cond); var++) {}
#define _SYS_UN_H 1
