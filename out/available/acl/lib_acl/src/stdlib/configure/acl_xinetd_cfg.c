...
#define RETURN(x) { if (content_buf) acl_myfree(content_buf); return (x); }
...
#define ERETURN(x) { if (content_buf) acl_myfree(content_buf); if (xcp) { if (xcp->nv_array) acl_array_destroy(xcp->nv_array, __nv_pair_free); acl_myfree(xcp); } return (x); }
