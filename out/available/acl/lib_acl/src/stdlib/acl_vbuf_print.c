...
#define VBUF_STRCAT(bp,s) { const unsigned char *_cp = (const unsigned char *) (s); int _ch; while ((_ch = *_cp++) != 0) { ACL_VBUF_PUT((bp), _ch); } }
...
#define VBUF_SKIP(bp) { while ((bp)->ptr < (bp)->data + (bp)->len && *(bp)->ptr) { (bp)->ptr++; } }
...
#define SO_RCVBUF 8
#define CHECK_OVERFLOW(_i,_max) do { if (_i >= _max) acl_msg_fatal("fmt overflow: i: %d MAX: %d", (_i), (_max)); } while (0)
