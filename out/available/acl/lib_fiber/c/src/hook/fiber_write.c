...
#define RING_LAST_APPL(head,app_type,ring_member) (ring_pred(head) != (head) ? RING_TO_APPL(ring_pred(head), app_type, ring_member) : 0)
...
#define RING_PREPEND(ring_in,entry_in) do { RING *ring_ptr = (ring_in), *entry_ptr = (entry_in); entry_ptr->pred = ring_ptr->pred; entry_ptr->succ = ring_ptr; entry_ptr->parent = ring_ptr->parent; ring_ptr->pred->succ = entry_ptr; ring_ptr->pred = entry_ptr; ring_ptr->parent->len++; } while (0)
...
#define HTABLE_STAT_DUPLEX_KEY 2
...
#define FILE_ALLOC(__fe,__type,_fd) do { (__fe) = file_event_alloc(_fd); (__fe)->fiber_r->status = FIBER_STATUS_NONE; (__fe)->fiber_w->status = FIBER_STATUS_NONE; (__fe)->mask = (__type); (__fe)->type = TYPE_EVENTABLE; } while (0)
...
#define FOREACH_REVERSE(iter,container) for ((container)->iter_tail(&(iter), (container)); (iter).ptr; (container)->iter_prev(&(iter), (container)))
...
#define RING_DETACH(entry_in) do { RING *succ, *pred, *entry_ptr = (entry_in); succ = entry_ptr->succ; pred = entry_ptr->pred; if (succ != NULL && pred != NULL) { pred->succ = succ; succ->pred = pred; entry_ptr->parent->len--; entry_ptr->succ = entry_ptr->pred = NULL; } } while (0)
...
#define CHECK_WRITE_RESULT(_fe,_n) do { int _err; if (_n >= 0) { return _n; } _err = acl_fiber_last_error(); fiber_save_errno(_err); if (!error_again(_err)) { return -1; } if (wait_write(_fe) == -1) { return -1; } } while (0)
...
#define RING_LAST(head) (ring_pred(head) != (head) ? ring_pred(head) : 0)
#define __FLT32_IS_IEC_60559__ 2
#define PF_UNIX PF_LOCAL
#define PTHREAD_CANCEL_ASYNCHRONOUS PTHREAD_CANCEL_ASYNCHRONOUS
#define SAFE_STRNCPY(_obj,_src,_size) do { if (_size > 0) { size_t _n = strlen(_src); _n = _n > (size_t ) _size - 1? (size_t) _size - 1 : _n; memcpy(_obj, _src, _n); _obj[_n] = 0; } } while (0)
...
#define SET_TIME(x) do { struct timeval _tv; gettimeofday(&_tv, NULL); (x) = ((long long) _tv.tv_sec) * 1000 + ((long long) _tv.tv_usec)/ 1000; } while (0)
...
#define RING_APPEND(ring_in,entry_in) do { RING *ring_ptr = (ring_in), *entry_ptr = (entry_in); entry_ptr->succ = ring_ptr->succ; entry_ptr->pred = ring_ptr; entry_ptr->parent = ring_ptr->parent; ring_ptr->succ->pred = entry_ptr; ring_ptr->succ = entry_ptr; ring_ptr->parent->len++; } while (0)
#define ENOSYS 38
#define _SC_MEMLOCK_RANGE _SC_MEMLOCK_RANGE
#define IPV6_ORIGDSTADDR 74
#define ITER_INFO(iter,container) (container)->iter_info(&(iter), (container))
#define EDEADLK 35
...
#define _SS_SIZE 128
#define RING_FIRST(head) (ring_succ(head) != (head) ? ring_succ(head) : 0)
#define EL3RST 47
...
#define ring_append(r,e) do { ((RING*)(e))->succ = ((RING*)(r))->succ; ((RING*)(e))->pred = (RING*)(r); ((RING*)(e))->parent = ((RING*)(r))->parent; ((RING*)(r))->succ->pred = (RING*)(e); ((RING*)(r))->succ = (RING*)(e); ((RING*)(r))->parent->len++; } while (0)
...
#define RING_FIRST_APPL(head,app_type,ring_member) (ring_succ(head) != (head) ? RING_TO_APPL(ring_succ(head), app_type, ring_member) : 0)
...
#define BOOL_MAX 1
#define ring_prepend(r,e) do { ((RING*)(e))->pred = ((RING*)(r))->pred; ((RING*)(e))->succ = (RING*)(r); ((RING*)(e))->parent = ((RING*)(r))->parent; ((RING*)(r))->pred->succ = (RING*)(e); ((RING*)(r))->pred = (RING*)(e); ((RING*)(r))->parent->len++; } while (0)
...
#define FOREACH(iter,container) for ((container)->iter_head(&(iter), (container)); (iter).ptr; (container)->iter_next(&(iter), (container)))
...
#define ring_init(__ring) do { RING *_ring = __ring; _ring->pred = _ring->succ = _ring; _ring->parent = _ring; _ring->len = 0; } while (0)
#define _FCNTL_H 1
...
#define RING_FOREACH_REVERSE(iter,head_ptr) for ((iter).ptr = ring_pred((head_ptr)); (iter).ptr != (head_ptr); (iter).ptr = ring_pred((iter).ptr))
...
#define RING_FOREACH(iter,head_ptr) for ((iter).ptr = ring_succ((head_ptr)); (iter).ptr != (head_ptr); (iter).ptr = ring_succ((iter).ptr))
#define SOL_IRDA 266
...
#define ring_detach(e) do { RING *_succ, *_pred; if (((RING*)(e))->parent != (RING*)(e)) { _succ = ((RING*)(e))->succ; _pred = ((RING*)(e))->pred; if (_succ && _pred) { _pred->succ = _succ; _succ->pred = _pred; ((RING*)(e))->parent->len--; ((RING*)(e))->succ = (RING*)(e); ((RING*)(e))->pred = (RING*)(e); ((RING*)(e))->parent = (RING*)(e); ((RING*)(e))->len = 0; } } } while (0)
...
#define CHECK_SET_NBLOCK(_fd) do { if (var_hook_sys_api) { FILE_EVENT *_fe = fiber_file_get(_fd); if (_fe && IS_NDUBLOCK(_fe)) { non_blocking(_fd, NON_BLOCKING); CLR_NDUBLOCK(_fe); } } } while (0)
