...
#define HTTP_LEN_ROUND(_ctx) (_ctx->chunk_len > 0 ? (_ctx->chunk_len > _ctx->read_cnt + var_http_buf_size ? var_http_buf_size : _ctx->chunk_len - _ctx->read_cnt) : var_http_buf_size)
...
#define DISABLE_READ(x) do { acl_aio_disable_read((x)); } while (0)
