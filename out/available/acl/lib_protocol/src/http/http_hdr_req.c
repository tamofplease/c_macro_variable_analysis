...
#define PF_ALG 38
#define RETURN do { if (argv) acl_argv_free(argv); return; } while(0);
...
#define TRUNC_BLANK_NORETURN(_x_) do { char *_ptr_; while(*_x_ == ' ' || *_x_ == '\t') _x_++; _ptr_ = _x_; while (*_ptr_) { if (*_ptr_ == ' ' || *_ptr_ == '\t') { *_ptr_ = 0; break; } _ptr_++; } } while (0);
...
#define TRUNC_BLANK(_x_) do { char *_ptr_; while(*_x_ == ' ' || *_x_ == '\t') _x_++; if (*_x_ == 0) RETURN; _ptr_ = _x_; while (*_ptr_) { if (*_ptr_ == ' ' || *_ptr_ == '\t') { *_ptr_ = 0; break; } _ptr_++; } } while (0);
