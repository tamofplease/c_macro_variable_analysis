...
#define _VEC_CVT(src,dst,src_t,dst_t,n) do { for (int i = 0; i < n; ++i) { if (_ISNAN(((src_t *)src)[i], src_t)) ((dst_t *)dst)[i] = INT32_MIN; else ((dst_t *)dst)[i] = ((src_t *)src)[i]; } } while (0)
...
#define FP_NAN 0
#define PACKED_VEC_CVT(name,src_field,dst_field,src_t,dst_t,n) void vec_cvt ##name(NO_CPU, const union xmm_reg *src, union xmm_reg *dst) { _VEC_CVT(src->src_field, dst->dst_field, src_t, dst_t, n); memset(dst->dst_field + n, 0, sizeof(*dst) - n * sizeof(*dst->dst_field)); }
...
#define VEC_PACKED_OP(name,op,field,size,n) void vec_ ##name ##size(NO_CPU, union xmm_reg *src, union xmm_reg *dst) { for (int i = 0; i < n; ++i) { dst->field[i] op ##= src->field[i]; } }
...
#define VEC_CVT(name,src_t,dst_t) void vec_cvt ##name(NO_CPU, const src_t *src, dst_t *dst) { _VEC_CVT(src, dst, src_t, dst_t, 1); }
