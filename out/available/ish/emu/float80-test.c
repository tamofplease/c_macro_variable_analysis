...
#define _test(op,a,b) ua.ld = a; ub.ld = b; actual = f80_ ##op(ua.f, ub.f); expected = (long double) a cop_ ##op (long double) b; assertf(actual == expected, "f80_"#op"(%Le, %Le) = %s", ua.ld, ub.ld, actual ? "true" : "false")
