...
#define YYBACKUP(Token,Value) do if (yychar == YYEMPTY) { yychar = (Token); yylval = (Value); YYPOPSTACK (yylen); yystate = *yyssp; goto yybackup; } else { yyerror (&yylloc, scanner, extra, YY_("syntax error: cannot back up")); YYERROR; } while (0)
...
#define YYLLOC_DEFAULT(current,rhs,n) do { if ((n) > 0) current = (rhs)[1]; else current = -1; } while (0)
