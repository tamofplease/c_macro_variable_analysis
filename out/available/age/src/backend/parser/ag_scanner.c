...
#define _ERRNO_H 1
#define YY_DO_BEFORE_ACTION yyg->yytext_ptr = yy_bp; yyleng = (int) (yy_cp - yy_bp); yyg->yy_hold_char = *yy_cp; *yy_cp = '\0'; yyg->yy_c_buf_p = yy_cp;
...
#define update_location() do { yyextra.last_loc = yytext - yyextra.scan_buf; } while (0)
...
#define yy_set_bol(at_bol) { if ( ! YY_CURRENT_BUFFER ){ yyensure_buffer_stack (yyscanner); YY_CURRENT_BUFFER_LVALUE = yy_create_buffer( yyin, YY_BUF_SIZE , yyscanner); } YY_CURRENT_BUFFER_LVALUE->yy_at_bol = at_bol; }
#define WNOHANG 1
...
#define yy_set_interactive(is_interactive) { if ( ! YY_CURRENT_BUFFER ){ yyensure_buffer_stack (yyscanner); YY_CURRENT_BUFFER_LVALUE = yy_create_buffer( yyin, YY_BUF_SIZE , yyscanner); } YY_CURRENT_BUFFER_LVALUE->yy_is_interactive = is_interactive; }
...
#define YY_FATAL_ERROR(msg) do { ereport(ERROR, (errmsg_internal("%s", msg))); yy_fatal_error(NULL, NULL); } while (0)
#define PG_PORT_H 
