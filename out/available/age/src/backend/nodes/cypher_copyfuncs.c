...
#define COPY_SCALAR_FIELD(fldname) (extended_newnode->fldname = extended_from->fldname)
...
#define COPY_LOCALS(nodeTypeName) nodeTypeName *extended_newnode = (nodeTypeName *)newnode; nodeTypeName *extended_from = (nodeTypeName *)from; Assert(is_ag_node(newnode, nodeTypeName)); Assert(is_ag_node(from, nodeTypeName));
...
#define COPY_BITMAPSET_FIELD(fldname) (extended_newnode->fldname = bms_copy(extended_from->fldname))
...
#define COPY_NODE_FIELD(fldname) (extended_newnode->fldname = copyObject(extended_from->fldname))
...
#define COPY_STRING_FIELD(fldname) (extended_newnode->fldname = extended_from->fldname ? pstrdup(extended_from->fldname) : (char *) NULL)
