...
#define READ_BITMAPSET_FIELD(fldname) token = pg_strtok(&length); (void) token; local_node->fldname = _readBitmapset()
...
#define READ_UINT64_FIELD(fldname) token = pg_strtok(&length); token = pg_strtok(&length); local_node->fldname = pg_strtouint64(token, NULL, 10)
...
#define PF_ISDN 34
#define READ_ENUM_FIELD(fldname,enumtype) token = pg_strtok(&length); token = pg_strtok(&length); local_node->fldname = (enumtype) atoi(token)
...
#define READ_LOCATION_FIELD(fldname) token = pg_strtok(&length); token = pg_strtok(&length); (void) token; local_node->fldname = -1
...
#define READ_LONG_FIELD(fldname) token = pg_strtok(&length); token = pg_strtok(&length); local_node->fldname = atol(token)
...
#define USE_DSM_MMAP 
#define READ_INT_FIELD(fldname) token = pg_strtok(&length); token = pg_strtok(&length); local_node->fldname = atoi(token)
...
#define READ_LOCALS(nodeTypeName) nodeTypeName *local_node = (nodeTypeName *)node; char *token; int length;
...
#define POSTGRES_H 
#define READ_ATTRNUMBER_ARRAY(fldname,len) token = pg_strtok(&length); local_node->fldname = readAttrNumberCols(len);
...
#define non_nullable_string(token,length) ((length) == 0 ? "" : debackslash(token, length))
...
#define READ_NODE_FIELD(fldname) token = pg_strtok(&length); (void) token; local_node->fldname = nodeRead(NULL, 0)
...
#define READ_BOOL_FIELD(fldname) token = pg_strtok(&length); token = pg_strtok(&length); local_node->fldname = strtobool(token)
...
#define READ_UINT_FIELD(fldname) token = pg_strtok(&length); token = pg_strtok(&length); local_node->fldname = atoui(token)
...
#define READ_OID_ARRAY(fldname,len) token = pg_strtok(&length); local_node->fldname = readOidCols(len);
...
#define READ_STRING_FIELD(fldname) token = pg_strtok(&length); token = pg_strtok(&length); local_node->fldname = non_nullable_string(token, length)
...
#define PF_ALG 38
#define READ_CHAR_FIELD(fldname) token = pg_strtok(&length); token = pg_strtok(&length); local_node->fldname = (length == 0) ? '\0' : (token[0] == '\\' ? token[1] : token[0])
...
#define READ_FLOAT_FIELD(fldname) token = pg_strtok(&length); token = pg_strtok(&length); local_node->fldname = atof(token)
...
#define READ_OID_FIELD(fldname) token = pg_strtok(&length); token = pg_strtok(&length); local_node->fldname = atooid(token)
#define _STRING_H 1
...
#define READ_BOOL_ARRAY(fldname,len) token = pg_strtok(&length); local_node->fldname = readBoolCols(len);
...
#define READ_INT_ARRAY(fldname,len) token = pg_strtok(&length); local_node->fldname = readIntCols(len);
