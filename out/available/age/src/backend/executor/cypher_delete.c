...
#define Decrement_Estate_CommandId(estate) estate->es_output_cid--; estate->es_snapshot->curcid--;
...
#define Increment_Estate_CommandId(estate) estate->es_output_cid++; estate->es_snapshot->curcid++;
...
#define IsMVCCSnapshot(snapshot) ((snapshot)->satisfies == HeapTupleSatisfiesMVCC || (snapshot)->satisfies == HeapTupleSatisfiesHistoricMVCC)
...
#define InitNonVacuumableSnapshot(snapshotdata,xmin_horizon) ((snapshotdata).satisfies = HeapTupleSatisfiesNonVacuumable, (snapshotdata).xmin = (xmin_horizon))
...
#define InitToastSnapshot(snapshotdata,l,w) ((snapshotdata).satisfies = HeapTupleSatisfiesToast, (snapshotdata).lsn = (l), (snapshotdata).whenTaken = (w))
...
#define RelationAllowsEarlyPruning(rel) ( RelationNeedsWAL(rel) && !IsCatalogRelation(rel) && !RelationIsAccessibleInLogicalDecoding(rel) )
#define SIGKILL 9
