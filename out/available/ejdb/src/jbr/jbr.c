...
#define JBR_RC_REPORT(ret_,r_,rc_) do { if ((ret_) >= 500) iwlog_ecode_error3(rc_); const char *err = iwlog_ecode_explained(rc_); if (!iwn_http_response_write(r_, ret_, "text/plain", err, -1)) { ret_ = -1; } else { ret_ = 1; } } while (0)
...
#define API_COLL_UNLOCK(jbc_,rci_,rc_) do { rci_ = pthread_rwlock_unlock(&(jbc_)->rwl); if (rci_) IWRC(iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci_), rc_); API_UNLOCK((jbc_)->db, rci_, rc_); } while (0)
...
#define WS_HANDLE_PING_PONG 0x08U
...
#define IWN_WF_METHODS_ALL (IWN_WF_GET | IWN_WF_PUT | IWN_WF_POST | IWN_WF_DELETE | IWN_WF_HEAD | IWN_WF_OPTIONS | IWN_WF_PATCH)
...
#define API_WLOCK(db_,rci_) ENSURE_OPEN(db_); rci_ = pthread_rwlock_wrlock(&(db_)->rwl); if (rci_) return iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci_)
...
#define IW_READVNUMBUF64(buf_,num_,step_) do { (num_) = 0; int64_t _base_ = 1; int _i_ = 0; while (1) { if (((const signed char*) (buf_))[_i_] >= 0) { (num_) += _base_ * ((const signed char*) (buf_))[_i_]; break; } (num_) += _base_ * ~(((const signed char*) (buf_))[_i_]); _base_ <<= 7; _i_++; } (step_) = _i_ + 1; } while (0)
...
#define IW_SETVNUMBUF(len_,buf_,num_) do { int32_t _num_ = (num_); if (_num_ == 0) { ((signed char*) (buf_))[0] = 0; (len_) = 1; } else { (len_) = 0; while (_num_ > 0) { int _rem_ = _num_ & 0x7f; _num_ >>= 7; if (_num_ > 0) { ((signed char*) (buf_))[(len_)] = ~(_rem_); } else { ((signed char*) (buf_))[(len_)] = _rem_; } (len_)++; } } } while (0)
...
#define binn_map_foreach(map,id,value) binn_iter_init(&iter, map, BINN_MAP); while (binn_map_next(&iter, &id, &value))
...
#define IW_VNUMSIZE(num_) ((num_) < 0x80ULL ? 1 : (num_) < 0x4000ULL ? 2 : (num_) < 0x200000ULL ? 3 : (num_) < 0x10000000ULL ? 4 : (num_) < 0x800000000ULL ? 5 : (num_) < 0x40000000000ULL ? 6 : (num_) < 0x2000000000000ULL ? 7 : (num_) < 0x100000000000000ULL ? 8 : (num_) < 0x8000000000000000ULL ? 9 : 10)
#define IW_PATH_STR "/"
...
#define binn_list_foreach(list,value) binn_iter_init(&iter, list, BINN_LIST); while (binn_list_next(&iter, &value))
#define BINN_UINT8 0x20
#define __DECL_SIMD_atan2f128x 
#define IW_VNUMSIZE32(num_) ((num_) < 0x80ULL ? 1 : (num_) < 0x4000ULL ? 2 : (num_) < 0x200000ULL ? 3 : (num_) < 0x10000000ULL ? 4 : 5)
...
#define ENSURE_OPEN(db_) if (!(db_) || !((db_)->open)) { iwlog_error2("Database is not open"); return IW_ERROR_INVALID_STATE; }
...
#define IW_SETVNUMBUF64(len_,buf_,num_) do { int64_t _num_ = (num_); if (_num_ == 0) { ((signed char*) (buf_))[0] = 0; (len_) = 1; } else { (len_) = 0; while (_num_ > 0) { int _rem_ = _num_ & 0x7f; _num_ >>= 7; if (_num_ > 0) { ((signed char*) (buf_))[(len_)] = ~(_rem_); } else { ((signed char*) (buf_))[(len_)] = _rem_; } (len_)++; } } } while (0)
...
#define IW_RANGES_OVERLAP(IW_s1_,IW_e1_,IW_s2_,IW_e2_) ( ((IW_e1_) > (IW_s2_) && (IW_e1_) <= (IW_e2_)) || ((IW_s1_) >= (IW_s2_) && (IW_s1_) < (IW_e2_)) || ((IW_s1_) <= (IW_s2_) && (IW_e1_) >= (IW_e2_)))
...
#define API_RLOCK(db_,rci_) ENSURE_OPEN(db_); rci_ = pthread_rwlock_rdlock(&(db_)->rwl); if (rci_) return iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci_)
...
#define IW_READVNUMBUF64_2(buf_,num_) do { (num_) = 0; int64_t _base_ = 1; int _i_ = 0; while (1) { if (((const signed char*) (buf_))[_i_] >= 0) { (num_) += _base_ * ((const signed char*) (buf_))[_i_]; break; } (num_) += _base_ * ~(((const signed char*) (buf_))[_i_]); _base_ <<= 7; _i_++; } } while (0)
...
#define IW_READVNUMBUF(buf_,num_,step_) do { (num_) = 0; int32_t _base_ = 1; int _i_ = 0; while (1) { if (((const signed char*) (buf_))[_i_] >= 0) { (num_) += _base_ * ((const signed char*) (buf_))[_i_]; break; } (num_) += _base_ * ~(((const signed char*) (buf_))[_i_]); _base_ <<= 7; _i_++; } (step_) = _i_ + 1; } while (0)
...
#define API_UNLOCK(db_,rci_,rc_) rci_ = pthread_rwlock_unlock(&(db_)->rwl); if (rci_) IWRC(iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci_), rc_)
#define __UINT8_TYPE__ unsigned char
#define SCNuFAST16 __PRIPTR_PREFIX "u"
#define __KERNEL_OLD_TIMEVAL_MATCHES_TIMEVAL64 (__WORDSIZE == 64)
#define __WORDSIZE 64
#define __timer_t_defined 1
#define BINN_CURRENCY 0x83
...
#define API_WLOCK2(db_,rci_) rci_ = pthread_rwlock_wrlock(&(db_)->rwl); if (rci_) return iwrc_set_errno(IW_ERROR_THREADING_ERRNO, rci_)
...
#define EJDB2_GIT_REVISION "ea30bd84"
...
#define IW_CMP2(rv_,vp1_,vp1sz_,vp2_,vp2sz_) do { (rv_) = 0; int min_ = (vp1sz_) < (vp2sz_) ? (vp1sz_) : (vp2sz_); for (int i = 0; i < min_; i++) { (rv_) = (int) (((const uint8_t*) (vp1_))[i] - ((const uint8_t*) (vp2_))[i]); if (rv_) { break; } } } while (0)
#define _SC_SHRT_MAX _SC_SHRT_MAX
#define __INT64_TYPE__ long int
#define BC_DIM_MAX _POSIX2_BC_DIM_MAX
#define __FLT_MAX_EXP__ 128
#define KEY_PREFIX_IDXMETA "i."
#define BINN_DATE 0xA2
...
#define IW_CMP(rv_,vp1_,vp1sz_,vp2_,vp2sz_) do { (rv_) = 0; int min_ = (vp1sz_) < (vp2sz_) ? (vp1sz_) : (vp2sz_); for (int i = 0; i < min_; i++) { (rv_) = (int) (((const uint8_t*) (vp1_))[i] - ((const uint8_t*) (vp2_))[i]); if (rv_) { break; } } if ((rv_) == 0) (rv_) = (vp1sz_) - (vp2sz_); } while (0)
#define PRIdLEAST8 "d"
...
#define binn_object_foreach(object,key,value) binn_iter_init(&iter, object, BINN_OBJECT); while (binn_object_next(&iter, key, &value))
...
#define JQP_EXPR_NODE_HEAD jqp_unit_t type; struct JQP_EXPR_NODE *next; struct JQP_JOIN *join; void *opaque; uint8_t flags;
